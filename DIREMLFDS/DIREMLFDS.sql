...
DECLARE @MostRecent INT = (SELECT MAX(DirectorEmailParameterId) from cls..DirectorEmailParameters)
DECLARE @LowerLimit INT = (SELECT DelinquencyLowerLimit FROM CLS..DirectorEmailParameters where DirectorEmailParameterId = @MostRecent)
DECLARE @UpperLimit INT = (SELECT DelinquencyUpperLimit FROM CLS..DirectorEmailParameters where DirectorEmailParameterId = @MostRecent)
DECLARE @MaxNumber INT = (SELECT MaxEmails FROM CLS..DirectorEmailParameters where DirectorEmailParameterId = @MostRecent)

/*JAMS PART for inserting email campaign requests*/
DECLARE @EmailCampaignId INT = (SELECT EmailCampaignId FROM CLS.emailbtcf.EmailCampaigns WHERE LetterId = 'DIREMLFED.html')
