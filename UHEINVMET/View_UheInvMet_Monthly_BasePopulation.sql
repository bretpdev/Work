...
/****** Object:  View [UheInvMet].[Monthly_BasePopulation]    Script Date: 11/5/2019 1:18:40 PM ******/
SET ANSI_NULLS ON
GO
...
SELECT
	...
	CASE 
		...
		WHEN LN10.LC_STA_LON10 = 'D' THEN COALESCE(DATEDIFF(DAY,LN16.LD_DLQ_OCC, GETDATE()) + 1 - (DATEDIFF(DAY, CAST(DATEADD(DAY,-DAY(GETDATE()), CAST(GETDATE() AS DATE)) AS DATETIME),GETDATE())),0) /* get delinquency as of beginning of month*/
		...
		FROM 
			...
			( --rehab loans
				SELECT
					...
					LN09.LN_SEQ
				FROM 
					UDW..LN09_RPD_PIO_CVN LN09
				WHERE
					LN09.LD_LON_RHB_PCV IS NOT NULL OR REPLACE(CAST(LN09.IF_LON_SRV_DFL_LON AS VARCHAR(16)), ' ', '') != ''
			) LN09 
