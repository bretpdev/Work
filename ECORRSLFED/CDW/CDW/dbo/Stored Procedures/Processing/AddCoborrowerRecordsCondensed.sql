CREATE PROCEDURE [dbo].[AddCoborrowerRecordsCondensed]
...
FROM
	...
	INNER JOIN CLS..DuplicateLetters DL
		ON BorrLt20.RM_DSC_LTR_PRC = DL.Letter
		...
		FROM
			...
			(
				SELECT
					AY10.BF_SSN,
					AY10.PF_REQ_ACT,
					MAX(AY10_COB.LN_ATY_SEQ) AS LN_ATY_SEQ,
				...
				(
					...
					ON AY10.LD_ATY_REQ_RCV = AY10_COB.LD_ATY_REQ_RCV
					AND AY10.PF_REQ_ACT = AY10_COB.PF_REQ_ACT
					AND AY10.BF_SSN = AY10_COB.BF_SSN
				GROUP BY
					AY10.BF_SSN,
					AY10.PF_REQ_ACT
			)AY10
				ON AY10.BF_SSN = LN85.BF_SSN
				AND AY10.LN_ATY_SEQ = LN85.LN_ATY_SEQ
			INNER JOIN AY10_BR_LON_ATY AY10A
				ON AY10A.BF_SSN = AY10.BF_SSN
				AND AY10A.LN_ATY_SEQ = AY10.LN_ATY_SEQ
	) LN85
	...
	LEFT JOIN CLS.dbo.SystemLetterExclusions SLE
		ON SLE.LetterId = BorrLt20.RM_DSC_LTR_PRC
		AND SLE.SystemLetterExclusionReasonId = '3' --Generate by own script
	LEFT JOIN CLS.dbo.SpecialLetterRecipients SLR
		ON SLR.LetterId = BorrLt20.RM_DSC_LTR_PRC
	LEFT JOIN LT20_LTR_REQ_PRC_Coborrower EndrLt20
		...
		AND EndrLt20.InactivatedAt IS NULL
WHERE
		...
		(
			...
			(
				...
				(
					...
					AND SLR.LetterId IS NULL
				)
			)
		)
	)
	AND SLE.LetterId IS NULL
	AND SLR.LetterId IS NULL --Dont Add coborrower records for special letter recipient letters
	AND LN85.LF_ATY_RCP = BorrLt20.RF_SBJ_PRC 
