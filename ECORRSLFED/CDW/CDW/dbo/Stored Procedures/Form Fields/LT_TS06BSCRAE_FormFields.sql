CREATE PROCEDURE [dbo].[LT_TS06BSCRAE_FormFields]
...
DECLARE	@LetterId VARCHAR(10) = 'TS06BSCRAE'
DECLARE @PF_REQ_ACT VARCHAR(5) = (SELECT AC11.PF_REQ_ACT FROM AC11_ACT_REQ_LTR AC11 WHERE PF_LTR = @LetterId)

IF @IsCoborrower = 0
	BEGIN
		SELECT
			CONVERT(VARCHAR(10), LN72.LD_ITR_EFF_END, 101) [SCRA_RATE_EXPIRATION_DATE]
		FROM
			PD10_PRS_NME PD10
			...
			(	--Adding Active Interest Rate
				...
				WHERE
					...
					AND LN72.LD_ITR_EFF_END != '01/01/9999'
