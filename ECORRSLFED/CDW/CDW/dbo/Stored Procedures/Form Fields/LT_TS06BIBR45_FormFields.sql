CREATE PROCEDURE [dbo].[LT_TS06BIBR45_FormFields]
...
DECLARE	@LetterId VARCHAR(10) = 'TS06BIBR45'
...
IF @IsCoborrower = 0
	BEGIN
		SELECT DISTINCT
			...
			CONVERT(VARCHAR(10),
			CASE
				WHEN DAY(RS05.BD_ANV_QLF_IBR) <= LN66.DueDay THEN DATEADD(DAY, LN66.DueDay - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR)--set the day to DUE_DAY
				WHEN DAY(RS05.BD_ANV_QLF_IBR) > LN66.DueDay THEN DATEADD(MONTH, 1, DATEADD(DAY, LN66.DueDay - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR))--add a month and set the day to DUE_DAY
			END, 101) AS PermStandardBeginDate,
			'$' + CAST(LN66.LA_RPS_ISL AS VARCHAR) AS PermStandardAmount
		FROM 
			PD10_PRS_NME PD10
				...
				WHERE
					...
					(
						...
						(
							...
							AND LN65.LC_TYP_SCH_DIS IN('IP','IL')
