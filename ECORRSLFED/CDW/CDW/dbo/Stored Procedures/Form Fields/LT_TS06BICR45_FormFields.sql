CREATE PROCEDURE [dbo].[LT_TS06BICR45_FormFields]
...
DECLARE	@LetterId VARCHAR(10) = 'TS06BICR45'
...
IF @IsCoborrower = 0
	BEGIN
		SELECT DISTINCT
			...
			CONVERT(VARCHAR(10), CASE WHEN DAY(RS05.BD_ANV_QLF_IBR) <= LN66.DueDay
					THEN DATEADD(DAY, LN66.DueDay - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR)--set the day to DUE_DAY
				 WHEN DAY(RS05.BD_ANV_QLF_IBR) > LN66.DueDay
					THEN DATEADD(MONTH, 1, DATEADD(DAY, LN66.DueDay - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR))--add a month and set the day to DUE_DAY
			END, 101) AS AltPayDate,
		...
		FROM 
			...
			(
				SELECT
					LN65.BF_SSN,
					MAX(DAY(RS10.LD_RPS_1_PAY_DU)) AS DueDay,
				...
				WHERE
					...
					(
						(
							...
							AND LN65.LC_TYP_SCH_DIS in ('C1','C2','C3')
						) --Active CA with a CP pending if no requal
						OR
						(
							LN66.LN_GRD_RPS_SEQ = 1
							AND LN65.LC_TYP_SCH_DIS in ('CQ')
