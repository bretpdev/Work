...
BEGIN TRY
	...
	INSERT INTO ULS..ArcAddProcessing (ArcTypeId, AccountNumber, ARC, ScriptId, ProcessOn, Comment, IsReference, IsEndorser, ProcessingAttempts, CreatedAt, CreatedBy) --uncomment for promotion
	SELECT DISTINCT
		1 AS ArcTypeId, --Atd22ByLoan (Add arc by sequence number)
	...
	FROM
		(
			...
			FROM
				...
				INNER JOIN PD24_PRS_BKR PD24
					ON PD24.DF_PRS_ID = PD10.DF_PRS_ID
					AND PD24.DC_BKR_STA != '05' 
					AND PD24.DD_BKR_DCH_RCV IS NULL 
...
--select * from uls..ArcAddProcessing where ScriptId = 'IDRCHPT13';--for testing only