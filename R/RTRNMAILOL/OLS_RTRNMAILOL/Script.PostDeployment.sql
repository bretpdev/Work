...
BEGIN
    INSERT INTO rtrnmailol.BarcodeData(AccountIdentifier, LetterId, CreateDate, ReceivedDate, ProcessedAt, AddedAt, AddedBy, DeletedAt, DeletedBy)
    SELECT
	    RecipientId,
	    LetterId,
	    CreateDate,
...
SET
    SourceFile = 'RTRNMAILOL'
WHERE
    HtmlFileId = (SELECT HtmlFileId FROM ULS.emailbatch.HtmlFiles WHERE HtmlFile = 'RTRNMLEML.html')
...
SET
	...
	CommentId = NULL
WHERE
	ArcId = (SELECT ArcId FROM ULS.emailbatch.Arcs WHERE Arc = 'D030A')
	AND SourceFile = 'RTRNMAILOL'
