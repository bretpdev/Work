...
{
	class OneLinkCommaParser : IDemographicsParser
	{
		private ReflectionInterface RI { get; set; }
		public OneLinkCommaParser(ReflectionInterface ri)
		{
			RI = ri;
		}
		...
		public QueueTask Parse()
		{
			...
			{
				case "EMAIL":
					task = new QueueTask(RejectAction.Sources.EMAIL, SystemCode.Sources.ONELINK_EMAIL);
					break;
				case "AUTOPAY":
					task = new QueueTask(RejectAction.Sources.AUTOPAY, SystemCode.Sources.AUTOPAY);
					break;
				default:
					task = new QueueTask(RejectAction.Sources.DUDE, SystemCode.Sources.DUDE);
