Public Class frmBankruptcySearch
    Inherits System.Windows.Forms.Form

#Region " Windows Form Designer generated code "

    Public Sub New(ByVal TTestMode As Boolean, ByRef TCaller As frmMain)
        MyBase.New()
        ...
        SpecializedControls(0) = New SpecializedControl
        SpecializedControls(0).BTN = Select1
        SpecializedControls(0).TB = Results1
        SpecializedControls(1) = New SpecializedControl
        SpecializedControls(1).BTN = Select2
        SpecializedControls(1).TB = Results2
        SpecializedControls(2) = New SpecializedControl
        SpecializedControls(2).BTN = Select3
        SpecializedControls(2).TB = Results3
        SpecializedControls(3) = New SpecializedControl
        SpecializedControls(3).BTN = Select4
        SpecializedControls(3).TB = Results4
        SpecializedControls(4) = New SpecializedControl
        SpecializedControls(4).BTN = Select5
        SpecializedControls(4).TB = Results5
        SpecializedControls(5) = New SpecializedControl
        SpecializedControls(5).BTN = Select6
        SpecializedControls(5).TB = Results6
        SpecializedControls(6) = New SpecializedControl
        SpecializedControls(6).BTN = Select7
        SpecializedControls(6).TB = Results7
        SpecializedControls(7) = New SpecializedControl
        SpecializedControls(7).BTN = Select8
        SpecializedControls(7).TB = Results8
        SpecializedControls(8) = New SpecializedControl
        SpecializedControls(8).BTN = Select9
        SpecializedControls(8).TB = Results9
        SpecializedControls(9) = New SpecializedControl
        SpecializedControls(9).BTN = Select10
        SpecializedControls(9).TB = Results10
        TestMode = TTestMode
        Caller = TCaller
    End Sub
...
#End Region
    ...
    Private SpecializedControls(10) As SpecializedControl
    ...
    Private Sub ResetFormControls()
        ...
        While I < Me.Controls.Count
            ...
            If Me.Controls(I).GetType().ToString() = "System.Windows.Forms.TextBox" Then
                CType(Me.Controls(I), System.Windows.Forms.TextBox).Clear() 'clear text out of all text boxes
            End If
            I = I + 1
        End While
        'reset object values
        I = 0
        While I < 10
            SpecializedControls(I).AccNum = ""
            SpecializedControls(I).Tried = True
            I = I + 1
        End While
        Searchlbl.Enabled = True
        tbCaseNumber.Enabled = True
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        Dim CaseNumChars As New ArrayList
        Dim I As Integer
        Dim II As Integer
        Dim ActualConn As SqlClient.SqlConnection
        Dim Comm As New SqlClient.SqlCommand
        Dim Reader As SqlClient.SqlDataReader
        If tbCaseNumber.TextLength = 0 Then
            MessageBox.Show("You must provide a bankruptcy case number to search for.", "Bankruptcy Case Number Needed", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
        'be sure all characters are alpha-numeric
        CaseNumChars.AddRange(tbCaseNumber.Text.ToCharArray())
        While I < CaseNumChars.Count()
            If Char.IsLetterOrDigit(CChar(CaseNumChars(I))) = False Then
                MessageBox.Show("All characters for bankruptcy case number must be alpha-numeric.  Please try again.", "Non Alpha-Numeric Character Found", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Exit Sub
            End If
            I = I + 1
        End While
        'get data from DB
        If TestMode Then
            ActualConn = New SqlClient.SqlConnection("Data Source=OPSDEV;Initial Catalog=DOCID;Integrated Security=SSPI;")
        Else
            ActualConn = New SqlClient.SqlConnection("Data Source=NOCHOUSE;Initial Catalog=DOCID;Integrated Security=SSPI;")
        End If
        Comm.Connection = ActualConn
        ActualConn.Open()
        Comm.CommandText = "SELECT Acc, FName + ' ' + LName as TheName FROM Bankruptcy WHERE CaseID = '" + tbCaseNumber.Text + "'"
        Reader = Comm.ExecuteReader()
        'populate form controls
        While Reader.Read()
            If II = 10 Then
                MessageBox.Show("The application wasn't able to handle the number of return records.  Please contact Systems Support.", "Too many records", MessageBoxButtons.OK, MessageBoxIcon.Error)
                ResetFormControls()
                Me.Hide()
                Exit Sub
            End If
            SpecializedControls(II).TB.Text = Reader("TheName")
            SpecializedControls(II).AccNum = Reader("Acc")
            SpecializedControls(II).Tried = False
            SpecializedControls(II).TB.Enabled = True
            SpecializedControls(II).BTN.Enabled = True
            II = II + 1
        End While
        'if there were no results found in DB
        If II = 0 Then
            MessageBox.Show("No records found.", "No Results Found", MessageBoxButtons.OK, MessageBoxIcon.Information)
            tbCaseNumber.Focus()
            tbCaseNumber.SelectAll()
            Exit Sub
        End If
        Reader.Close()
        ActualConn.Close()
        'disable search form controls
        Searchlbl.Enabled = False
        tbCaseNumber.Enabled = False
        btnSearch.Enabled = False
        Label1.Enabled = False
    End Sub

    'handles all select button clicks
    Private Sub ButtonClickHandler(ByVal IDX As Integer)
        Dim I As Integer
        Dim AllTried As Boolean
        AllTried = True
        'move data to main form
        Caller.tbSSN.Text = SpecializedControls(IDX).AccNum
        SpecializedControls(IDX).BTN.Enabled = False
        SpecializedControls(IDX).TB.Enabled = False
        SpecializedControls(IDX).Tried = True
        'check if all buttons possible have been pushed
        While I < 10
            If SpecializedControls(I).Tried = False Then
                AllTried = False
            End If
            I = I + 1
        End While
        'if all have been pushed then close window else just activiate main window
        If AllTried = True Then
            ResetFormControls()
            Caller.btnSearch.Focus()
            Caller.Activate()
            Me.Hide()
        Else
            Caller.btnSearch.Focus()
    ...
    Private Sub Select3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select3.Click
        ButtonClickHandler(2)
    End Sub

    Private Sub Select4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select4.Click
        ButtonClickHandler(3)
    End Sub

    Private Sub Select5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select5.Click
        ButtonClickHandler(4)
    End Sub

    Private Sub Select6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select6.Click
        ButtonClickHandler(5)
    End Sub

    Private Sub Select7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select7.Click
        ButtonClickHandler(6)
    End Sub

    Private Sub Select8_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select8.Click
        ButtonClickHandler(7)
    End Sub

    Private Sub Select9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select9.Click
        ButtonClickHandler(8)
    End Sub

    Private Sub Select10_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Select10.Click
        ButtonClickHandler(9)
    End Sub

    'set focus to case number field when form is activated
    Private Sub frmBankruptcySearch_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
        tbCaseNumber.Focus()
    End Sub

    Private Sub frmBankruptcySearch_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
