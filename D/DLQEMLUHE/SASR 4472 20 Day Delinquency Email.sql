...
DECLARE @EmailCampaignIdB INT = (SELECT EC.EmailCampaignId FROM ULS.emailbatch.EmailCampaigns EC INNER JOIN ULS.emailbatch.HTMLFiles H ON H.HTMLFileId = EC.HTMLFileId INNER JOIN ULS.emailbatch.Comments C ON C.CommentId = EC.CommentId WHERE H.HTMLFile = '20DLDUDTUH.html' AND C.Comment = '20 day delinquency email sent to borrower')
DECLARE @EmailCampaignIdE INT = (SELECT EC.EmailCampaignId FROM ULS.emailbatch.EmailCampaigns EC INNER JOIN ULS.emailbatch.HTMLFiles H ON H.HTMLFileId = EC.HTMLFileId INNER JOIN ULS.emailbatch.Comments C ON C.CommentId = EC.CommentId WHERE H.HTMLFile = '20DLDUDTUH.html' AND C.Comment = '20 day delinquency email sent to endorser')
...
FROM
	...
	INNER JOIN UDW..LN16_LON_DLQ_HST LN16 
		...
		AND
		(
			LN16.LN_DLQ_MAX + 1 = 20
			OR
			(
				LN16.LN_DLQ_MAX + 1 IN(20,21,22,23)
