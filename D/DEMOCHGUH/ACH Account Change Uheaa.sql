--DEMOCHGUH: ACCOUNT CHANGE UHEAA
BEGIN TRY
	BEGIN TRANSACTION
		...
		SELECT
			...
			CONCAT(--emailbatch.EmailProcessing data
					CAST(PD10.DF_SPE_ACC_ID AS VARCHAR(10))
					,',',COALESCE(LTRIM(RTRIM(PD10.DM_PRS_1)),'')
					,',',EMAIL.EmailAddress,
					CONCAT(',Demo Change Notice sent to Borrower: ',
							CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ABA) != UDW.dbo.Decryptor(BR30.BF_EFT_ABA) THEN 'ACH Routing Number changed' END,
							CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ACC) != UDW.dbo.Decryptor(BR30.BF_EFT_ACC) THEN 'ACH Account Number changed' END
						)
				) AS EmailData,
			CONCAT('Demo Change Notice sent to Borrower: ',
					CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ABA) != UDW.dbo.Decryptor(BR30.BF_EFT_ABA) THEN CONCAT('ACH Routing Number changed by ', RTRIM(LN83.LF_LST_USR_LN83), ' via ', RTRIM(LN83.LF_LST_SRC_LN83),' ') END,
					CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ACC) != UDW.dbo.Decryptor(BR30.BF_EFT_ACC) THEN CONCAT('ACH Account Number changed by ', RTRIM(LN83.LF_LST_USR_LN83), ' via ', RTRIM(LN83.LF_LST_SRC_LN83)) END
				) AS ArcAddData,
		...
		FROM
			...
			INNER JOIN UDW..BR30_BR_EFT BR30_History
				ON BR30_History.BF_SSN = BR30.BF_SSN
				...
				AND (--decrypts account/routing numbers for accurate comparison
						UDW.dbo.Decryptor(BR30_History.BF_EFT_ABA) != UDW.dbo.Decryptor(BR30.BF_EFT_ABA)
						OR UDW.dbo.Decryptor(BR30_History.BF_EFT_ACC) != UDW.dbo.Decryptor(BR30.BF_EFT_ACC)
					)
		...
		SELECT DISTINCT
			...
			CONCAT('Demo Change Notice sent to Co-Borrower: ',
					CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ABA) != UDW.dbo.Decryptor(BR30.BF_EFT_ABA) THEN CONCAT('ACH Routing Number changed by ', RTRIM(LN83.LF_LST_USR_LN83), ' via ', RTRIM(LN83.LF_LST_SRC_LN83), ' ') END,
					CASE WHEN UDW.dbo.Decryptor(BR30_History.BF_EFT_ACC) != UDW.dbo.Decryptor(BR30.BF_EFT_ACC) THEN CONCAT('ACH Routing Number changed by ', RTRIM(LN83.LF_LST_USR_LN83), ' via ', RTRIM(LN83.LF_LST_SRC_LN83)) END
				) AS ArcAddData,
			IIF(EA.Arc IS NOT NULL, 1, 0) AS ArcNeeded
		FROM
			UDW..PD10_PRS_NME PD10
