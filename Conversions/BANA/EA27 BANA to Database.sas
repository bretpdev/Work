...
FILENAME EA27_IN "X:\Archive\BANA\TEST\Wave 4 Test Files\Files from AES\EA27 Test Files\File1*";
FILENAME ACH_IN "X:\Archive\BANA\Production Files\Supplemental Files\ACH_20160202152615.CSV";
FILENAME EMAIL_IN "X:\Archive\BANA\Production Files\Supplemental Files\EMAILUHEAA_20160202.txt";

/*DATA EMAIL_DATA; */
/* infile EMAIL_IN delimiter = ',' FIRSTOBS=2 MISSOVER DSD lrecl=32767 ;*/
/* input Client_Cd :$2. SSN :$9. Pkt :2. Borrower_Email_Address :$254.;*/
/*run;*/
/**/
/*DATA ACH_FILE; */
/* INFILE ACH_IN DELIMITER = ',' FIRSTOBS=2 MISSOVER DSD LRECL=32767 ;*/
/* INPUT SSN :$9. Pkt :2. Prof :2.  Cycle_Dt :YYMMDD10. PLID :1. Commonline_ID :$50. Loan_Identification :$50. Routng_No :$23. Acctyp :$1. AccNo :$23.;*/
/*RUN;*/
/**/
/*DATA ACH_FILE;*/
/*SET ACH_FILE;*/
/*FORMAT Cycle_Dt MMDDYY10.;*/
/*RUN;*/
...
IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'ACH_FILE'))
  DROP TABLE dbo.ACH_FILE  

IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'ACH_DATA'))
  DROP TABLE dbo.ACH_DATA 

IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'EMAIL_DATA'))
  DROP TABLE dbo.EMAIL_DATA  
);
DISCONNECT FROM EA27;
QUIT;

/*DATA EA27.EMAIL_DATA;*/
/*SET EMAIL_DATA;*/
/*RUN;*/
/**/
/*DATA EA27.ACH_FILE;*/
...
/*	Commonline_ID,*/
/*	Loan_Identification,*/
/*	dbo.Encryptor(Routng_No) AS EncryptedRoutingNumber,*/
/*	dbo.Encryptor(AccNo) AS EncryptedAccountNumber,*/
/*	Acctyp*/
/*INTO ACH_DATA*/
/*FROM ACH_FILE;*/
/**/
...
DATA _01BorrowerRecord(keep=BorrowerSSN loan_number PACKETNumber OriginalSSN StudentSSN SeparationDate NumberMonthsGrace GuarantorCode LoanStatusCode DriverLicenseNumber 
	...
	_05SupplementalBorrowerRecord(KEEP=BorrowerSSN loan_number HomePhoneNumber AlternatePhoneNumber	HomePhoneCode 
		...
		SchoolCode GradationFactor NextGradationDate SchoolCampusCode IBREligibilityIndicator PartialFinancialHardshipAmount PermanentStandardPayAmount 
...
;
	...
	IF RECORD_TYP = '05' THEN do;
		INPUT HomePhoneNumber $ 13-22
			...
			ClaimCode $54
			SchoolCode $ 56-61
			@63 GradationFactor 9.5
			@72 NextGradationDate mmddyy6.
		...
		format AddressEffectiveDate AddressStatusDate IBRForgiveStartDate IBRCreateDate RehabilitationDate 
			...
			PFHBeginDate PFHEndDate FuturePFHBeginDate FuturePFHEndDate BorrowersOldestDisbursement NextGradationDate mmddyy10.;
	...
	IF RECORD_TYP = '07' THEN do;
		INPUT DisbursementNumber 3-4
			...
			@41 BorrowerPaidInterestAmount 7.2
			@48 PrincipalBalanceOutstanding 8.2
			@56 BorrowerAccruedInterest 7.2
			@63 InterestReceivableGovernment 7.2
			BenefitCode $ 70
