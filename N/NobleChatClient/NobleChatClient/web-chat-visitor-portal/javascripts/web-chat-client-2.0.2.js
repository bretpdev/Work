...
WebChatClient.prototype.connect = function (fromUser, socketIoStatusCallback, xmppServerStatusCallback, callback) {
    ...
    var setupSocketIOAndPostConnect = function(service)
    {
        ...
        socket.on('connection', function (s) {
            self.log('webChatClient connected to socket io.');
        });

        socket.on('disconnect', function (s) {
            self.log('webChatClient disconnected from socket io.');
            socketIoStatusCallback('The browser has been disconnected from the chat gateway.');
        });

         socket.on('xmpp status', function (data) {
            self.log('webChatClient received xmpp status through socket io: ', data.state);
            xmppServerStatusCallback(data);
        });

        socket.on('chat message', function (msg) {
            self.log('webChatClient received through socket io message: ', msg);
            self.updateCallback([msg], []);
        });

        socket.on('chat presence', function (msg) {
            self.log('webChatClient received through socket io presence: ', msg);
            self.updateCallback([], [msg]);
        });

        socket.on('chat status', function (msg) {
            self.log('webChatClient received through socket io status: ', msg);
            self.updateCallback([], [msg]);
        });

        $.post(service.chatApiUrl + "connect", { user: fromUser }, function (data) {
...
WebChatClient.prototype.joinConference = function (conferenceName, fromUser, callback) {
    sendMessage(this, "joinConference", { connectionId: connectionId, conferenceName: conferenceName, user: fromUser }, callback);
};

WebChatClient.prototype.joinConferenceAndWaitOnCustomerConference = function (conferenceName, fromUser, callback) {
    sendMessage(this, "joinConferenceAndWaitOnCustomerConference", { connectionId: connectionId, conferenceName: conferenceName, user: fromUser }, callback);
};

WebChatClient.prototype.cancelCustomerWaitForConference = function (conferenceName, fromUser, callback) {
    sendMessage(this, "cancelCustomerWaitForConference", { connectionId: connectionId, conferenceName: conferenceName, user: fromUser }, callback);
};

WebChatClient.prototype.leaveConference = function (conferenceName, fromUser, callback) {
    sendMessage(this, "leaveConference", { connectionId: connectionId, conferenceName: conferenceName, user: fromUser }, callback);
};
