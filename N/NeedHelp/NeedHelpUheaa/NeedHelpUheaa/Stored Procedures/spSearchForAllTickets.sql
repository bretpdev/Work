CREATE PROCEDURE [dbo].[spSearchForAllTickets]
	@SearchOption						VARCHAR(50) = 'AllTickets',
...
BEGIN
	...
	WHERE	--If all tickets was selected then return all tickets without any other filtering criteria
			...
			(@SearchOption = 'OpenTickets' AND tk.[Status] NOT IN ('Withdrawn', 'Resolved', 'Complete', 'Closed')) OR
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spSearchForAllTickets] TO [db_executor]
    AS [dbo];
