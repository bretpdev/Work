CREATE PROCEDURE [dbo].[spSearchForTickets]
	@SearchOption						VARCHAR(50) = 'UserSearchCriteria',
...
BEGIN
	...
	WHERE	--If all tickets was selected then return all tickets without any other filtering criteria
			(@SearchOption = 'AllTickets' AND (@Type = '' OR @Type = tk.TicketCode)) OR
			...
			(@SearchOption = 'OpenTickets' AND tk.[Status] NOT IN ('Withdrawn', 'Resolved', 'Complete', 'Closed', 'Verified') AND (@Type = '' OR @Type = tk.TicketCode)) OR
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spSearchForTickets] TO [db_executor]
    AS [dbo];
