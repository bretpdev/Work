...
/****** Object:  View [uhedaymet].[Daily_Military]    Script Date: 11/5/2019 1:00:23 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [uhedaymet].[Daily_Military]
...
FROM
	...
	(
		...
		WHERE
			LN72.LC_STA_LON72 = 'A'
			AND CAST(GETDATE() AS DATE) BETWEEN LN72.LD_ITR_EFF_BEG AND LN72.LD_ITR_EFF_END /*last day prev month*/
	) LN72
	...
	(
		...
		FROM
			...
			INNER JOIN UDW..LN50_BR_DFR_APV LN50
				...
				AND CAST(GETDATE() AS DATE) BETWEEN LN50.LD_DFR_BEG AND LN50.LD_DFR_END /*last day prev month*/
