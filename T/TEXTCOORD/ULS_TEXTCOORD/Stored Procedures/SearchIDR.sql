CREATE PROCEDURE [textcoord].[SearchIDR]
	@NumberToSend INT,
	@DelqLower INT,
	@DelqUpper INT,
	@AgeLower INT,
	...
	(
		...
		FROM
			...
			(
				...
				WHERE
					BL.PerformanceCategory NOT IN ('PIF','TRN','PIFPRV','TRNPRV','PRV') 
			)BL
			...
			(
				...
				FROM
					UDW..AY10_BR_LON_ATY AY10
				WHERE
					AY10.PF_REQ_ACT IN ('IBDNY', 'IBPND')
					AND CAST(AY10.LD_ATY_REQ_RCV AS DATE) >= CAST(DATEADD(DAY,-7,GETDATE()) AS DATE)
					AND AY10.LC_STA_ACTY10 = 'A'
			) IdrArc
				ON IdrArc.BF_SSN = PD10.DF_PRS_ID
			LEFT JOIN
			(
				SELECT
					BF_SSN,
