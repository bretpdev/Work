...
%LET RPTLIB = T:\SAS;
FILENAME REPORTZ "&RPTLIB/UNWQ02.NWQ02RZ";
FILENAME REPORT2 "&RPTLIB/UNWQ02.NWQ02R2";
FILENAME REPORT3 "&RPTLIB/UNWQ02.NWQ02R3";
FILENAME REPORT4 "&RPTLIB/UNWQ02.NWQ02R4";
...
DATA RPHN;
	SET WORK.PHN;
	WHERE SUBSTR(DF_SPE_ACC_ID,1,1) = 'P';
RUN;
...
DATA RPHN;
	SET RPHN;
	IF DC_ALW_ADL_PHN = 'L' THEN RNUM = 'R5';
	ELSE RNUM = 'R4';
	RUN_DATE = TODAY();
RUN;
...
DATA NRPHN;
	SET NRPHN;
	IF DC_ALW_ADL_PHN = 'L' THEN RNUM = 'R3';
	ELSE RNUM = 'R2';
	RUN_DATE = TODAY();
RUN;
...
IF _N_ = 1 THEN
	DO;
		PUT	'Total Unknown Phone Numbers'
			','
			'Total Landline Phone Numbers'
			','
			'Total Phone Numbers';
	END;
/* write data*/	
