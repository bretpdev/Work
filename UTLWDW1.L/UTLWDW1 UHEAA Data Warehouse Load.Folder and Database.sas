...
%LET LOGLIB = X:\Reports\LSS\Logs\UDW;
%LET TESTMSG = - THIS IS LIVE;

/*TEST*/
/*%LET RPTLIB = T:\sas;*/
/*%LET LOGLIB = X:\PADU\SAS\UDW\Logs;*/
/*%LET TESTMSG = - THIS IS A TEST;*/

/*Specify a back-up location for the report files here:*/
%LET RPTLIBX = X:\archive\sas;
/*%LET RPTLIBX = T:\sas\archive;*/

/*LIVE*/
%LET MD = %STR(REQUIRED="FILEDSN=X:\PADR\ODBC\UDW.dsn; update_lock_typ=nolock; bl_keepnulls=no");
%LET BSYS = %STR(REQUIRED="FILEDSN=X:\PADR\ODBC\BSYS.dsn; update_lock_typ=nolock; bl_keepnulls=no");

/*TEST*/
/*%LET MD = %STR(REQUIRED="FILEDSN=X:\PADR\ODBC\UDW_TEST.dsn; update_lock_typ=nolock; bl_keepnulls=no");*/
/*%LET BSYS = %STR(REQUIRED="FILEDSN=X:\PADR\ODBC\BSYS_TEST.dsn; update_lock_typ=nolock; bl_keepnulls=no");*/
...
FILENAME STATMESS EMAIL to=("&stat");
...
%MACRO FILECHECK(i);
	%GLOBAL FILEREF;
	%LET FILEREF = &RPTLIB/ULWDW1.LWDW1R&I..*;
	FILENAME REPORT&I "&RPTLIB/ULWDW1.LWDW1R&I..*";
	%IF %SYSFUNC(FEXIST(REPORT&I)) = 0 %THEN %DO;
			%PUT "The file was not in FTP, checking in archive.";
		FILENAME REPORT&I "&RPTLIBX/ULWDW1.LWDW1R&I..&day_of_year";
		%IF %SYSFUNC(FEXIST(REPORT&I)) = 0 %THEN %DO;
	...
	%IF &k > 0 %THEN %DO;
		DATA _NULL_;
		FILE ERRMESS subject="UDW Error - Missing File&TESTMSG";
			PUT "The UDW Update Process was Aborted for Report &k!";
...
%IF &error > 0 %THEN %DO;
DATA _NULL_;
FILE ERRMESS SUBJECT= 'UDW Error - Corrupted Data';
	PUT "The UDW Update Process was Aborted:";
...
RUN;
%MEND;
%MACRO DELETEALLROWS(SQLTBL);
PROC SQL; 
	DELETE FROM UDW.&SQLTBL;
QUIT;
%MEND;
PROC FORMAT LIBRARY=WORK.FORMATS CNTLIN=UDW.FORMATTRANSLATION; RUN;
