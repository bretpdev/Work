CREATE PROCEDURE [barcodefed].[GetCaliforniaBorForEnd]
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	SELECT DISTINCT
		...
		COALESCE(PD32.DX_ADR_EML, PD32.DX_CNC_EML_ADR) [Recipient],
		RTRIM(PD10_Bor.DM_PRS_1) [FirstName],
		RTRIM(PD10_Bor.DM_PRS_LST) [LastName],
		--PD10_Bor.DF_SPE_ACC_ID + ',' + RTRIM(PD10_Bor.DM_PRS_1) + ' ' + RTRIM(PD10_Bor.DM_PRS_LST) + ',' + COALESCE(PD32.DX_ADR_EML, PD32.DX_CNC_EML_ADR) [EmailData],
		1 [ArcNeeded],
	...
	FROM
		...
		INNER JOIN PD30_PRS_ADR PD30
			ON PD10_Bor.DF_PRS_ID = PD30.DF_PRS_ID
		INNER JOIN 
			( -- email address
				SELECT
					*,
			...
			ON PD10_Bor.DF_PRS_ID = PD32.DF_PRS_ID
