...
(
	...
	FROM
		...
		(
			SELECT
				...
				ABS(MIN(
						CASE
							...
							WHEN TILP_CREDIT.LD_FAT_APL_DAYS IS NOT NULL THEN TILP_CREDIT.LD_FAT_APL_DAYS
							ELSE NULL
						END
						)
					) AS LD_FAT_APL_DAYS
			FROM
				...
				(
					SELECT
						...
						LN90.LN_SEQ,
						CASE 
							WHEN LN90.PC_FAT_SUB_TYP = '54' AND LN90.LC_STA_LON90 = 'A' AND LN90.PC_FAT_TYP = '10' AND ISNULL(LN90.LC_FAT_REV_REA, '') = '' THEN 20
							ELSE NULL
						...
						LN90.BF_SSN NOT IN
							(
								...
								FROM
									UDW..LN90_FIN_ATY LN90
								WHERE
									(LN90.PC_FAT_TYP = '10' AND LN90.PC_FAT_SUB_TYP = '54' AND LN90.LC_STA_LON90 = 'A' AND ISNULL(LN90.LC_FAT_REV_REA, '') = '')
									OR
									(LN90.PC_FAT_TYP = '10' AND LN90.PC_FAT_SUB_TYP != '54' AND LN90.LC_STA_LON90 = 'A' AND ISNULL(LN90.LC_FAT_REV_REA, '') = '')
							)
				) TILP_CREDIT
					ON TILP_CREDIT.BF_SSN = LN10.BF_SSN
					AND TILP_CREDIT.LN_SEQ = LN10.LN_SEQ
				LEFT JOIN
				(
					SELECT DISTINCT
						LN90.BF_SSN,
						...
						LN90.BF_SSN NOT IN
							(
								...
								GROUP BY
									...
									LN90.LD_FAT_APL
								HAVING
									DATEDIFF(DAY, LN90.LD_FAT_APL, GETDATE()) < 21 AND SUM(LN10.LA_CUR_PRI + LN10.LA_NSI_OTS) < 50
							)
				) ADJUSTMENT
			...
			WHERE
				LN10.LC_STA_LON10 = 'R'
				AND (LN10.LA_CUR_PRI + LN10.LA_NSI_OTS) BETWEEN .01 AND 49.99
				AND (LN10.LA_CUR_PRI + LN10.LA_NSI_OTS) != LN15.DSB_AMT
				AND TOTAL_BAL_TILP.BAL < 50
				AND (ISNULL(TOTAL_BAL_NONTILP.BAL, 0) + TOTAL_BAL_TILP.BAL) < 74.99 --Check if the NON-TILP balance is null and set to 0 then add it to the tilp balance to make sure they are under $75
	...
	WHERE
		(POP.[STATUS] = 'CONSOLIDATED' AND POP.BAL < 50)
		OR
		(
			POP.[STATUS] = 'NOT CONSOLIDATED'
			AND POP.LD_FAT_APL_DAYS > 20 AND POP.BAL < 50
		)
)L
