CREATE PROCEDURE [dbo].[LT_US06BCAP_Loans]
	@LetterId VARCHAR(10),
	@BF_SSN VARCHAR(9),
	@IsCoborrower BIT = 0,
	@RN_ATY_SEQ_PRC INT
...
IF @IsCoborrower = 0
	BEGIN
		SELECT
			...
			'$' + CONVERT(VARCHAR(15), ISNULL(LN90Cap.LA_FAT_CUR_PRI, 0.00), 1) AS Amount_Capitalized,
			'$' + CONVERT(VARCHAR(15), ISNULL(LN90.TAPTP,0), 1) AS TAPTP,
			'$' + CONVERT(VARCHAR(15), ISNULL(LN90.TAPTI,0), 1) AS TAPTI,
			'$' + CONVERT(VARCHAR(15), ISNULL(LN90.TAPTF,0), 1) AS TAPTF,
			'$' + CONVERT(VARCHAR(15), ISNULL(LN90.TAGAP,0), 1) AS TAGAP
		FROM
			LN10_LON LN10
			INNER JOIN
			(
			...
			(
				SELECT
					LN90.BF_SSN,
					LN90.LN_SEQ,
					ABS(CAST(ISNULL(LN90.LA_FAT_CUR_PRI,0) AS DECIMAL (18,2))) AS LA_FAT_CUR_PRI
				FROM 
					LN90_FIN_ATY LN90
					INNER JOIN
					(
						...
						AND MaxLN90.LN_SEQ = LN90.LN_SEQ
						AND MaxLN90.LD_FAT_EFF = LN90.LD_FAT_EFF
				WHERE
					LN90.PC_FAT_TYP = '70'
				...
				ON LN90Cap.BF_SSN = LN10.BF_SSN
				AND LN90Cap.LN_SEQ = LN10.LN_SEQ
