CREATE PROCEDURE [dbo].[LT_USTSMSTMT_FormFields]
	@BF_SSN  VARCHAR(9),
	@IsCoborrower BIT = 0,
	@RN_ATY_SEQ_PRC INT
AS
...
DECLARE @PF_REQ_ACT VARCHAR(5) = (SELECT AC11.PF_REQ_ACT FROM AC11_ACT_REQ_LTR AC11 WHERE PF_LTR = 'USTSMSTMT')
...
IF @IsCoborrower = 0 --borrower
	BEGIN
		SELECT 
			@TaxYear AS TaxYear,
			REPLACE(FORMAT(SUM(ISNULL(MR64.LA_BR_INT_PD_YR,0)), 'C'), ',', '') AS TotalYearlyInterestPaidAmount
		FROM
			...
			(
				...
				FROM
					...
					(
						...
						FROM			
							MR64_BR_TAX MR64
						WHERE
							MR64.LF_TAX_YR = @TaxYear
							AND MR64.BF_SSN = @BF_SSN
				...
				AND MR64.LN_SEQ = LN10.LN_SEQ
				AND MR64.LF_TAX_YR = @TaxYear
		WHERE
