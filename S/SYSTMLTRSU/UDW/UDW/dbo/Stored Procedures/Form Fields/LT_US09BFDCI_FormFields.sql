CREATE PROCEDURE [dbo].[LT_US09BFDCI_FormFields]
	@BF_SSN  VARCHAR(9),
	@IsCoborrower BIT = 0,
	@RN_ATY_SEQ_PRC INT
AS
...
DECLARE @PF_REQ_ACT VARCHAR(5) = (SELECT AC11.PF_REQ_ACT FROM AC11_ACT_REQ_LTR AC11 WHERE PF_LTR = 'US09BFDCI')

--Special case letter going to coborrower
	SELECT DISTINCT
		LTRIM(RTRIM(PD10.DM_PRS_1)) + ' ' + LTRIM(RTRIM(PD10.DM_PRS_LST)) AS BorrName,
		'$' + CONVERT(VARCHAR(15), SUM(ISNULL(LN80.LN_LTE_FEE, 0.00) + ISNULL(LN80.LA_BIL_PAS_DU,0.00)+ ISNULL(LN80.LA_BIL_CUR_DU,0.00)), 1) AS TotalAmt
	FROM
		LN10_LON LN10
	...
	WHERE 
		LN10.BF_SSN = @BF_SSN
	GROUP BY
		LTRIM(RTRIM(PD10.DM_PRS_1)) + ' ' + LTRIM(RTRIM(PD10.DM_PRS_LST))
END