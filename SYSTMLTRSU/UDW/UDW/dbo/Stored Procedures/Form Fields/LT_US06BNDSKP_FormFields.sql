CREATE PROCEDURE [dbo].[LT_US06BNDSKP_FormFields]
	@BF_SSN VARCHAR(9),
	@IsCoborrower BIT = 0,
    @RN_ATY_SEQ_PRC INT
AS
...
DECLARE @PF_REQ_ACT VARCHAR(5) = (SELECT AC11.PF_REQ_ACT FROM AC11_ACT_REQ_LTR AC11 WHERE PF_LTR = 'US06BNDSKP')

--Special endorser only letter
	SELECT DISTINCT
