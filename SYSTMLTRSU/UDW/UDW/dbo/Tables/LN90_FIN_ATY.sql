...
GO
CREATE NONCLUSTERED INDEX [ix_LN90_FIN_ATY_LC_STA_LON90_PC_FAT_TYP_PC_FAT_SUB_TYP_LD_FAT_EFF_includes]
    ON [dbo].[LN90_FIN_ATY]([LC_STA_LON90] ASC, [PC_FAT_TYP] ASC, [PC_FAT_SUB_TYP] ASC, [LD_FAT_EFF] ASC)
    INCLUDE([BF_SSN], [LN_SEQ], [LC_FAT_REV_REA], [LA_FAT_NSI], [LA_FAT_CUR_PRI]) WITH (FILLFACTOR = 95);
...
GO
CREATE CLUSTERED INDEX [CIX_BFSSN_LNSEQ_LNFATSEQ_LFLSTDTSLN90]
    ON [dbo].[LN90_FIN_ATY]([BF_SSN] ASC, [LN_SEQ] ASC, [LN_FAT_SEQ] ASC, [LF_LST_DTS_LN90] ASC) WITH (FILLFACTOR = 95, PAD_INDEX = ON);
