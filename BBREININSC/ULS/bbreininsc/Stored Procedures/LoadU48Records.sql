CREATE PROCEDURE [bbreininsc].[LoadU48Records]
AS
BEGIN
...
--Determine if any OTHER bills are 16 or more days delinquent AND ACTIVE (diff between due and sat dates).
INSERT INTO ULS.bbreininsc._LoanStrikeCount
SELECT DISTINCT
	LN80.BF_SSN,
	LN80.LN_SEQ,
	LN10.LD_LON_1_DSB,
	COUNT(*) OVER(PARTITION BY LN80.BF_SSN, LN80.LN_SEQ) AS CountStrikes,
	SUM(CASE WHEN DATEDIFF(DAY,LN80.LD_BIL_DU_LON,ISNULL(LN80.LD_BIL_STS_RIR_TOL,GETDATE())) > 31 THEN 1 ELSE 0 END) OVER(PARTITION BY LN80.BF_SSN, LN80.LN_SEQ) AS FatalStrike
FROM
	UDW..LN10_LON LN10
	INNER JOIN UDW..LN54_LON_BBS LN54 
		ON LN54.BF_SSN = LN10.BF_SSN 
	...
	(
		...
		ON First48.BF_SSN = LN10.BF_SSN
		AND First48.LN_SEQ = LN10.LN_SEQ
		AND First48.BillNumber <= 48 --first 48 bills are the only exclusionary ones
	INNER JOIN UDW..LN80_LON_BIL_CRF LN80
		ON LN80.BF_SSN = First48.BF_SSN
		AND LN80.LN_SEQ = First48.LN_SEQ
		AND LN80.LN_SEQ_BIL_WI_DTE = First48.LN_SEQ_BIL_WI_DTE
		AND LN80.LD_BIL_DU_LON = First48.LD_BIL_DU_LON
		AND LN80.LD_BIL_CRT = First48.LD_BIL_CRT
		AND LN80.LN_BIL_OCC_SEQ = First48.LN_BIL_OCC_SEQ
	INNER JOIN UDW..BL10_BR_BIL BL10
		ON BL10.BF_SSN = LN80.BF_SSN
		AND BL10.LN_SEQ_BIL_WI_DTE = LN80.LN_SEQ_BIL_WI_DTE
		AND BL10.LD_BIL_CRT = LN80.LD_BIL_CRT
...
WHERE
	...
	AND DATEDIFF(DAY,LN80.LD_BIL_DU_LON,ISNULL(LN80.LD_BIL_STS_RIR_TOL,GETDATE())) >= 16 --Overdue by at least 16 days
				...
				CASE WHEN InactiveBillStrikes.AppliedDate < InactiveBills.LD_BIL_DU_LON THEN 0 --Applied Forb prior to bill due date. Treat as non delq
					 ELSE 
						CASE WHEN InactiveBillStrikes.AppliedDate >= InactiveBills.LD_BIL_DU_LON THEN 
								CASE WHEN DATEDIFF(DAY,InactiveBills.LD_BIL_DU_LON, InactiveBillStrikes.AppliedDate) > 31 THEN 1 --Fatal strike applied date
									 ELSE 0
								END
							 ELSE 
								CASE WHEN DATEDIFF(DAY,InactiveBills.LD_BIL_DU_LON, ISNULL(InactiveBills.LD_BIL_STS_RIR_TOL,GETDATE())) > 31 THEN 1 --Fatal strike satisfy date
								     ELSE 0
								END
						END
...
WHERE
	...
	(
		...
		OR 
		( --Only use standard strike counter for loans disbursed prior to 2006
			LSC.LD_LON_1_DSB <= '2006-05-01'
			AND LSC.CountStrikes <= 3 --Has strikes but doesnt have enough to disqualify
			AND LSC.FatalStrike = 0 --Forces any borrower with a delq bill greater than 31 days to auto fail reinstatement
		)
		OR
		( --Newer loans only care about the fatal delinquency and are more leinient
			LSC.LD_LON_1_DSB > '2006-05-01'
			AND LSC.FatalStrike = 0 --Forces any borrower with a delq bill greater than 31 days to auto fail reinstatement
		)
	)
