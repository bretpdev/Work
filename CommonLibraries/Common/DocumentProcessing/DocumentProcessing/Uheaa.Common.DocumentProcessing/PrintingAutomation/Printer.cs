...
namespace Uheaa.Common.DocumentProcessing
{
    internal class Printer
    {
        ...
        public bool ChangePrinterSetting(PrinterInfo info)
        {

            if (((int)info.Duplex < -1) || ((int)info.Duplex > 3))
            {
            ...
            {
                dm = this.GetPrinterSettings(info.DefaultPrinter);
                dm.dmDefaultSource = (short)info.Source;
                dm.dmOrientation = (short)info.Orientation;
                dm.dmPaperSize = (short)info.Size;
                dm.dmDuplex = (short)info.Duplex;
