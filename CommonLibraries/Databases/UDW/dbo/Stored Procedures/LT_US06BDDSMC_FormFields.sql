...
CREATE PROCEDURE [dbo].[LT_US06BDDSMC_FormFields]
 ...
 BEGIN
	SELECT DISTINCT
		CONVERT(VARCHAR(10), MAX(LN80.LD_BIL_DU_LON) OVER (PARTITION BY LN80.BF_SSN), 101) [DueDate],
		SUM(LN80.LA_BIL_CUR_DU) OVER (PARTITION BY LN80.BF_SSN) [InstallAmount],
		COALESCE(BR30.BA_EFT_ADD_WDR, 0.00) AS [AdditionalAmount],
		(SUM(LN80.LA_BIL_CUR_DU) OVER (PARTITION BY LN80.BF_SSN) + (COALESCE(BR30.BA_EFT_ADD_WDR, 0.00))) [TotalAmount]
	FROM
