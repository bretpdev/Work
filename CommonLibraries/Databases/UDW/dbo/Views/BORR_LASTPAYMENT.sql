...
AS
SELECT     TOP (100) PERCENT a.DF_SPE_ACC_ID, a.LST_PMT_RCVD, CONVERT(DECIMAL(9, 2), a.TRAN_AMT) AS TRAN_AMT, COALESCE (C.LST_PMT_IVR, '') 
                      AS LST_PMT_IVR, CONVERT(DECIMAL(9, 2), COALESCE (ABS(SUM(D.TRAN_AMT)), 0)) AS LST_AMT_IVR
FROM         (SELECT     A.DF_SPE_ACC_ID, A.LST_PMT_RCVD, ABS(SUM(B.TRAN_AMT)) AS TRAN_AMT
                       FROM          (SELECT     DF_SPE_ACC_ID, CONVERT(VARCHAR, MAX(CAST(LD_FAT_EFF AS DATETIME)), 101) AS LST_PMT_RCVD
                                               ...
                                               GROUP BY DF_SPE_ACC_ID) AS A LEFT OUTER JOIN
                                              dbo.LN90_FinancialTran AS B ON A.DF_SPE_ACC_ID = B.DF_SPE_ACC_ID AND A.LST_PMT_RCVD = B.LD_FAT_EFF AND 
                                              B.PC_FAT_TYP = '10' AND B.LC_FAT_REV_REA = ''
                       GROUP BY A.DF_SPE_ACC_ID, A.LST_PMT_RCVD) AS a LEFT OUTER JOIN
                      ...
                      dbo.LN90_FinancialTran AS D ON C.DF_SPE_ACC_ID = D.DF_SPE_ACC_ID AND C.LST_PMT_IVR = CONVERT(varchar, 
                      CAST(D.LD_FAT_EFF AS datetime), 112) AND D.PC_FAT_TYP = '10' AND D.PC_FAT_SUB_TYP = '10' AND D.LC_FAT_REV_REA = ''
