...
BEGIN
	...
	begin
		...
		begin
			set @RirInt = 0
			select @RirInt = @RirInt + 1 from LN55_LON_BBS_TIR where LC_STA_LN55 = 'A' and LC_LON_BBT_STA = 'Q' and BF_SSN = @Ssn and LN_SEQ = @LoanSeq
		end
		else if @PgmCode = 'BT2'
		begin
			set @RirInt = 0
		...
		begin
			...
			select @RirInt = @RirInt + 1 from LN55_LON_BBS_TIR where LC_STA_LN55 = 'A' and LC_LON_BBT_STA = 'Q' and BF_SSN = @Ssn and LN_SEQ = @LoanSeq and LF_LON_BBS_TIR = '02'
		end
	end
END
