...
(
	SELECT DISTINCT
		...
		SUM(CASE WHEN DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) > BD.LD_PIF_RPT AND BD.LD_PIF_RPT IS NOT NULL THEN 1 ELSE 0 END) AS PIF_CT_B4_REP_MO,
		--MAX(ISNULL(PifPrevious.PIF_CT_B4_REP_MO,0)) AS PIF_CT_B4_REP_MO,
		SUM(CASE WHEN BD.LD_PIF_RPT BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) AND DATEADD(MONTH, ((YEAR(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)) - 1900) * 12) + MONTH(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)), -1) THEN 1 ELSE 0 END) AS PIF_CT_REP_MO,
		--MAX(ISNULL(PifCurrent.PIF_CT_REP_MO,0)) AS PIF_CT_REP_MO,
		SUM(CASE WHEN BD.LC_STA_LON10 = 'D' AND BD.LA_OTS_PRI_ELG  = 0 AND DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) > BD.LD_STA_LON10  AND BD.LD_STA_LON10 IS NOT NULL THEN 1 ELSE 0 END) AS DSTAT_CT_B4_REP_MO,
		--MAX(ISNULL(DcvPrevious.DSTAT_CT_B4_REP_MO,0)) AS DSTAT_CT_B4_REP_MO,
		SUM(CASE WHEN BD.LC_STA_LON10 = 'D' AND BD.LA_OTS_PRI_ELG  = 0 AND BD.LD_STA_LON10 BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) AND DATEADD(MONTH, ((YEAR(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)) - 1900) * 12) + MONTH(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)), -1) THEN 1 ELSE 0 END) AS DSTAT_CT_REP_MO,
	...
	FROM
		...
		LEFT JOIN [FsaInvMet].[Daily_LoanLevel] PerfLoan ON PerfLoan.BF_SSN = BD.BF_SSN
