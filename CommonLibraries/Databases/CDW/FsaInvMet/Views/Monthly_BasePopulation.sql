...
FROM 
	...
	( -- forbearance indicator
		...
		WHERE
			...
			AND LN60.LD_FOR_END >= DATEADD(D,-(DAY(GETDATE())),GETDATE()) /*Last Day Prev Month*/
			AND LN60.LD_FOR_APL <= DATEADD(D,-(DAY(GETDATE())),GETDATE()) /*Last Day Prev Month*/
			AND LN60.LD_FOR_BEG <= DATEADD(D,-(DAY(GETDATE())),GETDATE()) /*Last Day Prev Month*/
	) FORB 
		ON FORB.BF_SSN = LN10.BF_SSN
	LEFT JOIN CDW..LN80_LON_BIL_CRF LN80
		ON LN10.BF_SSN = LN80.BF_SSN
		AND LN10.LN_SEQ = LN80.LN_SEQ
		AND LN80.LD_BIL_DU_LON BETWEEN DATEADD(D,-(DAY(DATEADD(M,-1,GETDATE()-2))),DATEADD(M,-1,GETDATE()-1)) /*first day prev month*/ AND DATEADD(D,-(DAY(GETDATE())),GETDATE()) /*Last Day Prev Month*/
...
WHERE 
	...
	AND LN10.LD_LON_ACL_ADD <= DATEADD(D,-(DAY(GETDATE())),GETDATE()) /*last day prev month*/
