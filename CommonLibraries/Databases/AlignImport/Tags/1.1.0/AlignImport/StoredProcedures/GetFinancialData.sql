...
SELECT DISTINCT
		...
		CASE
			WHEN loan.ln_accr_govt_int_curr_qtd != '00000000000' THEN SUBSTRING(loan.ln_accr_govt_int_curr_qtd,1, 7)
			ELSE ''
		END AS [LA_SIN_OTS_PCV],
		'05/11/2014' AS [LD_SIN_ACR_THU_PCV],
		CASE
			WHEN loan.ln_loan_type = 'STAF' AND IN_DEF.br_ssn IS NULL THEN dbo.CONVERT_DATE(GRP.gr_conversion_date)
		...
		END AS [LC_ELG_SIN],
		CASE
			WHEN loan.ln_loan_type IN ('CONS' ,'CONU') AND dbo.CONVERT_DATE(loan.ln_1st_disb_date) BETWEEN '11/13/1997' AND '10/01/1998' THEN 'SV'
		...
		LEFT JOIN 
			...
			LEFT JOIN
				(
					...
					FROM
						[dbo].[ITGGSQLDF_Def_Forb]
					WHERE
						dbo.CONVERT_DEFFORB_DATE(f1_end_date) > GETDATE()
