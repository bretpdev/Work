...
AS
	...
	END AS LN_RPS_TRM, 
	CASE
		WHEN D.number_of_payments IS NULL THEN cast((round(((cast(ISNULL(l.ln_curr_principal, 00) as money) / 100) / bal.balance) * cast(ISNULL(g.gr_monthly_pmt_amt_curr,00) as money), 0) / 100) as decimal(10,2))
		ELSE cast((round(((cast(ISNULL(l.ln_curr_principal, 00) as money) / 100) / bal.balance) * cast(ISNULL(d.group_payment_amount,00) as money), 0) / 100) as decimal(10,2))
	END AS LA_RPS_ISL
    from
