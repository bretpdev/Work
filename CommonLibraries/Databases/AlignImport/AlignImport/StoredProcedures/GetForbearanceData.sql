...
AS
    SELECT DISTINCT
        ...
        Replace(Str(FORB.control_num + convert(int, isnull(EB.MAX_FORB_CONTROL_NUM, 0)), 3), ' ' , '0') AS LF_FOR_CTL_NUM, 
        'Y' AS LI_CAP_FOR_INT_REQ,
        CASE    
            WHEN [f1_type_of_defer_forb] IN ('FFH', 'FFI', 'FFO', 'FFV', 'FMH', 'FA1', 'FV0') THEN [f1_low_payment_amount]
            ELSE '' 
        ...
        '' AS LD_FOR_APL, 
        CASE    
            WHEN [f1_type_of_defer_forb] IN ('FH1','FO1', 'FV1', 'FW1') THEN [f1_low_payment_amount]
            ELSE '' 
    ...
    FROM
		...
		LEFT JOIN [dbo].[ExistingBorrowers] EB on EB.BF_SSN = FORB.br_ssn and EB.CompassLoanSeq = LNMAP.CommpassLoanSeq
    WHERE 
        forb.br_ssn = @SSN
