...
DECLARE @EmailCampaign VARCHAR(20) = 'DL160DFFED.html' --Letter Id For Email Campaigns Table
DECLARE @SasId VARCHAR(20) = '160DLEMFD'
DECLARE @DelinquencyDays INT = 160
...
WHERE
	...
	(
		(
			@DateName = 'Monday'
			AND LN16.LN_DLQ_MAX + 1 BETWEEN  @DelinquencyDays AND (@DelinquencyDays+2)
			AND LN16.LC_STA_LON16 = 1
		)
		OR
		(
			@DateName != 'Monday' --It is not necessary, but it is more clear
			AND LN16.LN_DLQ_MAX + 1 = @DelinquencyDays
			AND LN16.LC_STA_LON16 = 1
		)
	)
