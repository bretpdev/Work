...
namespace VERFORBUH
{
    public class VerbalForbearanceScript
    {
        ...
        public int Run()
        {
            ...
            if (!Ver.CheckStatusesBeforeClaim(!IsCollectionSuspense, IsCollectionSuspense ? "Collection Suspension forbearance" : "Verbal forbearance"))
                return 0;
            ...
            {
                if (Ver.LoanStatuses.Any() && Ver.LoanStatuses.All(o => o.WC_DW_LON_STA.IsIn("07", "08", "09", "10", "11", "12")))
                {
                    ArcData arc = new ArcData(DataAccessHelper.CurrentRegion)
                    {
                        AccountNumber = AccountNumber,
        ...
        private void AddTheForbearance(ForbearanceDetails forbDetails, string ssn, bool is120, bool hasCoBwr)
        {
            ...
            if (ActuallyAddForb(ssn, "05", forbDetails.StartDate.Value, ref endDate, forbDetails.ForbearanceReason, ddo, is120, hasCoBwr))
            {
        ...
        private bool ActuallyAddForb(string ssn, string type, DateTime start, ref DateTime end, string reason, DateTime? ddo, bool is120, bool hasCoBwr)
        {
            ...
            if (RI.CheckForText(1, 72, "TSX30"))
            {
                for (int row = 8; !RI.CheckForText(23, 2, "90007"); row++)
                {
                    ...
                    if (!CheckLoan(ssn, start, reason, true))
                        return false;
                }
            }
            else
        ...
        private bool CheckLoan(string ssn, DateTime start, string reason, bool multiple = false)
        {
