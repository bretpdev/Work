...
BEGIN TRY
	...
	WHERE
		...
		AND DATEADD(MONTH,-1,CONVERT(DATE,(LN60.LD_FOR_END))) <= @TODAY
	...
	FROM
		...
		(
			SELECT
				...
				AAP.ARC,
				AAP.ScriptId,
				CONVERT(DATE,AAP.CreatedAt) AS CreatedAt,
				ALSS.LoanSequence
			FROM
				ULS..ArcAddProcessing AAP
				INNER JOIN ULS..ArcLoanSequenceSelection ALSS
					ON AAP.ArcAddProcessingId = ALSS.ArcAddProcessingId
		 ) ExistingAAP
			ON ExistingAAP.AccountNumber = NewData.DF_SPE_ACC_ID
			AND ExistingAAP.LoanSequence = NewData.LN_SEQ
			AND ExistingAAP.ARC = @ARC
			AND ExistingAAP.ScriptId = @ScriptId
			AND ExistingAAP.CreatedAt = @TODAY
	WHERE
...
--select * from uls..ArcAddProcessing where ScriptId = 'CNCRCRTFD';--for testing only