...
GO
CREATE NONCLUSTERED INDEX [IX_InactivatedAt_EcorrDocumentCreatedAt_CreatedAt]
    ON [dbo].[LT20_LTR_REQ_PRC]([InactivatedAt] ASC, [EcorrDocumentCreatedAt] ASC, [CreatedAt] ASC)
    INCLUDE([RM_DSC_LTR_PRC]);
...
GO
CREATE NONCLUSTERED INDEX [IX_InactivatedAt_CreatedAt]
    ON [dbo].[LT20_LTR_REQ_PRC]([InactivatedAt] ASC, [CreatedAt] ASC)
    INCLUDE([RM_DSC_LTR_PRC]);


GO
CREATE NONCLUSTERED INDEX [IX_DFSPEACCID_RMAPLPGMPRC_RTRUNSRTDTSPRC_RNSEQLTRCTPRC]
    ON [dbo].[LT20_LTR_REQ_PRC]([DF_SPE_ACC_ID] ASC, [RM_APL_PGM_PRC] ASC, [RT_RUN_SRT_DTS_PRC] ASC, [RN_SEQ_LTR_CRT_PRC] ASC)
    INCLUDE([RN_SEQ_REC_PRC]);


GO
CREATE NONCLUSTERED INDEX [IX_RILTRDELPRC_InactivatedAt]
