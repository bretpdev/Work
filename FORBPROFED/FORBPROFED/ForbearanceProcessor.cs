...
namespace FORBPROFED
{
    class ForbearanceProcessor
    {
        ...
        public void WorkRecord(ForbProcessingRecord record)
        {
            ReflectionInterface ri = GetSingleRI();
        ...
        public ReflectionInterface GetSingleRI()
        {
            ...
            BatchProcessingHelper helper = BatchProcessingLoginHelper.Login(logRun, ri, Program.ScriptId, "BatchCornerstone");
            return ri;
        }
        ...
        public void HandleATS0H(ReflectionInterface ri, ForbProcessingRecord record)
        {
            ...
            if (!record.SelectAllLoans)
            {
                ...
                if (loans != null && loans.Count == 0)
                {
                    ...
                    ArcProcessor.AddArcError(record, DA, logRun, ErrorMessages.NoLoansSelected(record, ri), loans);
                    return;
                }
            }
            ...
            if(ri.CheckForText(23,2,"50108"))
            {
                ...
                ArcProcessor.AddArcError(record, DA, logRun, ErrorMessages.BorrowerHasNoLoans(record, ri), loans);
                return;
            }
            ...
            if(EnterForbRecord(ri, record, loans))
            {
                ...
                ArcProcessor.AddArcSuccess(record, DA, logRun, loans);
            }
        }
        ...
        public bool EnterForbRecord(ReflectionInterface ri, ForbProcessingRecord record, List<int> loans)
        {
            ...
            if(IfScreenThenError(ri, "TSX30", record, ErrorMessages.GetTSX30(record, ri), loans))
            {
                return false;
            }
            if(IfScreenThenError(ri, "TSX31", record, ErrorMessages.GetTSX31(record, ri), loans))
            {
                return false;
            }
            if(CheckScreenCodeError(ri, "TSX32", record, ErrorMessages.GetTSX32(record, ri), loans))
            ...
            if(!ri.CheckForText(23, 2, "01004"))
            {
                ...
                ArcProcessor.AddArcError(record, DA, logRun, ErrorMessages.Get01004(record, ri), loans);
                return false;
            }
            ...
            while (row <= endRow)
            {
                ...
                if(row == endRow)
                {
                    ...
                    if (ri.CheckForText(23, 2, "90007"))
                    {
                        ...
                        ArcProcessor.AddArcError(record, DA, logRun, ErrorMessages.BadForbType(record, ri), loans);
                        return null;
                    }
                    continue;
                }
        ...
        public bool CheckScreenCodeError(ReflectionInterface ri, string screenCode, ForbProcessingRecord record, string comment, List<int> loans)
        {
            if (!ri.CheckForText(1, 72, screenCode))
            {
                ...
                ArcProcessor.AddArcError(record, DA, logRun, comment, loans);
                return true;
            }
            return false;
        }
        ...
        /// </summary>
        /// <param name="screenCode">Code to validate against(5 characters)</param>
        public bool IfScreenThenError(ReflectionInterface ri, string screenCode, ForbProcessingRecord record, string comment, List<int> loans)
        {
            if (ri.CheckForText(1, 72, screenCode))
            {
                logRun.AddNotification("Unexpected Screen Encountered, Received: " + ri.GetText(1, 72, 5), NotificationType.ErrorReport, NotificationSeverityType.Critical);
                ArcProcessor.AddArcError(record, DA, logRun, comment, loans);
                return true;
            }
            return false;
