...
TRUNCATE TABLE FsaInvMet.Daily_LoanLevel;
INSERT INTO FsaInvMet.Daily_LoanLevel
SELECT
	LoanPerf.BF_SSN,
	LoanPerf.LN_SEQ,
...
FROM
	( --Loan level performance category with priority
		SELECT
			Perf.*,
			ROW_NUMBER() OVER (PARTITION BY Perf.BF_SSN ORDER BY Perf.PerfCategoryPriority, Perf.LN_SEQ) AS LoanStatusPriority
		FROM
		( --Loan level performance category
			...
			FROM
				FsaInvMet.Daily_BasePopulation BP
				INNER JOIN FsaInvMet.Daily_PerformanceCategory PF
