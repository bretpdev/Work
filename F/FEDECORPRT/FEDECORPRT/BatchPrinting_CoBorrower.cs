...
namespace FEDECORPRT
{
    ...
    {
        private List<CoBorrowerInformation> GetCoBorrowers(string borrowerSSN)
        {
            return DA.GetCoBorrowersForBorrower(borrowerSSN);
        }
        ...
        private void InsertCoBorrowerInformation(List<CoBorrowerInformation> cbi, ScriptData file, PrintProcessingData data, Dictionary<string, string> internalNames)
        {
            foreach(CoBorrowerInformation cb in cbi)
            {
                ...
                DA.InsertPrintProcessingRecordCoBwr(cb.AccountNumber, file.ScriptID, letterData,data.CostCenter, file.Letter, data.BF_SSN);
            }
        }
        ...
        private void LoadAdditionalCoBorrowerFileData(ScriptData file)
        {
            ...
            //file.GetArcData(DA); SHOULD ALREADY BE LOADED FROM LoadAdditionalFileData
            file.LetterDataForCoBorrowers = DA.PopulatePrintProcessingCoBorrowerData(file.ScriptDataId);
        }

        private void DoEcorrCoBorrower(ScriptData file)
        {
            ...
            Parallel.ForEach(file.LetterDataForCoBorrowers, new ParallelOptions { MaxDegreeOfParallelism = int.MaxValue }, data =>
            {
                if (!data.EcorrDocumentCreatedAt.HasValue)
                    DoEcorr(file, data, ecorrTemplatePath, true);
            });
        }
        ...
        public Dictionary<string,string> GetInternalHeaderNamesHelper()
        {
            ...
            foreach (DataRow r in nameMap.Rows)
            {
                ...
                {
                    PL.AddNotification("Error when setting up internal name mapping from [print].HeaderReplacementCoBorrower. Missing column.", NotificationType.ErrorReport, NotificationSeverityType.Critical);
        ...
        public string DoLetterDataReplace(string letterData, CoBorrowerInformation cb, ScriptData file, Dictionary<string, string> internalNames)
        {
            ...
            {
                PL.AddNotification("The column count of the header and data lines did not match. File Header: " + file.FileHeader + "Data: " + letterData, NotificationType.FileFormatProblem, NotificationSeverityType.Critical);
                ...
                if(internalNames.ContainsKey(header[i]))
                {
                    ...
                    switch (internalNames[header[i]])
                    {
                        ...
                        case "FOREIGNSTATE":
                            ...
                            break;
                        default:
                            PL.AddNotification("Internal mapping from [print].HeaderReplacementCoBorrower contained key without an enumerated value.", NotificationType.ErrorReport, NotificationSeverityType.Critical);
