...
%CREATE_FILES(4);
/*==============================================================*/
/*UTLWO34 BORROWERS WITH MORE THAN ONE 1029 PLUS INTEREST CREDIT*/
/*==============================================================*/
/*LIBNAME DLGSUTWH DB2 DATABASE=DLGSUTWH OWNER=OLWHRM1;*/
/*%LET RPTLIB = %SYSGET(reportdir);*/
%LET RPTLIB = T:\SAS;
FILENAME REPORT2 "&RPTLIB/ULWO34.LWO34R2";
FILENAME REPORTZ "&RPTLIB/ULWO34.LWO34RZ";
...
SELECT DISTINCT A.BF_SSN
	,A.LN_SEQ
	,ABS(D.LA_FAT_CUR_PRI) AS LA_FAT_CUR_PRI
	,D.LN_FAT_SEQ
	,D.LD_FAT_EFF
	,B.MXDT
	,PICBB.PF_REQ_ACT 			AS PICBB_ARC
	,PICBB.LD_REQ_RSP_ATY_PRF	AS PICBB_DT
	,C.DM_PRS_LST
	,A.IC_LON_PGM
	,C.DF_SPE_ACC_ID
...
INNER JOIN (
	SELECT BF_SSN
		,LN_SEQ
		,COUNT(*) 
		,MAX(LD_FAT_EFF) AS MXDT
	FROM OLWHRM1.LN90_FIN_ATY 
	WHERE PC_FAT_TYP  = '10'
	AND PC_FAT_SUB_TYP = '29'
	AND LC_FAT_REV_REA = ''
	AND LC_STA_LON90 = 'A'
	GROUP BY BF_SSN
		,LN_SEQ
...
LEFT OUTER JOIN (
	...
	WHERE AY10.PF_REQ_ACT = 'PICBB'
	AND AY10.LC_STA_ACTY10 = 'A' 
	GROUP BY LN85.BF_SSN, 
		LN85.LN_SEQ,
		AY10.PF_REQ_ACT
	 ) PICBB
	ON A.BF_SSN = PICBB.BF_SSN
	AND A.LN_SEQ = PICBB.LN_SEQ
WHERE D.PC_FAT_TYP  = '10'
AND D.PC_FAT_SUB_TYP = '29'
AND D.LC_FAT_REV_REA = ''
AND D.LC_STA_LON90 = 'A'
...
SELECT *
FROM CONNECTION TO DB2 (
SELECT DISTINCT CNSL.BF_SSN
	,CNSL.LN_SEQ 
/*	,CNSL_LN90.LN_FAT_SEQ	*/
/*	,CNSL_LN90.LD_FAT_EFF */
/*	,U1029.LN_ATY_SEQ*/
/*	,U1029.LD_REQ_RSP_ATY_PRF*/
/*	,B.TRX_COUNT*/
/*	,C.ARC_COUNT*/

FROM OLWHRM1.LN10_LON CNSL
INNER JOIN OLWHRM1.LN10_LON PLUS
	ON CNSL.BF_SSN = PLUS.BF_SSN
INNER JOIN OLWHRM1.LN90_FIN_ATY CNSL_LN90
	ON CNSL.BF_SSN = CNSL_LN90.BF_SSN
	AND CNSL.LN_SEQ = CNSL_LN90.LN_SEQ
INNER JOIN OLWHRM1.LN85_LON_ATY LN85
	ON PLUS.BF_SSN = LN85.BF_SSN
	AND PLUS.LN_SEQ = LN85.LN_SEQ
INNER JOIN OLWHRM1.AY10_BR_LON_ATY U1029
	ON U1029.BF_SSN = LN85.BF_SSN
	AND U1029.LN_ATY_SEQ = LN85.LN_ATY_SEQ
	AND CNSL_LN90.LD_FAT_EFF = U1029.LD_REQ_RSP_ATY_PRF
INNER JOIN (
	SELECT BF_SSN
		,LN_SEQ
		,COUNT(*) AS TRX_COUNT
	...
	ON CNSL.BF_SSN = B.BF_SSN
	AND CNSL.LN_SEQ = B.LN_SEQ
INNER JOIN (
	SELECT X.BF_SSN 
		,COUNT(*) AS ARC_COUNT
	FROM OLWHRM1.LN85_LON_ATY X
	...
	WHERE Y.PF_REQ_ACT = 'U1029'
	AND Y.LC_STA_ACTY10 = 'A'
	AND Z.IC_LON_PGM = 'PLUS'
	AND Z.LC_STA_LON10 = 'R'
	GROUP BY X.BF_SSN 
	 ) C
	ON PLUS.BF_SSN = C.BF_SSN

WHERE CNSL.IC_LON_PGM IN ('SUBCNS','SUBSPC','UNCNS','UNSPC')
AND PLUS.IC_LON_PGM = 'PLUS'
AND U1029.PF_REQ_ACT = 'U1029'
AND U1029.LC_STA_ACTY10 = 'A'
AND CNSL_LN90.PC_FAT_TYP  = '10'
AND CNSL_LN90.PC_FAT_SUB_TYP = '29'
AND CNSL_LN90.LC_FAT_REV_REA = ''
AND CNSL_LN90.LC_STA_LON90 = 'A'
AND CNSL.LC_STA_LON10 = 'R'
AND B.TRX_COUNT <= C.ARC_COUNT
AND NOT EXISTS (
	SELECT *
	...
	INNER JOIN OLWHRM1.LN85_LON_ATY Y
		ON X.BF_SSN = Y.BF_SSN
		AND X.LN_ATY_SEQ = Y.LN_ATY_SEQ
	WHERE Y.BF_SSN = CNSL.BF_SSN
	AND Y.LN_SEQ = CNSL.LN_SEQ
	AND X.PF_REQ_ACT = 'PICBB'
...
DISCONNECT FROM DB2;
/*%PUT  SQLXRC= >>> &SQLXRC <<< ||| SQLXMSG= >>> &SQLXMSG >>>  ;  * INCLUDES ERROR MESSAGES TO SAS LOG  ;*/
/*%SQLCHECK (SQLRPT=ULWO34.LWO34RZ);*/
/*QUIT;*/
ENDRSUBMIT;

DATA DFS1;SET WORKLOCL.DFS1;RUN;
DATA DFS2;SET WORKLOCL.DFS2;RUN;

/*FINISH PROCESSING*/
PROC SQL;
CREATE TABLE BWMTOPICX AS 
SELECT DISTINCT A.BF_SSN
	,A.DM_PRS_LST
	,A.LN_SEQ
	,A.LA_FAT_CUR_PRI
	...
	,A.DF_SPE_ACC_ID
FROM DFS1 A
WHERE A.MXDT > A.PICBB_DT
AND A.LD_FAT_EFF = A.MXDT
AND NOT EXISTS (
	SELECT *
	FROM DFS2 X
...
PROC SORT DATA=BWMTOPICX NODUPKEY;
BY DF_SPE_ACC_ID DM_PRS_LST LN_SEQ LA_FAT_CUR_PRI LD_FAT_EFF LN_FAT_SEQ;
RUN;
...
DATA _NULL_;
	...
	USER = '';
	ACT_DT = LD_FAT_EFF;
	DESCRIPTION = CATX(',',	DF_SPE_ACC_ID,DM_PRS_LST,LN_SEQ,LA_FAT_CUR_PRI,PUT(LD_FAT_EFF,MMDDYY10.));
