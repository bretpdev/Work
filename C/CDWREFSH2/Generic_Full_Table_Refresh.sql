...
--### CONFIGURATION
DECLARE @LocalTableName VARCHAR(MAX) = 'dbo.AY20_ATY_TXT'
DECLARE @RemoteTableName VARCHAR(MAX) = 'PKUB.AY20_ATY_TXT'
DECLARE @DateColumn VARCHAR(MAX) = 'LF_LST_DTS_AY20'
DECLARE @DaysPerCycle VARCHAR(MAX) = '10'
...
DECLARE @SetRemoteMinDateQuery NVARCHAR(MAX) = 'SELECT @RemoteMinDate = MINDATE FROM OPENQUERY(LEGEND,''' + @GetRemoteMinDateQuery + ''')'
