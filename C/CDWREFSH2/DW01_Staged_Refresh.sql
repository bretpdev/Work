...
TRUNCATE TABLE [DW01_DW_CLC_CLU_STAGE] 


DECLARE @SQLStatement VARCHAR(MAX) = 
'
	INSERT INTO  
		[dbo].[DW01_DW_CLC_CLU_STAGE] 
	SELECT
		*
	FROM
		OPENQUERY
...
FROM
	...
	(
		SELECT
			COUNT(*) [LocalCount]
		FROM
			CDW..DW01_DW_CLC_CLU_STAGE

	) L ON 1 = 1

IF @CountDifference != 0 AND @LoopCount > 0
	BEGIN
		RAISERROR('DW01_DW_CLC_CLU_STAGE - The remote and local record counts do not match.  The local count is off by %i records.  A full refresh of the table is required.', 16, 11, @CountDifference)
	END
ELSE IF @CountDifference != 0 AND @LoopCount = 0
	BEGIN
		...
		GOTO RefreshStart;
	END
ELSE IF @CountDifference = 0 	--###### MERGE LOCAL WITH STAGING TABLE
	BEGIN
		MERGE 
