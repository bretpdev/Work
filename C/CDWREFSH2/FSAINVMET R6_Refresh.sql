--based on FSAINVMET R6.rdl SSRS data set
BEGIN TRY
	BEGIN TRANSACTION

		INSERT INTO CDW.FsaInvMet.R6
			(
				 ReportDate
				,Cat01Count
				,Cat02Count
		...
		SELECT
			...
			MAX(CASE WHEN CategoryCount.PerformanceCategory = '13' THEN CategoryCount.BorrowerCount ELSE NULL END) AS Cat13Count, --'National Emergency Forb'
...
END TRY
BEGIN CATCH
	PRINT 'FSAINVMET R6_Refresh.sql transaction NOT committed.';
