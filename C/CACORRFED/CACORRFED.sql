--CACORRFED.sql: CA Correspondence Response - FED
BEGIN TRY
	BEGIN TRANSACTION
		...
		INTO
			#lettersToAdd_BOR
		FROM
			CLS.docid.DocumentResponseStateMapping DRSM
			INNER JOIN CLS.docid.DocIdMapping DID
			...
			INNER JOIN CDW..AY10_BR_LON_ATY AY10
				...
				AND CAST(AY10.LD_ATY_REQ_RCV AS DATE) >= CAST(PD30.DF_LST_DTS_PD30 AS DATE)
				AND CAST(AY10.LD_ATY_REQ_RCV AS DATE) > @OneWeekLookback --only get records within past week
				AND AY10.LC_STA_ACTY10 = 'A' --active
			LEFT JOIN CLS.docid.DocumentsProcessedResponse DPR
			...
			LEFT JOIN CLS.[print].ArcScriptDataMapping ASDM
		...
		;
		--select * from #lettersToAdd_BOR --TEST
		...
		FROM
			...
			INNER JOIN CLS.[print].ScriptData SD
				...
				AND SD.LetterId = @LetterId
				AND SD.Active = 1 --active
			INNER JOIN CDW..PD30_PRS_ADR EndPD30
			...
			INNER JOIN CDW..PD10_PRS_NME BorrPD10
			...
			INNER JOIN CDW..AY10_BR_LON_ATY AY10
				...
				AND CAST(AY10.LD_ATY_REQ_RCV AS DATE) >= CAST(EndPD30.DF_LST_DTS_PD30 AS DATE)
				AND CAST(AY10.LD_ATY_REQ_RCV AS DATE) > @OneWeekLookback --only get records within past week
				AND AY10.LC_STA_ACTY10 = 'A' --active
		...
		FROM
			...
			LEFT JOIN CLS.[print].PrintProcessing EXISTING_DATA --active flag not needed since want to identify all matching records
		...
		INSERT INTO CLS.docid.DocumentsProcessedResponse
		(
			BF_SSN, 
			LN_ATY_SEQ, 
			PrintProcessingId
		...
		;
		--select * from CLS.docid.DocumentsProcessedResponse --TEST
		...
		FROM
			#lettersToAdd LTA
			LEFT JOIN CLS..ArcAddProcessing EXISTING_AAP
			...
			LEFT JOIN CLS.docid.DocumentsProcessedResponse EXISTING_DPR
		...
		;
		--select * from CLS.docid.DocumentsProcessedResponse where InvalidAddressArcAddProcessingId is not null --TEST		
