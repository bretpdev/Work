...
DECLARE @MinSaleNameDLO VARCHAR(20) = (SELECT LoanSale FROM CDW..CS_Transfer3_LoanSaleMapping WHERE LoanSaleId = @MinSaleDLO)
DECLARE @MinSaleLNC INT = 100
DECLARE @MinSaleNameLNC VARCHAR(20) = (SELECT LoanSale FROM CDW..CS_Transfer3_LoanSaleMapping WHERE LoanSaleId = @MinSaleLNC)
