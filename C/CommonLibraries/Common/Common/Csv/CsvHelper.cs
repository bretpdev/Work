...
namespace Uheaa.Common
{
    ...
    public static class CsvHelper
    {
        ...
        public static List<string> GetHeaderAndLineFromObject(object obj, string delimiter)
        {
            return new List<string>()
            {
                string.Join(delimiter, obj.GetType().GetProperties().OrderBy(r => r.GetAttributeValue<Order, int?>(o => o.PropertyOrder, null)).
                    Where(p => !p.HasAttribute<Ignore>()).Select(q => q.Name).ToList()),
                GetLineFromObject(obj, delimiter)
            };
        }

        public static string GetLineFromObject(object obj, string delimiter)
        {
            return string.Join(delimiter, obj.GetType().GetProperties().OrderBy(r => r.GetAttributeValue<Order, int?>(o => o.PropertyOrder, null))
                .Where(p => !p.HasAttribute<Ignore>()).Select(q => q.GetValue(obj) ?? "").ToList());
        }
        ...
        public static CsvParseResults<T> ParseTo<T>(string headerLine, string[] lines) where T : new()
        {
            ...
            if (!results.HeaderValidationResults.MissingFields.Any())
            {
                ...
                for (int headerIndex = 0; headerIndex < headers.Length; headerIndex++)
                {
                    ...
                    {
                        var name = prop.Name;
                        var attr = prop.GetCustomAttribute<CsvHeaderNameAttribute>();
                        if (attr != null)
                            name = attr.HeaderName;
                        if (PropertyHeaderMatch(name, header))
                        {
                ...
                for (int lineNumber = 0; lineNumber < lines.Length; lineNumber++)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (!setterIndices.ContainsKey(i))
                                continue;
                            var setter = setterIndices[i];
                            var fieldType = setter.GetParameters().First().ParameterType;
                            var nullableUnderlyingType = Nullable.GetUnderlyingType(fieldType);
                            if (nullableUnderlyingType == null)
                                setter.Invoke(obj, new object[] { Convert.ChangeType(values[i], fieldType) });
                            else
                            {
                                if (string.IsNullOrWhiteSpace(values[i]))
                                    setter.Invoke(obj, new object[] { null });
                                else
                                    setter.Invoke(obj, new object[] { Convert.ChangeType(values[i], nullableUnderlyingType) });
                            }
                        }
