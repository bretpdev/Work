...
SELECT DISTINCT
		...
		END AS [IF_GTR],
		CASE
			WHEN loan.ln_loan_type = 'PLUS' THEN loan.[br_benefiting_student_ssn] ELSE loan.br_ssn
		END AS [LF_STU_SSN],
		'' AS [LA_CUR_ILG],
		...
		CASE
			WHEN loan.si_school_code + loan.si_branch IN ('00000000','') AND LOAN.ln_loan_type IN ('CONS','CONU') THEN ' 88888800'
			ELSE loan.si_school_code + loan.si_branch
		END AS [LF_DOE_SCL_ORG],
		...
		CASE
			WHEN loan.ln_loan_type in ('STAF', 'CONS') THEN SUBSTRING(loan.ln_accr_govt_int_curr_qtd,1, 7)
			ELSE ''
		END AS [LA_SIN_OTS_PCV],
		CASE
			WHEN loan.ln_loan_type = 'STAF' AND IN_DEF.br_ssn IS NULL THEN COALESCE(dbo.CONVERT_DATE(GRP.gr_conversion_date), NI.[previous_deferment_end_date])
			WHEN loan.ln_loan_type IN ('STAF', 'CONS') AND IN_DEF.br_ssn IS NOT NULL THEN IN_DEF.END_DATE
			ELSE NULL --'05/11/2014'
		END AS [LD_SIN_ACR_THU_PCV],
		CASE
			WHEN loan.ln_loan_type = 'STAF' AND IN_DEF.br_ssn IS NULL THEN COALESCE(dbo.CONVERT_DATE(GRP.gr_conversion_date), NI.[previous_deferment_end_date])
			WHEN loan.ln_loan_type IN ('STAF', 'CONS') AND IN_DEF.br_ssn IS NOT NULL THEN IN_DEF.END_DATE
			ELSE NULL
		END  AS [LD_SIN_LST_PD_PCV],
		'' AS [LC_STA_NEW_BR],
		'' AS [LC_SCY_PGA],
		dbo.CONVERT_DATE(loan.ln_1st_disb_date) AS [LD_LON_1_DSB],
		CASE
			WHEN loan.ln_grade_level = '00' THEN loan.ln_grade_level
			WHEN loan.ln_grade_level IN ('01','10') THEN '01'
			WHEN loan.ln_grade_level IN ('02','20') THEN '02'
			WHEN loan.ln_grade_level IN ('03','30') THEN '03'
			WHEN loan.ln_grade_level IN ('04','40') THEN '04'
			WHEN loan.ln_grade_level = '05' THEN loan.ln_grade_level
			WHEN loan.ln_grade_level = '06' THEN loan.ln_grade_level
			WHEN loan.ln_grade_level = '07' THEN loan.ln_grade_level
		END AS [LC_ACA_GDE_LEV], 
		...
		CASE	
			WHEN BB.br_ssn IS NOT NULL AND GRP_BB.B02 = 'Y' OR GRP_BB.B03 = 'Y' OR GRP_BB.B07 = 'Y' OR GRP_BB.C02 = 'Y' THEN 'Q'
			ELSE ''
		...
		CASE
			...
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.B02 = 'Y' AND GRP_BB.C02 != 'Y' AND GRP_BB.B07 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'WY2'
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.B07 = 'Y' AND GRP_BB.B02 != 'Y'  AND GRP_BB.C02 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'WY3'
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.C02 = 'Y' AND GRP_BB.B02 != 'Y'  AND GRP_BB.B07 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'WY3'
		...
		CASE
			WHEN loan.ln_loan_type IN ('CONS' ,'CONU') THEN 
				(SELECT SUM((CAST(ln.ln_orig_loaned_amt AS int)))  FROM [ITELSQLDF_Loan_NoZeroBalances] ln WHERE ln.br_ssn = bor.br_ssn)
		...
		CASE	
			WHEN BB.br_ssn IS NOT NULL AND GRP_BB.B02 = 'Y' OR GRP_BB.B03 = 'Y' OR GRP_BB.B07 = 'Y' OR GRP_BB.C02 = 'Y' THEN '25'
			ELSE ''
		...
		CASE
			WHEN loan.ln_loan_type = 'PLUS' THEN 'I'
			ELSE 'B'
		...
		END AS [LC_ELG_SIN],
		CASE
			WHEN loan.ln_loan_type IN ('CONS' ,'CONU') 
			     AND isnull(dbo.CONVERT_DATE(loan.ln_application_date), dbo.CONVERT_DATE(loan.ln_1st_disb_date)) 
				 BETWEEN '11/13/1997' AND '10/01/1998' THEN 'SV'
			...
			WHEN (loan.ln_loan_type IN ('STAF', 'STAU')
				...
				AND (dbo.CONVERT_DATE(loan.ln_conversion_date) > '05/11/2014')) THEN 'C1' 
			WHEN (loan.ln_loan_type IN ('STAF', 'STAU')
				AND (dbo.CONVERT_DATE(loan.ln_1st_disb_date) >= '07/01/95' AND dbo.CONVERT_DATE(loan.ln_1st_disb_date) < '07/01/06')
				AND (dbo.CONVERT_DATE(loan.ln_conversion_date) <=  '05/11/2014')
		...
		CASE
			WHEN GRP_BB.B02 = 'Y' OR GRP_BB.B03 = 'Y' OR GRP_BB.B07 = 'Y' OR GRP_BB.C02 = 'Y' THEN 'Y'
			ELSE 'N'
		END AS [LON84_DAT_ENT_IND],
		CASE
			WHEN GRP_BB.B02 = 'Y' AND GRP_BB.B03 != 'Y' AND GRP_BB.B07 != 'Y' AND GRP_BB.C02 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'O24'
			WHEN GRP_BB.B07 = 'Y' AND GRP_BB.B02 != 'Y' AND GRP_BB.B03 != 'Y' AND GRP_BB.C02 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'W24'
			WHEN GRP_BB.C02 = 'Y' AND GRP_BB.B02 != 'Y' AND GRP_BB.B03 != 'Y' AND GRP_BB.B07 != 'Y' AND GRP_BB.B06 != 'Y' THEN 'W24'
		...
		CASE
			...
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.B02 = 'Y' AND GRP_BB.C02 != 'Y' AND GRP_BB.B07 != 'Y' AND GRP_BB.B06 != 'Y' THEN '02000'
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.B07 = 'Y' AND GRP_BB.B02 != 'Y'  AND GRP_BB.C02 != 'Y' AND GRP_BB.B06 != 'Y' THEN '03000'
			WHEN GRP_BB.B03 = 'Y' AND GRP_BB.C02 = 'Y' AND GRP_BB.B02 != 'Y'  AND GRP_BB.B07 != 'Y' AND GRP_BB.B06 != 'Y' THEN '03000'
		END AS [LR_RDC],
	...
	FROM
		...
		(
			...
			FROM [dbo].[ITELSQLDF_Loan_NoZeroBalances]
		) LOAN_BAL
		...
		(
			...
			FROM
				[dbo].[ITELSQLDF_Loan_NoZeroBalances]
			GROUP BY 
				br_ssn
		) BAL
			...
			LEFT JOIN
				(
					SELECT DISTINCT
						br_ssn,
						f1_loan_number,
						dbo.CONVERT_DEFFORB_DATE(f1_end_date) AS END_DATE
					FROM
						[dbo].[ITGGSQLDF_Def_Forb] DF
					WHERE
						GETDATE() BETWEEN dbo.CONVERT_DEFFORB_DATE(DF.f1_start_date) and dbo.CONVERT_DEFFORB_DATE(DF.f1_end_date)
						AND f1_canceled_flag = ''
				) IN_DEF
					ON IN_DEF.br_ssn = loan.br_ssn
					AND IN_DEF.f1_loan_number = loan.ln_num
			LEFT JOIN 
			(
				SELECT
					L.br_ssn,
					L.ln_num,
					CASE WHEN L.ln_loan_type in ('STAF') THEN MAX(dbo.CONVERT_DEFFORB_DATE(DF.f1_end_date)) ELSE NULL END [previous_deferment_end_date]
				FROM
					[dbo].[ITELSQLDF_Loan_NoZeroBalances] L
					INNER JOIN [dbo].[ITGGSQLDF_Def_Forb] DF on DF.br_ssn = L.br_ssn AND DF.f1_loan_number = L.ln_num
				WHERE
					DF.f1_canceled_flag != 'Y'
					AND
					GETDATE() > dbo.CONVERT_DEFFORB_DATE(DF.f1_end_date)
				GROUP BY
					L.br_ssn,
					L.ln_num,
					L.ln_loan_type
			) NI on NI.br_ssn = loan.br_ssn and NI.ln_num = loan.ln_num
	WHERE
		loan.br_ssn = @SSN
RETURN 0