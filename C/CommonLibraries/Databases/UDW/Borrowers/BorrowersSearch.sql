...
select b.BF_SSN as SSN
 ...
 where (@FirstName is null or b.DM_PRS_1 like @FirstName) and
       ...
       (@Phone is null or p.DN_DOM_PHN_ARA + p.DN_DOM_PHN_XCH + p.DN_DOM_PHN_LCL like @Phone) and
...
select b.BF_SSN as SSN, 
	   ...
	   MAX(case when p.DC_PHN = 'H' then p.DN_DOM_PHN_ARA + p.DN_DOM_PHN_XCH + p.DN_DOM_PHN_LCL end) as HomePhone,
	   MAX(case when p.DC_PHN = 'H' then p.CONSENT_IND end) as HomePhoneConsent,
	   MAX(case when p.DC_PHN = 'W' then p.DN_DOM_PHN_ARA + p.DN_DOM_PHN_XCH + p.DN_DOM_PHN_LCL end) as WorkPhone,
	   MAX(case when p.DC_PHN = 'W' then p.CONSENT_IND end) as WorkPhoneConsent,
	   MAX(case when p.DC_PHN = 'A' then p.DN_DOM_PHN_ARA + p.DN_DOM_PHN_XCH + p.DN_DOM_PHN_LCL end) as AlternatePhone,
