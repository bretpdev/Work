CREATE PROCEDURE [dbo].[LT_TS06BPYE45_IDR]
...
SELECT DISTINCT
	CONVERT(VARCHAR, DATEADD(DAY, -35, RS05.BD_ANV_QLF_IBR), 101) AS SoftDeadline,
	RS05.BA_PMN_STD_TOT_PAY AS PermStandardAmount,
	CASE WHEN DAY(RS05.BD_ANV_QLF_IBR) <= LN65.DUE_DAY 
			THEN CONVERT(VARCHAR,DATEADD(DAY, LN65.DUE_DAY - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR), 101)--set the day to DUE_DAY
		 WHEN DAY(RS05.BD_ANV_QLF_IBR) > LN65.DUE_DAY 
			THEN CONVERT(VARCHAR,DATEADD(MONTH, 1, DATEADD(DAY, LN65.DUE_DAY - DAY(RS05.BD_ANV_QLF_IBR), RS05.BD_ANV_QLF_IBR)),101)--add a month and set the day to DUE_DAY
