...
BEGIN
	...
	declare @ContactEmail as table(DF_SPE_ACC_ID char(10), EcorrCorrespondence bit, EcorrBilling bit, EcorrTax bit)
	IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'PH05_ContactEmail'))
	BEGIN
	...
	BEGIN
		declare @total int = (select count(*) from dbo.DW01_Loan where DF_SPE_ACC_ID = @AccountNumber);
		declare @deconverted int = (select count(*) from dbo.DW01_Loan where DF_SPE_ACC_ID = @AccountNumber and DW_LON_STA = 'DECONVERTED')
		set @NeedsDeconArc = cast(case when @total = @deconverted and @total > 0
	...
	SELECT
		...
		--E-CORR Indicator
		ISNULL(ECORR.EcorrCorrespondence, 0) as EcorrCorrespondence,
		ISNULL(ECORR.EcorrBilling, 0) as EcorrBilling,
		ISNULL(ECORR.EcorrTax, 0) as EcorrTax,
	...
	FROM
		...
		LEFT JOIN @ContactEmail ECORR ON ECORR.DF_SPE_ACC_ID = BORR.DF_SPE_ACC_ID
	WHERE BORR.DF_SPE_ACC_ID = @AccountNumber
END
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spMD_GetDemographicData] TO [Imaging Users]
    AS [dbo];
