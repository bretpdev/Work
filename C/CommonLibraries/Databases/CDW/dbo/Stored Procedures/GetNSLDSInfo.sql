...
AS
		...
		SELECT 
			CAST(@SpouseIndicator as char(1)) AS SpouseIndicator,
			...
			(COALESCE(GRSP.LA_CUR_PRI,0) + COALESCE(GRSP.WA_TOT_BRI_OTS,0)) AS OutstandingBalance,
		...
		WHERE 
			...
			AND IC_LON_PGM NOT IN ('PU','FI','NU', 'DU', 'EU', 'IC')
			AND LA_CUR_PRI > 0
RETURN 0