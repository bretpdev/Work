...
AS
	SELECT
		CASE
			WHEN loan.ln_loan_type = 'PLUS' OR loan.ln_loan_type = 'PLUSGB' THEN 'Y' ELSE 'N'
		...
		'' AS [DF_DRV_LIC],
		CASE
			WHEN loan.ln_loan_type = 'PLUS' OR loan.ln_loan_type = 'PLUSGB'
			THEN loan.[br_benefiting _student_ssn] ELSE loan.br_ssn
		END AS [DF_PRS_ID],
		'' AS [DI_US_CTZ],
		BWR.br_name_first AS [DM_PRS_1], 
		BWR.br_name_last AS [DM_PRS_LST], 
		'' AS [DM_PRS_LST_SFX],
		...
		'' AS [DC_DL_COR_TYP],
		CASE
			WHEN loan.ln_loan_type = 'PLUS' OR loan.ln_loan_type = 'PLUSGB' THEN LOAN.[br_benefiting _student_ssn] ELSE LOAN.br_ssn
		END AS [LF_STU_SSN],
		CASE
			WHEN loan.ln_loan_type = 'PLUS' OR loan.ln_loan_type = 'PLUSGB' THEN 'Y' ELSE 'N'
		...
		'1' AS [LN_STU_SPR_SEQ],
		'' AS [LD_SCL_CER_STU_STA], --TODO
		'N' AS [STU20_DAT_ENT_IND],
		'' AS [LC_STA_STU_ENR],
		'' AS [LD_ENR_BEG], 
		'' AS [LD_ENR_END], 
		'1' AS [LN_STU_ENR_SEQ],
