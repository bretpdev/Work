...
AS
    SELECT 
        ...
        '' AS LC_ENR_STA,
        DBO.CONVERT_DATE([f1_start_date]) AS LD_DFR_REQ_BEG,
        DBO.CONVERT_DATE([f1_end_date]) AS LD_DFR_REQ_END,
        '' AS LF_DFR_CTL_NUM, 
        CASE
            WHEN DEF.[f1_type_of_defer_forb] IN ('D10','D11','D12','D13','D15','D40') THEN [si_school_code] + [si_branch]
            ELSE '' 
        ...
        '000' AS LC_DFR_RSP,
        '' AS LD_DFR_APL, 
        DBO.CONVERT_DATE([f1_start_date]) AS LD_DFR_BEG,
        DBO.CONVERT_DATE([f1_end_date]) AS LD_DFR_END,
        '' AS LD_DFR_GRC_END, 
