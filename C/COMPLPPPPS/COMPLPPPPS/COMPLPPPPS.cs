...
{
    public class LppNelnetConversionPaymentPosting : ScriptBase
    {
        public LppNelnetConversionPaymentPosting(ReflectionInterface ri)
            : base(ri, "COMPLPPPPS")
        {
        ...
        {
            string[] fileToProcess = Directory.GetFiles(EnterpriseFileSystem.TempFolder, "Nelnet Payments*");

            if (fileToProcess.Count() > 1)
            {
                MessageBox.Show("Multiple Nelnet Payments.csv files exist.  Please resolve and try again.");
                EndDllScript();
            }
            else if (fileToProcess.Count() < 1)
            {
                MessageBox.Show("Nelnet Payments.csv file was not found.  Please resolve and try again.");
                EndDllScript();
            }
        ...
        private List<FileData> ReadCsvFile(string fileToProcess)
        {
            using (Processing pro = new Processing(fileToProcess))
            {
                ...
                if (pro.Data.Count < 1)
                {
                    MessageBox.Show("The Nelnet Payments.csv file on your T drive was empty.  Please investigate and try again.");
                    EndDllScript();
                }
