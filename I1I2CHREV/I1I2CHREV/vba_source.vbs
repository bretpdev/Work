...
Sub I1I2ClrhsRev()
    With Session
        'notify the user of the purpose of the script
        warn = MsgBox("This script works tasks in the I1 and I2 COMPASS queues.  Before running the script you should be logged in to the Clearinghouse website and it is suggested that you arrange your Reflection session and web browser windows so both are visible.  Click OK to continue or Cancel to end the script.", vbOKCancel, "I1/I2 Clearinghouse Review")
        If warn <> 1 Then End
        DocPath = "X:\PADD\Skip\"
        ...
        Do
            ...
            Do
                ...
                .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
                'if there are no tasks for I2, go to I1
                If .GetDisplayText(1, 72, 5) = "TXX6Y" And Queue = "I2" Then
                    Queue = "I1"
                'if there are no tasks for I1, prompt the user and end
                ElseIf .GetDisplayText(1, 72, 5) = "TXX6Y" And Queue = "I1" Then
                    MsgBox "There are no tasks left to work in either the I1 or I2 queues.", , "Processing Complete"
                    End
                Else
                    Exit Do
                End If
            ...
            Okay = 0
            Load I1I2Clrhs
            'populate dialog box controls with values from TS24
            SSN = .GetDisplayText(4, 16, 3) & .GetDisplayText(4, 20, 2) & .GetDisplayText(4, 23, 4)
            I1I2Clrhs.SSN = SSN
            I1I2Clrhs.BorrowerName = Trim(.GetDisplayText(4, 37, 40))
            I1I2Clrhs.Address1 = Trim(.GetDisplayText(8, 13, 30))
            I1I2Clrhs.Address2 = Trim(.GetDisplayText(9, 13, 30))
            I1I2Clrhs.City = Trim(.GetDisplayText(11, 13, 20))
            'if the country is not blank, get the foreign state (if it is blank, leave the state blank) and foreign country
            If .GetDisplayText(9, 55, 1) <> " " Then
                If .GetDisplayText(8, 61, 1) <> " " Then I1I2Clrhs.State = Trim(.GetDisplayText(8, 61, 15))
                I1I2Clrhs.Country = Trim(.GetDisplayText(9, 55, 25))
            Else
            'if the country is blank, get the domestic state and leave the country blank
                I1I2Clrhs.State = .GetDisplayText(11, 36, 2)
                I1I2Clrhs.Country = ""
            End If
            I1I2Clrhs.Status = Trim(.GetDisplayText(7, 36, 10))
            I1I2Clrhs.ZIP = Trim(.GetDisplayText(11, 40, 17))
            Phone = "(" & .GetDisplayText(13, 21, 3) & ") " & .GetDisplayText(13, 25, 8)
            'Get the alternate phone and e-mail if they're not blank.
            If Not Check4Text(15, 21, " ") Then AltPhone = "(" & GetText(15, 21, 3) & ") " & GetText(15, 25, 8) Else AltPhone = ""
            If Not Check4Text(17, 10, " ") Then Email = GetText(17, 10, 56) Else Email = ""
            'determine if the borrower only has PLUS loans
            'access TS26
            'school letter counter
            ls = 0
            'init array
            ReDim LtrSchls(0)
            .TransmitTerminalKey rcIBMClearKey
            .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
            .TransmitANSI "TX3Z/ITS26" & SSN
            .TransmitTerminalKey rcIBMEnterKey
            .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
            'review the loan type for each loan
            Row = 8
            PLOnly = "Y"
            hasTILP = False
            AllStopPursuit = True
            If .GetDisplayText(1, 72, 5) = "TSX28" Then
                Do
                    If .GetDisplayText(Row, 19, 4) = "TILP" Then hasTILP = True
                    PutText 21, 12, Format(GetText(Row, 2, 3), "0#"), "Enter" 'select loan
                    If Not Check4Text(3, 10, "CNSLD-STOP-PURSUIT") Then AllStopPursuit = False
                    If TS26() = "N" Then
                        ReDim Preserve LtrSchls(ls + 1)
                        LtrSchls(ls) = .GetDisplayText(13, 18, 8)
                        ls = ls + 1
                    End If
                    If .GetDisplayText(6, 66, 4) <> "PLUS" Then PLOnly = "N"
                    Hit "F12" 'back out of target screen
                    Row = Row + 1
                    If .GetDisplayText(Row, 3, 1) = " " Then
                        .TransmitTerminalKey rcIBMPf8Key
                        .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
                        If .GetDisplayText(23, 2, 5) = "90007" Then Exit Do
                        Row = 8
                    End If
                Loop
            Else
                If .GetDisplayText(6, 66, 4) <> "PLUS" Then PLOnly = "N"
                If .GetDisplayText(6, 66, 4) = "TILP" Then hasTILP = True
                If Not Check4Text(3, 10, "CNSLD-STOP-PURSUIT") Then AllStopPursuit = False
                ReDim Preserve LtrSchls(ls + 1)
                        LtrSchls(ls) = .GetDisplayText(13, 18, 8)
                        ls = ls + 1
            ...
            If PLOnly = "Y" Then
                MsgBox "The borrower only has PLUS loans so the script will generate letters to the schools which the student(s) attended.", , "PLUS Only Borrower"
                ReDim StuInfo(0) As StudentInfo
                GetStudentSSNFromTS26
                'check if any information was gathered
                If UBound(StuInfo) = 0 Then
                    'if nothing was gathered then complete the task
                    GoTo CompTask 'this kills me to do, but it keeps the code more readable
                Else
                    GatherStudentDemoInfoFromTX1J
                End If
                Okay = 3
            Else
                If hasTILP Then
                    MsgBox "The borrower has TILP loans.  Please review Clearinghouse and if the information on Clearinghouse is the same information on COMPASS then please call the College of Education.", vbCritical, "TILP Loans Found"
                Else
                    MsgBox "Please review Clearinghouse.", vbCritical, "FFELP Loans Only"
                End If
                'display the demographics screen
                .TransmitTerminalKey rcIBMClearKey
                .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
                .TransmitANSI "TX3Z/CTX1JB;" & SSN
                .TransmitTerminalKey rcIBMEnterKey
                .WaitForEvent rcKbdEnabled, "30", "0", 1, 1
                'display the dialog box and validate the entries made by the user
                Do
                    'prompt the user to review the information and hit <Insert> to continue
                    If warn <> 1 Then
                        Unload I1I2Clrhs
                        End
                    End If
                    ...
                    If I1I2Clrhs.Address1 = "" Or I1I2Clrhs.City = "" Or _
                       I1I2Clrhs.State = "" Or I1I2Clrhs.ZIP = "" Then
                        warn = MsgBox("A required field (Address1, City, State or ZIP) was left blank.  Click OK to return to the dialog box to complete the information or click cancel to quit.", vbOKCancel, "Information Entry Error")
                        If warn <> 1 Then
                            Unload I1I2Clrhs
                            End
                        End If
