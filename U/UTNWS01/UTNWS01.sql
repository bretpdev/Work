--UTNWS01 : REPAYMENT DISCLOSURES - BORROWERS
...
DROP TABLE IF EXISTS #MAINPOP,#RPSB_BasePop,#LN_LoanDetails,#RPS_Sums,#RD_Terms,#GD2_MonthlyRepaymentSchedule,#Output_TotalRepay,#LoanDetail,#Borrower_MRS_nonplus,#Borrower_MRS_plus,#Output_MonthlyRepaymentSchedule,#Borrower_nonplus,#Borrower_plus,#Output_LoanDetail,#POP;
...
BEGIN TRY
	BEGIN TRANSACTION;
		...
		SELECT DISTINCT
			...
			,LN65.LC_TYP_SCH_DIS
			,COALESCE(LN65.LA_ANT_CAP,0)	 AS LA_ANT_CAP --can be null
			,COALESCE(LN65.LA_RPD_INT_DIS,0) AS LA_RPD_INT_DIS --can be null
			,COALESCE(LN65.LA_TOT_RPD_DIS,0) AS LA_TOT_RPD_DIS --can be null
			,LN66.LN_RPS_TRM
			,LN66.LA_RPS_ISL
			...
			END AS IC_LON_PGM
			,DW01.WD_LON_RPD_SR
			,COALESCE(LN65.LA_CPI_RPD_DIS,0) AS LA_CPI_RPD_DIS --can be null
			,COALESCE(LN65.LA_ACR_INT_RPD,0) AS LA_ACR_INT_RPD --can be null
			,DEFR.LD_DFR_END
		INTO 
			#RPSB_BasePop
		FROM 
			#MAINPOP MP
			INNER JOIN RS10_BR_RPD RS10
				ON MP.BF_SSN = RS10.BF_SSN
				AND MP.LN_RPS_SEQ = RS10.LN_RPS_SEQ
			INNER JOIN LN65_LON_RPS LN65
		...
		--LN90: get sum of LA_FAT_NSI
			...
			(
				SELECT 
					LN90.BF_SSN
					,LN90.LN_SEQ
					,SUM(COALESCE(LN90.LA_FAT_NSI,0)) AS LA_FAT_NSI --can be null
				FROM 
					LN90_FIN_ATY LN90
				WHERE 
					LN90.LC_STA_LON90 = 'A'
					AND COALESCE(LN90.LC_FAT_REV_REA,'') = '' --can be null
		...
		WHERE 
			...
			AND LN10.LC_STA_LON10 = 'R' 
			AND RS10.LD_SNT_RPD_DIS <= @TODAY --do not print future printed disclosures
			AND RS10.LC_STA_RPST10 = 'A'
			AND LN65.LC_STA_LON65 = 'A'
		;
		--select * from #RPSB_BasePop --TEST
		...
		******************************************************************/
		...
		FROM
			#RPSB_BasePop
		;
		--select * from #LN_LoanDetails --TEST
		...
		SELECT
			...
			,CONVERT(DECIMAL(10,3),DL.STAT_RATE) AS STAT_RATE --statutory rate
			,SUM(DL.LA_CPI_RPD_DIS) AS LA_CPI_RPD_DIS --Current principal at time of repayment disclosure, already coalesced to 0 in #RPSB_BasePop
			,SUM(DL.LA_ANT_CAP) AS LA_ANT_CAP --Amount of anticipated interest capitalization, already coalesced to 0 in #RPSB_BasePop
		   ,(SUM(DL.LA_ANT_CAP) + SUM(DL.LA_CPI_RPD_DIS)) AS RS_PRI_RPY --LA_ANT_CAP and LA_CPI_RPD_DIS already coalesced to 0 in #RPSB_BasePop
			,SUM(DL.LA_RPD_INT_DIS) AS LA_RPD_INT_DIS --Disclosed amount of interest to be repaid,  LA_RPD_INT_DIS already coalesced to 0 in #RPSB_BasePop
			,SUM(DL.LA_TOT_RPD_DIS) AS LA_TOT_RPD_DIS --Total repayment amount (principal and interest) to be repaid, already coalesced to 0 in #RPSB_BasePop
			,SUM(COALESCE(BP.LA_FAT_NSI,0)) * -1 AS LA_FAT_NSI --Amount of non-subsidized (borrower) interest financial activity transaction, can be null
		INTO
			#RPS_Sums
		FROM
		...
		***************************************************************/
		SELECT
			...
			TS.PGM_TYP
		INTO
			#RD_Terms --Repayment Disclosure Terms
		FROM
			( -- term sequenced population
				SELECT
					...
					PGM_TYP,
					ROW_NUMBER() OVER (PARTITION BY BF_SSN, TermStartDate, PGM_TYP ORDER BY LN_RPS_TRM) AS TermSequenceASC
				FROM
					( -- total payments by term and gradation
						SELECT DISTINCT
							...
							RS.LN_RPS_TRM,
							BP.PGM_TYP,
							SUM(RS.LA_RPS_ISL) OVER (PARTITION BY RS.BF_SSN, RS.TermStartDate, BP.PGM_TYP) AS GradationTotalPayment,
							SUM(RS.LA_RPS_ISL) OVER (PARTITION BY RS.BF_SSN, RS.TermStartDate, RS.LN_RPS_TRM, BP.PGM_TYP) AS TermTotalPayment
						FROM 
		...
		(
			SELECT
				...
				RD.PGM_TYP
			FROM
				#RD_Terms RD --Repayment Disclosure Terms
			WHERE
		...
		FROM
			(--CTE calculated schedule
				SELECT
					...
					S.PGM_TYP,
					COUNT(*) OVER(PARTITION BY S.BF_SSN, S.PGM_TYP) AS SchedCount --partition PLUS and NONPLUS
				FROM
					SUMS S
				GROUP BY
					S.DF_SPE_ACC_ID,
		...
		;
		--select * from #GD2_MonthlyRepaymentSchedule --TEST
		...
		;
		--select * from #Output_TotalRepay --TEST
		...
		SELECT DISTINCT
			...
			,LD.LD_DFR_END
			,COUNT(*) OVER(PARTITION BY LD.BF_SSN) AS LoanCount
		INTO
			#LoanDetail
		FROM
			#RPSB_BasePop BP
		...
		**********************************************************/
		...
		ORDER BY
			...
			,LD.BF_SSN
		;
		--select * from #Borrower_MRS_nonplus --TEST
		...
		ORDER BY
			...
			,LD.BF_SSN
		;
		--select * from #Borrower_MRS_plus --TEST
						...
						( 
							...
							FROM    
								( 
									...
									FROM 
										#Borrower_MRS_nonplus IB --inner borrower data set
										LEFT JOIN
										(
											SELECT 
												DF_SPE_ACC_ID,
												...
												PGM_TYP,
												SchedCount,
												ROW_NUMBER() OVER(PARTITION BY BF_SSN ORDER BY BF_SSN, CONVERT(DATE,LD_RPS_1_PAY_DU)) AS LN_SEQ
											FROM
												#GD2_MonthlyRepaymentSchedule
											WHERE
												PGM_TYP = 'NONPLUS'
										) LD --loan data
											ON LD.BF_SSN = IB.BF_SSN
											AND LD.LN_SEQ = IB.Gradation
									WHERE
										OB.BF_SSN = IB.BF_SSN --outer borrower = inner borrower
								) x
							ORDER BY
								BF_SSN
							FOR XML PATH(''), TYPE
				...
				FROM
					( 
						SELECT DISTINCT
							...
							Gradation
						FROM 
							#Borrower_MRS_nonplus OB --outer borrower data set
						WHERE
							OB.PGM_TYP = 'NONPLUS'
					) OB
					--order by
							...
							FROM    
								( 
									SELECT DISTINCT
										...
										,COALESCE(CAST(LD.LD_RPS_1_PAY_DU AS VARCHAR(25)), '') AS LD_RPS_1_PAY_DU
									FROM 
										#Borrower_MRS_plus IB --inner borrower data set
										LEFT JOIN
										(
											SELECT 
												DF_SPE_ACC_ID,
				...
				FROM
					( 
						SELECT DISTINCT
							...
							Gradation
						FROM 
							#Borrower_MRS_plus OB --outer borrower data set
						WHERE
							OB.PGM_TYP = 'PLUS'
					) OB --outer borrower data set
				--order by
				--	obl.df_spe_acc_id
			) UNIONED
		;
		--select * from #Output_MonthlyRepaymentSchedule --TEST
		...
		**********************************************************/
		...
		ORDER BY
			...
			,LD.LN_SEQ
		;
		--select * from #Borrower_nonplus --TEST
		...
		ORDER BY
			 LD.DF_SPE_ACC_ID
			,LD.LN_SEQ
		;--select * from #Borrower_plus --TEST
						...
						( 
							...
							FROM    
								( 
									SELECT DISTINCT
										IB.Gradation,
										DENSE_RANK() OVER(PARTITION BY IB.BF_SSN ORDER BY COALESCE(LD.LN_SEQ,-1)) AS LN --force null loans to have a low rank (below 0) and order by that to force them to one side of the set
										--,LD.BF_SSN --TEST
										...
										,COALESCE(LD.LA_CPI_RPD_DIS,'')	AS LA_CPI_RPD_DIS --already coalesced to 0 in #RPSB_BasePop
										,COALESCE(LD.LA_ACR_INT_RPD,'')	AS LA_ACR_INT_RPD
										,COALESCE(LD.LD_DFR_END,'')		AS LD_DFR_END
									FROM 
										#Borrower_nonplus IB --inner borrower data set
										LEFT JOIN #LoanDetail LD
											ON LD.BF_SSN = IB.BF_SSN
											AND LD.LN_SEQ = IB.Gradation
											AND LD.PGM_TYP = IB.PGM_TYP
				...
				FROM
					( 
						SELECT DISTINCT
							...
							Gradation
						FROM 
							#Borrower_nonplus OB --outer borrower data set
						WHERE
							PGM_TYP = 'NONPLUS'
					) OB --outer borrower data set
					--order by
							...
							FROM    
								( 
									SELECT DISTINCT
										...
										,COALESCE(LD.LD_DFR_END,'')		AS LD_DFR_END
									FROM 
										#Borrower_plus IB --inner borrower data set
										LEFT JOIN #LoanDetail LD
											ON LD.BF_SSN = IB.BF_SSN
											AND LD.LN_SEQ = IB.Gradation
											AND LD.PGM_TYP = IB.PGM_TYP
				...
				FROM
					( 
						SELECT DISTINCT
							...
							Gradation
						FROM 
							#Borrower_plus OB --outer borrower data set
						WHERE
							PGM_TYP = 'PLUS'
					) OB --outer borrower data set
				--order by
		...
		;
		--select * from #Output_LoanDetail --TEST
		...
		********************************************/
		...
		FROM
			...
			(
				...
				ON R2.DF_SPE_ACC_ID = PH05.DF_SPE_ID
			LEFT JOIN PD10_PRS_NME PD10
				ON PD10.DF_SPE_ACC_ID = R2.DF_SPE_ACC_ID
		...
		;
		--select * from CLS.[print].PrintProcessing where ScriptDataId = 1 --TEST
		...
		********************************************/
		...
		FROM
			...
			(
				...
				ON R3.DF_SPE_ACC_ID = PH05.DF_SPE_ID
			LEFT JOIN PD10_PRS_NME PD10
				ON PD10.DF_SPE_ACC_ID = R3.DF_SPE_ACC_ID
			LEFT JOIN PD30_PRS_ADR PD30
				ON PD30.DF_PRS_ID = PD10.DF_PRS_ID
			LEFT JOIN
			(
		...
		WHERE
			...
			AND EXISTING_DATA.AccountNumber IS NULL --removes anyone already added within past 4 days
		;
		--select * from CLS.[print].PrintProcessing where ScriptDataId = 2 --TEST
		...
		SELECT
			...
			'Unable to insert a row into CLS.[print].PrintProcessing for account #' + DF_SPE_ACC_ID + '. Letter Type: ' + PGM_TYP + '. LetterData incomplete because ' + ErrorLocation + '. LetterData currently returned: ' + LetterData  AS LogMessage
		FROM
			#POP
		WHERE
			ErrorFlag = 1;
...
END TRY
BEGIN CATCH
	PRINT 'UTNWS01_borrower.sql transaction NOT committed.';
