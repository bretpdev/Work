...
FILENAME REPORT2 "&RPTLIB/CRI_Audit_Files_-_Current_Lender";
...
DATA _NULL_;            
      CALL SYMPUT('BEGIN', "'01/01/2013'" );
      CALL SYMPUT('END',"'12/31/2014'");
RUN;
...
PROC SQL;
	...
	CREATE TABLE DEMO_BOR AS
		...
		FROM CONNECTION TO DB2 
			(
				...
				WHERE 
					SUBSTR(LN10.IF_DOE_LDR,1,6) IN (&LENDERS)
					AND LN10.LC_STA_LON10 = 'R'
					AND LN35.LC_STA_LON35 = 'A'
					AND LN35.LD_OWN_EFF_SR < &END
					AND (LN35.LD_OWN_EFF_END >= &END OR LN35.LD_OWN_EFF_END IS NULL)
	...
	CREATE TABLE DEMO_LON AS
		...
		FROM CONNECTION TO DB2 
			(
				SELECT DISTINCT 
					...
					,CASE
						WHEN LN10.IF_DOE_LDR = ' ' THEN '000000'
						WHEN LN10.IF_DOE_LDR = LN10.LF_LON_CUR_OWN THEN '      '
					...
					,CASE
						WHEN DW01.WC_DW_LON_STA = '22' AND CANC.BF_SSN IS NOT NULL AND CANC.LN_SEQ IS NOT NULL THEN 'CA'
						...
						WHEN DW01.WC_DW_LON_STA = '22' AND PIF.BF_SSN IS NOT NULL AND PIF.LN_SEQ IS NOT NULL AND PIF.LD_FAT_EFF <= '12/31/2001' THEN 'PC'
						WHEN DW01.WC_DW_LON_STA = '22' AND PIF.BF_SSN IS NOT NULL AND PIF.LN_SEQ IS NOT NULL AND PIF.LD_FAT_EFF > '12/31/2001' THEN 'PN'
						WHEN DW01.WC_DW_LON_STA = '22' AND PIF.BF_SSN IS NULL AND PIF.LN_SEQ IS NULL AND CANC.BF_SSN IS NULL AND CANC.LN_SEQ IS NULL THEN 'PF'
					...
					,CASE
						WHEN DW01.WC_DW_LON_STA = '01' THEN SD10.LD_SCL_SPR		
						WHEN DW01.WC_DW_LON_STA IN ('02','23') THEN LN10.LD_LON_1_DSB
						WHEN DW01.WC_DW_LON_STA IN ('03','07','08','09','10','11','12','13','14','15') THEN DW01.WD_LON_RPD_SR
						WHEN DW01.WC_DW_LON_STA = '06' THEN LN38.LD_CU_STA_ENT
						WHEN DW01.WC_DW_LON_STA = '04' AND DFR.BF_SSN IS NOT NULL AND DFR.LN_SEQ IS NOT NULL THEN DFR.LD_DFR_BEG	
						WHEN DW01.WC_DW_LON_STA = '05' AND FORB.BF_SSN IS NOT NULL AND FORB.LN_SEQ IS NOT NULL THEN FORB.LD_FOR_BEG
						WHEN DW01.WC_DW_LON_STA = '22' AND CANC.BF_SSN IS NOT NULL AND CANC.LN_SEQ IS NOT NULL THEN CANC.LD_FAT_EFF
						WHEN DW01.WC_DW_LON_STA = '22' AND PIF.BF_SSN IS NOT NULL AND PIF.LN_SEQ IS NOT NULL THEN PIF.LD_FAT_EFF
						WHEN DW01.WC_DW_LON_STA = '22' AND CANC.BF_SSN IS NULL AND CANC.LN_SEQ IS NULL AND PIF.BF_SSN IS NULL AND PIF.LN_SEQ IS NULL THEN LN10.LD_PIF_RPT
						WHEN DW01.WC_DW_LON_STA = '16' THEN PD21.DD_DTH_STA 
						WHEN DW01.WC_DW_LON_STA = '17' THEN PD20.DD_DTH_NTF
						WHEN DW01.WC_DW_LON_STA = '18' THEN DIS_ALG.DD_PRS_DSA_SPS_SR
						WHEN DW01.WC_DW_LON_STA = '19' THEN DIS_VRF.DD_DSA_RPT 
						WHEN DW01.WC_DW_LON_STA = '20' AND PD24.DC_BKR_STA = '04' THEN PD24.DD_BKR_FIL 
						WHEN DW01.WC_DW_LON_STA = '21' AND PD24.DC_BKR_STA = '06' THEN PD24.DD_BKR_VER  
						ELSE '9999-12-31'
					...
					,DAYS(LN10.LD_END_GRC_PRD) + 1 AS DATE_ENTERED_REPAYMENT
					,LN72.LR_ITR * 1000 AS CURRENT_INTEREST_RATE
					,CASE
						WHEN LN65.LC_TYP_SCH_DIS IN ('IB','IL') THEN 'Y'
						ELSE 'N'
					 END AS IBR_CODE
					,CASE
						WHEN LN65.LC_TYP_SCH_DIS = 'IB' THEN 'PF'
					...
					END AS PFH_IBR_PAYMENT_PLAN_TYPE_CODE 			/*			PFH/IBR Payment Plan Type code*/
		...
		/*			PFH Start Date*/
					...
					,COALESCE(DELQ.LN_DLQ_MAX,0) AS DAYS_DELINQUENT /*			Number of Days Delinquent*/
					,LN10.IC_LON_PGM AS LOAN_TYPE
					,CLM.LD_FAT_EFF AS CLAIM_PAID_DATE
					,INT_CAP.AMT AS CAPITALIZED_INTEREST 			/*			Capitalized Interest*/
		/*			Loan Purchase Date added later after remote submit as it uses a local permanent data set*/
					,COALESCE(LOAN_SOLD.LD_FAT_EFF, '9999-12-31') AS DATE_LOAN_SOLD /*			Date Loan Sold*/
					...
					,DFR.LC_DFR_TYP AS DEFERMENT_TYPE 				/*			Deferment Type Code*/
					,CASE
						WHEN LN10.IC_LON_PGM IN ('UNSPC','SUBSPC') THEN 'Y'
						ELSE 'N'
...
DATA DEMO_BOR; SET DUSTER.DEMO_BOR; RUN;
DATA DEMO_LON; SET DUSTER.DEMO_LON; RUN;

PROC SORT DATA=DEMO_BOR; BY BF_SSN; RUN;
PROC SORT DATA=DEMO_LON; BY BF_SSN LN_SEQ; RUN;

DATA DEMO;
	MERGE DEMO_BOR DEMO_LON;
	BY BF_SSN;
RUN;
	...
	CREATE TABLE DEMO AS
		...
		FROM
			...
			LEFT JOIN PROGREVW.LOAN_PURCHASE_DATE LP
				ON DEMO.LF_LON_CUR_OWN = LP.LENDER_ID
	;
QUIT;
...
DATA _NULL_;
	...
	FORMAT LAST_NAME $QUOTE35. ;
	FORMAT FIRST_NAME $QUOTE15. ;
	FORMAT MIDDLE_INITIAL $1. ;
	FORMAT DD_BRT YYMMDDn8. ;
	FORMAT LF_LON_CUR_OWN $6. ;
	FORMAT LD_OWN_EFF_SR YYMMDDn8. ;
	FORMAT PREVIOUS_LENDER_CODE $6. ;
/*	FORMAT PREVIOUS_HOLDER_EFFECTIVE_DATE YYMMDDn8. ;*/
	FORMAT ORIGINAL_HOLDER $6. ;
/*	FORMAT ORIGINAL_HOLDER_EFFECTIVE_DATE YYMMDDn8. ;*/
	FORMAT CURRENT_GUARANTOR_CODE $3. ;
	FORMAT CURRENT_GUARANTOR_EFFECTIVE_DATE YYMMDDn8. ;
	FORMAT PREVIOUS_GUARANTOR_EFF_DTE $8. ;
	FORMAT ORIGINAL_GUARANTOR_CODE $3. ;
	FORMAT ORIGINAL_GUARANTOR_EFF_DTE $8. ;
	FORMAT CURRENT_SERVICER_CODE $6. ;
	FORMAT CURRENT_SERVICER_EFFECTIVE_DATE YYMMDDn8. ;
	FORMAT PREVIOUS_SERVICER_CODE $6. ;
	FORMAT PREVIOUS_SERVICER_EFFECTIVE_DATE $8. ;
	FORMAT ORIGINAL_SERVICER_CODE $6. ;
	FORMAT ORG_SERVICER_CODE_EFF_DTE $8. ;
	FORMAT BWR_ELECTRONIC_SIG_INDICATOR_CDE $1. ;
	FORMAT E_SIGNATURE_SOURCE_TYPE_CODE $7. ;
	FORMAT COMMONLINE_UNIQUE_ID $17. ;
	FORMAT COMMONLINE_UNIQUE_ID_SEQUENCE 2. ;
	FORMAT UNIQUE_LOAN_ID $16. ;
	FORMAT GUARANTY_DATE YYMMDDn8. ;
	FORMAT LOAN_PERIOD_BEGIN_DATE YYMMDDn8. ;
	FORMAT LOAN_PERIOD_END_DATE YYMMDDn8. ;
	FORMAT FIRST_DISBURSEMENT_DATE YYMMDDn8. ;
	FORMAT LOAN_STATUS_CODE $2. ;
/*	FORMAT LOAN_STATUS_DATE YYMMDDn8. ;*/
	FORMAT CANCELLATION_DATE YYMMDDn8. ;
	FORMAT RETURN_TO_LENDER_DATE YYMMDDn8. ;
	FORMAT OUT_OF_SCHOOL_DATE YYMMDDn8. ;
	FORMAT DATE_ENTERED_REPAYMENT YYMMDDn8. ;
	FORMAT CURRENT_INTEREST_RATE 5. ;
	FORMAT IBR_CODE $1. ;
	FORMAT PFH_IBR_PAYMENT_PLAN_TYPE_CODE $2. ;
	/*			PFH Start Date*/
	FORMAT TOTAL_BORROWER_PAYMENTS 8.2 ;
	FORMAT CURRENT_PRINCIPAL_BALANCE 8.2 ;
	FORMAT LATE_FEES_ASSESSED 8.2 ;
	FORMAT DAYS_DELINQUENT 8. ;
	FORMAT LOAN_TYPE $LON_STA. ;
	FORMAT CLAIM_PAID_DATE YYMMDDn8. ;
	FORMAT CAPITALIZED_INTEREST 8.2 ;
	FORMAT LOAN_TRANSFER_DATE $8. ;
	FORMAT DEFERMENT_BEGIN_DATE YYMMDDn8. ;
	FORMAT DEFERMENT_END_DATE YYMMDDn8. ;
	FORMAT FORBEARANCE_BEGIN_DATE YYMMDDn8. ;
	FORMAT FORBEARANCE_END_DATE YYMMDDn8. ;
	FORMAT DEFERMENT_TYPE DEF_TYP.;
	FORMAT SPOUSAL_CONSOLIDATION_LOAN $1. ;
	FORMAT FILLER $75. ;
