...
/****** Object:  View [uhedaymet].[Daily_BasePopulation]    Script Date: 11/5/2019 1:33:12 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [uhedaymet].[Daily_BasePopulation]
...
FROM 
	...
	( --Deferment indicator
		...
		WHERE
			...
			AND LN50.LD_DFR_END >= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
			AND LN50.LD_DFR_APL <= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
			AND LN50.LD_DFR_BEG <= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
	) InDefer 
	...
	( -- forbearance indicator
		...
		WHERE
			...
			AND LN60.LD_FOR_END >= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
			AND LN60.LD_FOR_APL <= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
			AND LN60.LD_FOR_BEG <= CAST(GETDATE() AS DATE) /*Last Day Prev Month*/
	) FORB 
	...
	( --Bill sent this month
		...
		WHERE
			BL10.LD_BIL_DU BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) /*first day of current month*/ AND CAST(DATEADD(SECOND,-1,DATEADD(MONTH, DATEDIFF(MONTH,0,GETDATE())+1,0)) AS DATE) -- last day of current month
			AND BL10.LC_IND_BIL_SNT != '5' --Exclude paid ahead bills from counting
			AND LN80.LD_BIL_DU_LON BETWEEN DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) /*first day of current month*/ AND CAST(DATEADD(SECOND,-1,DATEADD(MONTH, DATEDIFF(MONTH,0,GETDATE())+1,0)) AS DATE) -- last day of current month
...
WHERE 
	...
	AND LN10.LD_LON_ACL_ADD <= CAST(GETDATE() AS DATE) /*last day prev month*/
