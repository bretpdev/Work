...
/****** Object:  View [uhedaymet].[Daily_BorrowerLevel]    Script Date: 11/12/2019 8:16:14 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [uhedaymet].[Daily_BorrowerLevel]
...
(
	SELECT 
		...
		MAX(PerfLoan.DefermentIndicator) as LoanDefermentIndicator,
		...
		CASE 
			 WHEN COUNT(DISTINCT BD.IC_LON_PGM) = 1 THEN MAX(BD.IC_LON_PGM)
	...
	FROM
		[uhedaymet].[Daily_LoanLevel] BD
		LEFT JOIN [uhedaymet].[Daily_LoanLevel] PerfLoan 
...
WHERE 
	 BorrowerLevel.LOAN_COUNT != BorrowerLevel.PIF_CT_B4_REP_MO + BorrowerLevel.DSTAT_CT_B4_REP_MO
