...
CREATE PROCEDURE utlwm30.GetR2
AS
...
(
	SELECT DISTINCT 
		...
		BR01.BN_RHB_PAY_CTR AS [Rehab Counter],
		DC01.LD_NXT_PAY_DUE AS [Next Payment Due],
		CASE WHEN  ISNULL(PF_ACT, '') IN ('DRTRE','DRBWR') THEN 'Y' ELSE 'N' END AS [Rehab],
		DC01.LI_DIR_PAY,
		ROW_NUMBER() OVER (
			PARTITION BY
				PD01.DF_SPE_ACC_ID,
				BR01.BN_RHB_PAY_CTR,
				DC01.LD_NXT_PAY_DUE,
				CASE WHEN  ISNULL(PF_ACT, '') IN ('DRTRE','DRBWR') THEN 'Y' ELSE 'N' END
			ORDER BY 
				CASE 
					WHEN ISNULL(DC01.LI_DIR_PAY,'') = '' THEN 3
					WHEN ISNULL(DC01.LI_DIR_PAY,'') = 'N' THEN 2
					WHEN ISNULL(DC01.LI_DIR_PAY,'') = 'Y' THEN 1
					ELSE 4
				END
		) AS [Priority]
	FROM 
