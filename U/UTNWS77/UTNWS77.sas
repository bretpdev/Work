...
%LET RPTLIB = T:\SAS;
FILENAME REPORTZ "&RPTLIB/UNWS77.NWS77RZ";
FILENAME REPORT2 "&RPTLIB/UNWS77.NWS77R2";
FILENAME REPORT3 "&RPTLIB/UNWS77.NWS77R3";

/*LIBNAME SAS_TAB V8 '/sas/whse/progrevw';*/
LIBNAME SAS_TAB V8 'Y:\Development\SAS Test Files\progrevw';

/*There are two identical reports in this code, just for different populations. The first segment is written to R2, 
  the second to R3. The code for the second report starts on line 349. Notice all SAS datasets have the SAME NAME and 
  are therefore OVERWRITTEN*/
...
/*This is the start of the second report that gets written to R3*/
/*assign default values to macro variables so the variables are always resolved even when there are no campaigns for the report date*/
DATA _NULL_;
	CALL SYMPUT('BEG_DATE',&REPORT_DATE);
	CALL SYMPUT('END_DATE',&REPORT_DATE);
...
DATA _NULL_;
	...
	WHERE 
		&REPORT_DATE BETWEEN BEGIN_DATE AND END_DATE
		AND CAMPAIGN_NAME LIKE ('%CR 2651 Control')
	;
RUN;
...
PROC SQL;
      CREATE TABLE CURRENT_CAMPAIGN_BORROWERS AS
            ...
            WHERE
                  &REPORT_DATE BETWEEN BEGIN_DATE AND END_DATE
				  AND CAMP.CAMPAIGN_NAME LIKE '%CR 2651 Control'
      ;
QUIT;
