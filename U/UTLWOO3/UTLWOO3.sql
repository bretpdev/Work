/************ main query converted from SAS for use in SSIS package***********************/
USE UDW
GO

TRUNCATE TABLE ULS.scra.ActiveDutyValidation;
...
(--borrower data
	...
	WHERE
		LN10.LC_STA_LON10 = 'R' --released loans
		AND (LN10.LA_CUR_PRI + DW01.WA_TOT_BRI_OTS) > 0.00 --open loans
...
(--earmarks output group based on number
	SELECT
		*,
		CASE
			WHEN [TALLY] BETWEEN 1 AND 249999		THEN 'UTLWOO3R2'
			WHEN [TALLY] BETWEEN 250000 AND 499998	THEN 'UTLWOO3R3'
			WHEN [TALLY] BETWEEN 499999 AND 749997	THEN 'UTLWOO3R4'
			WHEN [TALLY] BETWEEN 749998 AND 999996	THEN 'UTLWOO3R5'
			WHEN [TALLY] BETWEEN 999997 AND 1249995	THEN 'UTLWOO3R6'
			ELSE 'UTLWOO3R7'
		END AS [FILENAME]
	FROM
...
SELECT DISTINCT [FILENAME] FROM [ULS].[scra].[ActiveDutyValidation]
