...
/***********************************************************************
 IF THIS JOB NEEDS TO BE RUN FOR A SPECIFIC SSN ENTER IT BELOW AND UNNCOMMENT THE APPROPRIATE CODE LINES:
************************************************************************/
/*%LET SSN = 'xxxxxxxxx';*single borrower;*/
...
DATA _NULL_;
	...
	CALL SYMPUT('DAYS_AGO_7',"'"||PUT(INTNX('DAY',TODAY(),-7,'BEGINNING'), MMDDYY10.)||"'");
	...
	CREATE TABLE BLSTMNT AS
		...
		FROM 
			...
			(
				SELECT DISTINCT 
					 BL10.BF_SSN
					,BL10.LD_BIL_CRT /*bill create date*/
					,BL10.LN_SEQ_BIL_WI_DTE
					,BL10.LC_IND_BIL_SNT /*bill sent indicator code*/
					,BL10.LD_RBL_LST
					,LN10.LN_SEQ
					,AY10.LD_ATY_REQ_RCV /*date arc request received*/
					,RS10.LD_RPS_1_PAY_DU /*1st repayment due date loan repayment schedule*/
					,LN10.LF_LON_CUR_OWN
					,LN10.LD_LON_1_DSB
					,LN10.IC_LON_PGM
					,CASE
						 WHEN LN80.LC_LON_STA_BIL = '1'
						 THEN 'REPAY'
						 WHEN LN80.LC_LON_STA_BIL = '2'
						 THEN 'FORB'
						 WHEN LN80.LC_LON_STA_BIL = '3'
						 THEN 'DEFER'
						 WHEN LN80.LC_LON_STA_BIL = '4'
						 THEN 'SCHOOL'
						 WHEN LN80.LC_LON_STA_BIL = '5'
						 THEN 'GRACE'
						 ELSE 'ERROR'
					 END AS LC_LON_STA_BIL
					,LN80.LR_INT_BIL /*interest rate at time of bill*/
					,COALESCE(LN80.LA_CUR_PRN_BIL,0) 
						+ (CASE
								WHEN COALESCE(LN80.LA_TOT_NSB_INT_OTS, 0.00) = 0.00 AND COALESCE(DW01.WA_TOT_BRI_OTS, 0.00) ^= 0.00
								THEN DW01.WA_TOT_BRI_OTS
								WHEN COALESCE(LN80.LA_TOT_NSB_INT_OTS, 0.00) ^= 0.00
								THEN LN80.LA_TOT_NSB_INT_OTS
								ELSE 0.00
							END)
						+ COALESCE(LN80.LA_LTE_FEE_OTS_PRT,0) 
					AS LA_CUR_PRN_BIL /*current principal printed on bill*/
					,LN80.LA_BIL_PAS_DU
					,LN80.LA_BIL_CUR_DU
					,LN80.LA_LTE_FEE_OTS_PRT AS LN_LTE_FEE
					,COALESCE(LN80.LA_BIL_DU_PRT, LN80.LA_BIL_CUR_DU - COALESCE(LN80.LA_TOT_BIL_STS, 0), 0) AS LA_BIL_DU_PRT /*amount due physically printed on bill*/
					,BL10.LC_BIL_TYP
					,BL10.LD_BIL_DU
					,BL10.LN_SEQ_BIL_WI_DTE
					,CASE
					...
					,CASE
					 ...
					 END AS BLL_DT
					,BL10.LC_BIL_MTD /*billing method code*/
					,BL10.LC_IND_BIL_SNT AS SENT_IND_GROUP
					/*for R2-99 processing*/
					,CASE
						WHEN PH05A.DI_CNC_EBL_OPI = 'Y' AND DI_VLD_CNC_EML_ADR = 'Y'
						THEN 'Y'
						ELSE 'N'
...
DATA BLSTMNT;
	...
	IF LD_ATY_REQ_RCV = . THEN OUTPUT; *1st time bill;
	ELSE IF LD_ATY_REQ_RCV < LD_BIL_CRT THEN OUTPUT; *standard bill;
	ELSE IF LD_ATY_REQ_RCV < LD_RBL_LST THEN OUTPUT; *rebill;
RUN;
...
RUN;
/*****************************************************
* ADD TERMS REMAINING COLUMN TO BLSTMNT DATA SET
******************************************************/
PROC SQL;
	CONNECT TO DB2 (DATABASE=&DB);
	CREATE TABLE LN72 AS
		SELECT	
			 LN72.BF_SSN
			,LN72.LN_SEQ
			,LN72.LD_STA_LON72
		...
		FROM
			...
			(
				...
				WHERE
					LN72.LC_STA_LON72 = 'A'
					AND	DAYS(CURRENT DATE) BETWEEN DAYS(LN72.LD_ITR_EFF_BEG) AND DAYS(LN72.LD_ITR_EFF_END)

				FOR READ ONLY WITH UR
			) LN72
				ON LN72.BF_SSN = BL.BF_SSN
				AND LN72.LN_SEQ = BL.LN_SEQ
	;

	CREATE TABLE _DEFER AS /*deferments staging table*/
		SELECT DISTINCT
			 DFR.BF_SSN
			,DFR.LN_SEQ
			,DFR.LN_RPS_SEQ
			...
			,DFR.LD_DFR_END
			,DFR.LC_DFR_TYP
			,(/*CentralData.dbo.PaymentsDue function equivalent*/
				INTCK('MONTH', DFR.StartDate, DFR.EndDate) /*aka DateDiff*/
				+ CASE 
					WHEN DAY(DFR.StartDate) <= DFR.DayOfMonth 
					THEN 1
					ELSE 0
				END /*aka StartDateBit*/
				+ (
					CASE
						WHEN DAY(DFR.EndDate) >= DFR.DayOfMonth 
						THEN 1
						ELSE 0
					END
				  )-1 /*aka EndDateBit*/
...
/*			,EndDate*/
/*			,DayOfMonth*/
/*			,INTCK('MONTH', StartDate, EndDate) AS DateDiff */
/*			,CASE WHEN DAY(StartDate) <= DayOfMonth THEN 1 ELSE 0 END AS StartDateBit*/
/*			,(CASE WHEN DAY(EndDate) >= DayOfMonth THEN 1 ELSE 0 END)-1 AS EndDateBit*/
		FROM
			BLSTMNT BL
			INNER JOIN CONNECTION TO DB2
			(
				SELECT DISTINCT
					...
					,DF10.LC_DFR_TYP
					,CASE
						WHEN DAYS(LN50.LD_DFR_BEG) > DAYS(RS10.LD_RPS_1_PAY_DU) 
						THEN LN50.LD_DFR_BEG 
						ELSE RS10.LD_RPS_1_PAY_DU 
					END AS StartDate
					,CASE
						WHEN DAYS(LN50.LD_DFR_END) < DAYS(CURRENT DATE)
						THEN LN50.LD_DFR_END 
						ELSE CURRENT DATE
					END AS EndDate
					,DAY(RS10.LD_RPS_1_PAY_DU) AS DayOfMonth
				FROM
					...
					INNER JOIN OLWHRM1.LN65_LON_RPS LN65 
						ON LN65.BF_SSN = DF10.BF_SSN 
						AND LN65.LN_SEQ = LN50.LN_SEQ
					INNER JOIN OLWHRM1.RS10_BR_RPD RS10 
						ON RS10.BF_SSN = DF10.BF_SSN 
						AND RS10.LN_RPS_SEQ = LN65.LN_RPS_SEQ
				WHERE
					DF10.LC_DFR_STA = 'A'
					AND DF10.LC_STA_DFR10 = 'A'
					AND LN50.LC_STA_LON50 = 'A'
					AND LN50.LC_DFR_RSP != '003' /*deferment request denied*/
					AND	DAYS(LN50.LD_DFR_BEG) <= DAYS(CURRENT DATE)
					AND DAYS(LN50.LD_DFR_END) >= DAYS(RS10.LD_RPS_1_PAY_DU)
					AND	RS10.LC_STA_RPST10 = 'A'

				FOR READ ONLY WITH UR
			) DFR
				ON DFR.BF_SSN = BL.BF_SSN
				AND DFR.LN_SEQ = BL.LN_SEQ	
	;

	CREATE TABLE _FORB AS /*forbearance staging table*/
		SELECT DISTINCT
			 FRB.BF_SSN
			,FRB.LN_SEQ
			,FRB.LN_RPS_SEQ
			...
			,FRB.LC_FOR_TYP
			,(/*CentralData.dbo.PaymentsDue function equivalent*/
				INTCK('MONTH', FRB.StartDate, FRB.EndDate) /*aka DateDiff*/
				+ CASE 
					WHEN DAY(FRB.StartDate) <= FRB.DayOfMonth 
					THEN 1 
					ELSE 0 
				END /*aka StartDateBit*/
				+ (
					CASE 
						WHEN DAY(FRB.EndDate) >= FRB.DayOfMonth 
						THEN 1 
						ELSE 0 
					END
				  )-1 /*aka EndDateBit*/
		...
		FROM
			...
			(
				SELECT DISTINCT
					...
					,FB10.LC_FOR_TYP
					,CASE
						WHEN DAYS(LN60.LD_FOR_BEG) > DAYS(RS10.LD_RPS_1_PAY_DU)
						THEN LN60.LD_FOR_BEG
						ELSE RS10.LD_RPS_1_PAY_DU 
					END AS StartDate
					,CASE
						WHEN DAYS(LN60.LD_FOR_END) < DAYS(CURRENT DATE)
						THEN LN60.LD_FOR_END 
						ELSE CURRENT DATE
					END AS EndDate
					,DAY(RS10.LD_RPS_1_PAY_DU) AS DayOfMonth
				FROM
					...
					INNER JOIN OLWHRM1.LN65_LON_RPS LN65 
						ON LN65.BF_SSN = FB10.BF_SSN 
						AND LN65.LN_SEQ = LN60.LN_SEQ
					INNER JOIN OLWHRM1.RS10_BR_RPD RS10 
						ON RS10.BF_SSN = FB10.BF_SSN 
						AND RS10.LN_RPS_SEQ = LN65.LN_RPS_SEQ	
				WHERE
					FB10.LC_FOR_STA = 'A'
					AND FB10.LC_STA_FOR10 = 'A'
					AND LN60.LC_STA_LON60 = 'A'
					AND LN60.LC_FOR_RSP != '003' /*forbearance request denied*/
					AND DAYS(LN60.LD_FOR_BEG) <= DAYS(CURRENT DATE)
					AND DAYS(LN60.LD_FOR_END) >= DAYS(RS10.LD_RPS_1_PAY_DU)
					AND RS10.LC_STA_RPST10 = 'A'

				FOR READ ONLY WITH UR
			)FRB
				ON FRB.BF_SSN = BL.BF_SSN
				AND FRB.LN_SEQ = BL.LN_SEQ
	;

	DISCONNECT FROM DB2;
QUIT;

*adds row numbers to LN72;
PROC SORT DATA = LN72;
	BY BF_SSN 
	   LN_SEQ 
	   DESCENDING LD_STA_LON72;
RUN;
DATA LN72;
	SET LN72;
	BY BF_SSN 
	   LN_SEQ;
	IF FIRST.LN_SEQ	THEN SEQ=0;
	SEQ+1;
RUN;

*adds row numbers to deferments staging table;
PROC SORT DATA = _DEFER;
	BY BF_SSN
	   LN_SEQ
	   DESCENDING LN_RPS_SEQ;
RUN;
DATA _DEFER;
	SET _DEFER;
	BY BF_SSN
...
*adds row numbers to forbearance staging table;
PROC SORT DATA = _FORB;
	BY BF_SSN
	   LN_SEQ
	   DESCENDING LN_RPS_SEQ;
RUN;
...
/*LIBNAME OLWHRM1 DB2 DATABASE=&DB OWNER=OLWHRM1;*UNCOMMENT FOR LIVE,COMMENT OUT FOR TEST;*/
PROC SQL;
	*deferment months already used;
	CREATE TABLE DEFER AS
		SELECT DISTINCT
			 _DEFER.BF_SSN
			,_DEFER.LN_SEQ
			,_DEFER.LN_RPS_SEQ
			,_DEFER.LD_RPS_1_PAY_DU
			,_DEFER.LD_DFR_BEG
			,_DEFER.LD_DFR_END
			,_DEFER.LC_DFR_TYP
			,_DEFER.SEQ
			,SumPartition.TotalDfrMonthsUsed
		FROM
			_DEFER
			LEFT JOIN
			(/*sum over partition equivalent using previously-calculated values*/
				SELECT 
					 BF_SSN
					,LN_SEQ
					,LN_RPS_SEQ
					,SUM(PaymentsDue) AS TotalDfrMonthsUsed
				FROM
					_DEFER
				GROUP BY
					BF_SSN
				...
				ON _DEFER.BF_SSN = SumPartition.BF_SSN
				AND _DEFER.LN_SEQ = SumPartition.LN_SEQ
	;

	*forbearance months already used;
	CREATE TABLE FORB AS
		SELECT DISTINCT
			 _FORB.BF_SSN
			,_FORB.LN_SEQ
			,_FORB.LN_RPS_SEQ
			,_FORB.LD_RPS_1_PAY_DU
			,_FORB.LD_FOR_BEG
			,_FORB.LD_FOR_END
			,_FORB.LC_FOR_TYP
			,_FORB.SEQ
			,SumPartition.TotalFrbMonthsUsed
		FROM
			_FORB
			LEFT JOIN
			(/*sum over partition equivalent using previously-calculated values*/
				SELECT 
					...
					,LN_SEQ
					,LN_RPS_SEQ
					,SUM(PaymentsDue) AS TotalFrbMonthsUsed
				FROM
					_FORB
				GROUP BY
					BF_SSN
				...
				ON _FORB.BF_SSN = SumPartition.BF_SSN
				AND _FORB.LN_SEQ = SumPartition.LN_SEQ
	;

	CREATE TABLE _RepaymentSchedules AS
		SELECT DISTINCT 
			 LN10.BF_SSN
			,LN10.LN_SEQ
			,RS10.LN_RPS_SEQ
			...
			,LN65.LA_TOT_RPD_DIS
			,LN65.LA_ANT_CAP
			,COALESCE(LN66.LN_RPS_TRM, 0) AS LN_RPS_TRM
			,LN66.LA_RPS_ISL
			,COALESCE(DEFER.TotalDfrMonthsUsed, 0) AS TotalDfrMonthsUsed
			,COALESCE(FORB.TotalFrbMonthsUsed, 0) AS TotalFrbMonthsUsed
			,INTCK('MONTH', RS10.LD_RPS_1_PAY_DU, TODAY()) - (COALESCE(FORB.TotalFrbMonthsUsed, 0) + COALESCE(DEFER.TotalDfrMonthsUsed, 0)) AS GradationMonths
			,RS10.LD_RPS_1_PAY_DU /*initial TermStartDate*/
		FROM
			OLWHRM1.PD10_PRS_NME PD10
			...
			INNER JOIN BLSTMNT BL /*limit pop to current bill pulldown*/
				ON LN10.BF_SSN = BL.BF_SSN
				AND LN10.LN_SEQ = BL.LN_SEQ	
			LEFT JOIN LN72 /*values calculated previously*/
				ON LN10.BF_SSN = LN72.BF_SSN
				AND LN10.LN_SEQ = LN72.LN_SEQ
				AND LN72.SEQ = 1
			LEFT JOIN DEFER /*values calculated previously*/
				ON DEFER.BF_SSN = PD10.DF_PRS_ID
				AND DEFER.LN_SEQ = LN10.LN_SEQ
				AND DEFER.LN_RPS_SEQ = RS10.LN_RPS_SEQ
			LEFT JOIN FORB /*values calculated previously*/
				ON FORB.BF_SSN = PD10.DF_PRS_ID
				AND FORB.LN_SEQ = LN10.LN_SEQ
				AND FORB.LN_RPS_SEQ = RS10.LN_RPS_SEQ
		WHERE
			LN10.LC_STA_LON10 = 'R'
			AND LN65.LC_STA_LON65 = 'A'
			AND RS10.LC_STA_RPST10 = 'A'
;QUIT;
/*LIBNAME OLWHRM1 CLEAR; *UNCOMMENT FOR LIVE,COMMENT OUT FOR TEST;*/

*calculate TermsToDate and TermStartDate per loan;
DATA _RepaymentSchedules;
	SET _RepaymentSchedules;
	BY BF_SSN LN_SEQ;
	IF FIRST.LN_SEQ THEN TermsToDate = 0; *initialize first term per loan seq to 0;
	TermsToDate + LN_RPS_TRM; *running total of terms per loan seq;
	TermStartDate = INTNX('MONTH', LD_RPS_1_PAY_DU, (TermsToDate - LN_RPS_TRM), 'SAME');
	FORMAT TermStartDate DATE9.;
RUN;

*determine current gradation;
PROC SQL;
	CREATE TABLE RepaymentSchedules AS
		SELECT
			*,
			CASE
				WHEN (
						CASE
							WHEN GradationMonths < 1 
							THEN 1 
							ELSE GradationMonths 
						END
					) BETWEEN (TermsToDate - LN_RPS_TRM + 1) AND TermsToDate
				THEN 1
				ELSE 0
			END AS CurrentGradation
		FROM
			_RepaymentSchedules
	;

*calculate terms remaining;
	CREATE TABLE TermsRemaining AS
		SELECT
			BL.*,
/*			RPS.BF_SSN,*/
/*			RPS.LN_SEQ,*/
			RPS.TermsToDate - RPS.GradationMonths AS TermsRemaining
		FROM
			(
				SELECT 
					RS.BF_SSN,
			...
			INNER JOIN RepaymentSchedules RPS
			...
			INNER JOIN BLSTMNT BL /*limit pop to current bill pulldown*/
				ON RPS.BF_SSN = BL.BF_SSN
				AND RPS.LN_SEQ = BL.LN_SEQ	
;QUIT;
...
PROC DATASETS NOPRINT;
	DELETE LN72 _DEFER _FORB DEFER FORB _RepaymentSchedules RepaymentSchedules TermsRemaining;	
QUIT;

*for testing;
/*endrsubmit*/
/*DATA LN72; SET DUSTER.LN72; RUN;*/
/*DATA _DEFER; SET DUSTER._DEFER; RUN;*/
/*DATA DEFER; SET DUSTER.DEFER; RUN;*/
/*DATA _FORB; SET DUSTER._FORB; RUN;*/
/*DATA FORB; SET DUSTER.FORB; RUN;*/
/*DATA _RepaymentSchedules; SET DUSTER._RepaymentSchedules (obs=100); RUN;*/
/*DATA RepaymentSchedules; SET DUSTER.RepaymentSchedules (obs=200); RUN;*/
/*DATA TermsRemaining; SET DUSTER.TermsRemaining (obs=200); RUN;*/
/*proc sql; select * from TermsRemaining order by bf_ssn, ln_seq; quit;*/
	...
	FROM 
		...
		(
			SELECT 
				LN10.BF_SSN
				,CASE
					WHEN PD30_B.DC_DOM_ST_B <> ' ' AND SUBSTR(PD30_B.DF_ZIP_CDE_B,6,1) = ' '
		...
		(
			SELECT DISTINCT 
				...
				,CASE 
					...
					WHEN PD10.DM_PRS_MID <> ' ' 
					THEN RTRIM(PD10.DM_PRS_1)||' '||RTRIM(PD10.DM_PRS_MID)||' '||RTRIM(PD10.DM_PRS_LST)||RTRIM(' '||RTRIM(PD10.DM_PRS_LST_SFX))
					ELSE 'ERROR'
				 END AS NAME
				,PD10.DM_PRS_1
				,PD10.DM_PRS_MID
				,RTRIM(PD10.DM_PRS_LST)||RTRIM(' '||RTRIM(PD10.DM_PRS_LST_SFX)) AS DM_PRS_LST
				/*legal address info _L */
				,CASE
					WHEN PD30_L.DC_DOM_ST <> ' ' AND SUBSTR(PD30_L.DF_ZIP_CDE,6,1) = ' '
		...
		(
			...
			FROM 
				...
				(
					...
					WHERE 
						COALESCE(TRIM(LC_FAT_REV_REA),'') = '' /*not reversed*/
						AND LC_STA_LON90 = 'A'
						AND PC_FAT_TYP = '10'
			...
			WHERE 
				...
				AND COALESCE(TRIM(LN90A.LC_FAT_REV_REA),'') = '' /*not reversed*/
	...
	FROM 
		...
		(
			SELECT DISTINCT 
				...
				,PD10.DM_PRS_MID
				,RTRIM(PD10.DM_PRS_LST)||RTRIM(' '||RTRIM(PD10.DM_PRS_LST_SFX)) AS DM_PRS_LST
			/*co-borrower billing address _B*/
				,CASE
					WHEN PD30_B.DC_DOM_ST_B <> ' ' AND SUBSTR(PD30_B.DF_ZIP_CDE_B,6,1) = ' '
					THEN SUBSTR(PD30_B.DF_ZIP_CDE_B,1,5)
					WHEN PD30_B.DC_DOM_ST_B <> ' ' AND SUBSTR(PD30_B.DF_ZIP_CDE_B,6,1) <> ' '
...
*uncomment for TESTING, comment for PROMOTION;
	...
	ENDRSUBMIT;
	DATA BLSTMNT; SET WORKLOCL.BLSTMNT; RUN;
	DATA COBRW_A; SET WORKLOCL.COBRW_A; RUN;
	DATA SV30; SET WORKLOCL.SV30; RUN;
...
%LN_BIL(BLSTMNT,19,TermsRemaining);
...
***************************************************************/
DATA BLSTMNT 
	(DROP = DM_PRS_1 DM_PRS_MID DM_PRS_LST DF_ZIP_CDE_L TermsRemaining
...
%MACRO LEFT_SHIFT;
	DATA BLSTMNT_DLQ (DROP=J I
		...
		TermsRemaining1 - TermsRemaining&COLUMN_COUNT
		) ;
	SET BLSTMNT_DLQ;
		*creates flexible number of new columns to hold old data;
		...
		ARRAY IN17 TermsRemaining1 - TermsRemaining&COLUMN_COUNT ;
		ARRAY OUT17 _TermsRemaining1 - _TermsRemaining&COLUMN_COUNT ;
		%DO A=1 %TO 17;
			J=1;
			DO I=1 TO &COLUMN_COUNT;
				IF IN&A.(I) NE ' ' 
		...
		SET BLSTMNT_DLQ;
			%DO I=1 %TO &COLUMN_COUNT;
				RENAME	
					...
					_TermsRemaining&I. = TermsRemaining&I. ;
			%END;
	RUN;
	DATA BLSTMNT_DLQ;
		SET BLSTMNT_DLQ;
		...
		PROC SQL NOPRINT;
			...
			(
				...
				FROM 
					BLSTMNT_TARGET TGT
				WHERE 
					DLQ.BF_SSN = TGT.BF_SSN
					AND DLQ.LN_SEQ&I. = TGT.LN_SEQ
			);
		QUIT;
	%END;
RUN;
...
PROC SQL NOPRINT;
	CREATE TABLE _BLSTMNT_BOR AS
		...
		FROM 
			BLSTMNT BLS
			LEFT JOIN BLSTMNT_DLQ DLQ
				ON BLS.BF_SSN = DLQ.BF_SSN
		WHERE 
			DLQ.BF_SSN IS NULL
	;
...
PROC SQL;
	...
	FROM 
		_BLSTMNT_BOR BOR
		LEFT JOIN PROGREVW.SPECIAL_MESSAGE MES
			ON 1=1
	WHERE
		MES.MESSAGE_ID = '1' /*message to borrower*/
	;
	...
	FROM 
		...
		LEFT JOIN PROGREVW.SPECIAL_MESSAGE MES
			ON 2=2
	WHERE
		MES.MESSAGE_ID = '2' /*message to endorser*/
	;
QUIT;
...
%MACRO CREATE_BILL_FILES(RPNO,CRIT);
	...
	DATA _NULL_;
		...
		FORMAT 	ACSKEY $18. 
		 		...
		 		DF_ZIP_CDE $18. 
		 		DM_FGN_CNY $15. 
		 		LD_LON_1_DSB1-LD_LON_1_DSB&MAX_LN MMDDYY10. 
		 		LN_SEQ1-LN_SEQ&MAX_LN 6. 
		 		IC_LON_PGM1-IC_LON_PGM&MAX_LN $6. 
		 		LC_LON_STA_BIL1-LC_LON_STA_BIL&MAX_LN $6. 
		 		LR_INT_BIL1-LR_INT_BIL&MAX_LN 7.3 
		 		LA_CUR_PRN_BIL1-LA_CUR_PRN_BIL&MAX_LN DOLLAR15.2 
		 		LA_BIL_PAS_DU1-LA_BIL_PAS_DU&MAX_LN DOLLAR15.2 
		 		LN_LTE_FEE1-LN_LTE_FEE&MAX_LN DOLLAR15.2 
		 		CUM_LTE_FEE_PD1-CUM_LTE_FEE_PD&MAX_LN DOLLAR15.2  /*cumulative late fees paid*/
		 		DAYS_DELQ1-DAYS_DELQ&MAX_LN BEST12. 
		 		TAPTP1-TAPTP&MAX_LN DOLLAR15.2 
		 		TAPTI1-TAPTI&MAX_LN DOLLAR15.2 
		 		TAGAP1-TAGAP&MAX_LN DOLLAR15.2 
		 		OPAFEL1-OPAFEL&MAX_LN DOLLAR15.2 
		 		LD_FAT_EFF MMDDYY10. 
		 		LA_FAT_CUR_PRI DOLLAR15.2 
		 		LA_FAT_NSI DOLLAR15.2 
		 		LA_FAT_LTE_FEE DOLLAR15.2 
		 		TAP DOLLAR15.2 
		 		LD_BIL_CRT MMDDYY10.  /*bill create date*/
		 		LN_SEQ_BIL_WI_DTE 6.  /*bill sequence*/
		 		LD_BIL_DU MMDDYY10. 
		 		LA_BIL_PAS_DU DOLLAR15.2 
		 		BOR_LTE_FEE DOLLAR15.2 
		 		LA_BIL_DU_PRT DOLLAR15.2 
		 		AMTDU DOLLAR15.2
		 		PX_MSG_BIL_ARA $79. 
		 		SCANLN $46. 
		 		CSTCNTR $6. 
		 		LF_EDS $9. 
		 		DAYS_DELQ BEST12. 
		 		TAPDFALILF DOLLAR15.2 
		 		AMTDUPLSELF DOLLAR15.2 
		 		IS_EDR BEST12. 
		 		EDR_SMADD BEST12.
		 		LATE_FEE_APP_DATE MMDDYY10. 
		 		EST_LATE_FEE DOLLAR15.2 
		 		CUM_LTE_FEE_PD DOLLAR15.2  /*cumulative late fees paid*/
		 		MESSAGE $255.  /*special message*/
		 		TermsRemaining 8. 
		;
	IF _N_ = 1 THEN 
...
%MACRO CREATE_BILL_FILES_EDR(RPNO,CRIT);
	DATA BILL_REP_DS;
		...
		*R27-39 processing;
		%DO I=1 %TO &MAX_LN;
			IF DI_VLD_ADR_endo = 'Y' /*at borrower level*/
			OR LC_LON_SND_CHC&I. = 'Y'; /*at bill level*/
		%END;
	RUN;
	PROC SORT DATA=BILL_REP_DS;
		BY CSTCNTR MSTRSRT STATE;
