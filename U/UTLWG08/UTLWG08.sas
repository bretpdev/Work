...
Report 2 lists loans guaranteed during the previous month for unaffilated schools.
Report 3 lists loans guaranteed during the previous month for out of state lenders
and unaffilated schools where the borrower does not have a Utah address, driver's
license, or SSN.
...
FILENAME REPORT2 "&RPTLIB/ULWG08.LWG08R2";
FILENAME REPORT3 "&RPTLIB/ULWG08.LWG08R3";
*/
...
FROM	OLWHRM1.GA10_LON_APP A INNER JOIN
		OLWHRM1.GA01_APP B ON
			...
			B.af_apl_ops_scl NOT IN ('02270800', '02270801', '02612200', '00367000', '00367600',
						 ...
						 '00522100', '00367901', '02572700', '02579000', '01116600', '88888800') FULL OUTER JOIN
...
%if &hasobs=1 %then
	%do;
		...
		TITLE2	"GUARANTEES FOR UNAFFILIATED SCHOOLS FOR THE MONTH OF &EFFDATE";
		FOOTNOTE	'JOB = UTLWG08     REPORT = ULWG08.LWG08R2';
		RUN;
	%end;
%else %if &hasobs=0 %then
	%do;
    	data _null_;
            file print notitles;
            put @01 'No records found for ULWG08.LWG08R2 -- UNAFFILIATED GUARANTEES';
        run;
    %end;
%mend rptprnt1;
%rptprnt1(worklocl.nonutguars);
/*PROC PRINTTO PRINT=REPORT3;
RUN;*/
OPTIONS PAGENO=1 LS=132;

%macro rptprnt2(dsname);
%let dsid=%sysfunc(open(&dsname));
%let hasobs=%sysfunc(attrn(&dsid,any));
%let rc=%sysfunc(close(&dsid));
%if &hasobs=1 %then
	%do;
		...
		FOOTNOTE	'JOB = UTLWG08     REPORT = ULWG08.LWG08R3';
		RUN;
	%end;
%else %if &hasobs=0 %then
	%do;
    	data _null_;
            file print notitles;
            put @01 'No records found for ULWG08.LWG08R3 -- UNAFFILIATED GUARANTEES WITH EDITS FOR UTAH LENDER AND UTAH ADDRESS, LICENSE, OR SSN';
        run;
    %end;
%mend rptprnt2;
%rptprnt2(worklocl.nonutguarsld);
