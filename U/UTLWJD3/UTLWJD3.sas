/*
LEGAL EMPLOYERS NOT RESPONDING
This report lists employers which have been served a judicial garnishment and are not
...
%LET RPTLIB = T:\SAS;
FILENAME REPORT2 "&RPTLIB/ULWJD3.LWJD3R2";
...
SELECT DISTINCT
	...
	RTRIM(B.IX_GEN_STR_ADR_1)							AS ADD1,/*<1>*/
	RTRIM(B.IX_GEN_STR_ADR_2)							AS ADD2,/*<1>*/
	RTRIM(B.IX_GEN_STR_ADR_3)							AS ADD3,/*<1>*/
	...
	F.la_clm_pri + F.la_clm_int	- F.la_pri_col +				/*<1>*/
	...
	F.la_leg_cst_acr - F.la_leg_cst_col +
		...
		G.la_clm_prj_col_cst							AS OTHER	/*</1>*/
FROM 	OLWHRM1.PD01_PDM_INF A INNER JOIN
		...
		OLWHRM1.DC02_BAL_INT G ON
			...
			G.AF_APL_ID_SFX = E.AF_APL_ID_SFX /*<1>AND
			G.LA_CLM_BAL > 100 */INNER JOIN
		OLWHRM1.DC11_LON_FAT P ON
			A.DF_PRS_ID = P.BF_SSN AND
			F.LD_LST_PAY = P.LD_TRX_EFF		
/*<1>ORDER BY D.BD_LEG_ACT_ATY_FIL*/
);
DISCONNECT FROM DB2;
ENDRSUBMIT;
DATA LENR;															/*<1>*/
...
FORMAT	LASTPAY MMDDYY10.
		FILED MMDDYY10.;
TITLE1 'LEGAL EMPLOYERS NOT RESPONDING';
TITLE2 'NO PAYMENT FOR 45 DAYS';
FOOTNOTE 'JOB = UTLWJD3     REPORT = ULWJD3.LWJD3R2';
RUN;
