...
FILENAME REPORT2 "&RPTLIB/ULWK24.LWK24R2";
FILENAME REPORT3 "&RPTLIB/ULWK24.LWK24R3";
FILENAME REPORT4 "&RPTLIB/ULWK24.LWK24R4";
FILENAME REPORTZ "&RPTLIB/ULWK24.LWK24RZ";
/*************************************************************************************
* NOTE: THE .txt FILE THAT THIS DATA STEP READS NEEDS TO FOLLOW THIS CONVENTION
* X1 VALUE, X2 VALUE, Y1 VALUE, Y2 VALUE, Z1 VALUE, Z2 VALUE 
* WHERE X1,X2 ARE VALUES FOR R2 - Y1,Y2 ARE VALUES FOR R3 AND Z1,Z2 ARE VALUES FOR R4
* A BREAKDOWN OF WHAT THESE VALUES REPRESENTS IS LISTED BELOW
* X1, Y1, Z1 - LAST REFERENCE CONTACT DATE FOR RESPECTIVE FILE
* X2, Y2, Z2 - LAST REFERENCE ATTEMPT DATE FOR RESPECTIVE FILE
**************************************************************************************/
DATA DATE_VALS;
/*LOCAL RUN*/
INFILE "Q:\Process Automation\RefCalDays\SUPER_REF_CALLS_DVAL.txt" DLM=',' MISSOVER DSD;
/*PRODUCTION RUN*/
/*INFILE "/sas/whse/progrevw/SUPER_REF_CALLS_DVAL.txt" DLM=',' MISSOVER DSD;*/
...
/*SET VALUES FOR REPORT2*/
    CALL SYMPUT('R2_DT1',INTNX('DAY',TODAY(),-X1,'BEGINNING'));
	CALL SYMPUT('R2_DT2',INTNX('DAY',TODAY(),-X2,'BEGINNING'));
/*SET VALUES FOR REPORT3*/
	CALL SYMPUT('R3_DT1',INTNX('DAY',TODAY(),-Y1,'BEGINNING'));
	CALL SYMPUT('R3_DT2',INTNX('DAY',TODAY(),-Y2,'BEGINNING'));
/*SET VALUES FOR REPORT4*/
	CALL SYMPUT('R4_DT1',INTNX('DAY',TODAY(),-Z1,'BEGINNING'));
	CALL SYMPUT('R4_DT2',INTNX('DAY',TODAY(),-Z2,'BEGINNING'));
RUN;
...
LEFT OUTER JOIN (
	...
	INNER JOIN OLWHRM1.GA14_LON_STA C
		ON GY.AF_APL_ID = C.AF_APL_ID
		AND GY.AF_APL_ID_SFX = C.AF_APL_ID_SFX
	WHERE 
		GY.AA_CUR_PRI > 0
		AND AC_LON_STA_TYP IN ('CR','DA','FB','IA','ID','IG','IM','RP','UA','UB','UC','UD','UI')
	) GA10
...
AND (
	(BR03.BI_DOM_PHN_VLD = 'Y' AND 
	 BR03.BN_RFR_DOM_PHN IS NOT NULL) 
	OR 
	(BR03.BI_ALT_PHN_VLD = 'Y' AND 
	 BR03.BN_RFR_ALT_PHN IS NOT NULL) 
	)
FOR READ ONLY WITH UR
);
...
FROM CONNECTION TO DB2 (
	...
	WHERE IF_WRK_GRP IN (
		'KCREFCLL','KDREFCLL','KAREFCLL',
		'REFRCAL1','REFRCAL2','REFRCALS'
		)
UNION
...
UNION
	...
	AND DAYS(LD_REQ_RSP_ATY_PRF) <> DAYS(CURRENT DATE) - 3
UNION
	SELECT DISTINCT BF_SSN AS DF_PRS_ID_TGT
		,'SP' AS EX_TYP
	FROM OLWHRM1.LN10_LON
	WHERE LI_CON_PAY_STP_PUR = 'Y'

FOR READ ONLY WITH UR
);
DISCONNECT FROM DB2;
/*%PUT  SQLXRC= >>> &SQLXRC <<< ||| SQLXMSG= >>> &SQLXMSG >>>  ;  * INCLUDES ERROR MESSAGES TO SAS LOG  ;*/
/*%SQLCHECK (SQLRPT=ULWK24.LWK24RZ);*/
/*QUIT;*/
...
AND B.DF_PRS_ID_BR NOT IN (
		...
		FROM FL1_A
	UNION
		SELECT DF_PRS_ID_TGT AS BF_SSN
		FROM EXC
		WHERE EX_TYP IN ('ST','CQ','CS','AC', 'SP')
	)
AND A.DF_PRS_ID_RFR NOT IN (
	SELECT DF_PRS_ID_TGT 
	FROM EXC
...
/********************************************************************
* CHECK FOR DUPS BETWEEN THE FILES
********************************************************************/
/*RSUBMIT;*/
/*PROC SQL;*/
/*CREATE TABLE DUPCHECK1 AS */
/*SELECT A.**/
/*FROM DUCE A*/
/*INNER JOIN TRIPLE B*/
/*	ON A.DF_PRS_ID_BR = B.DF_PRS_ID_BR*/
