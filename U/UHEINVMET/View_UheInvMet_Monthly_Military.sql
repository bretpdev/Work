...
/****** Object:  View [UheInvMet].[Monthly_Military]    Script Date: 11/5/2019 12:59:21 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [UheInvMet].[Monthly_Military]
...
FROM
	...
	(
		...
		WHERE
			LN72.LC_STA_LON72 = 'A'
			AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) BETWEEN LN72.LD_ITR_EFF_BEG AND LN72.LD_ITR_EFF_END /*last day prev month*/
	) LN72
	...
	(
		...
		FROM
			...
			INNER JOIN UDW..LN50_BR_DFR_APV LN50
				...
				AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) BETWEEN LN50.LD_DFR_BEG AND LN50.LD_DFR_END /*last day prev month*/
...
WHERE
	...
	(
		...
		(LN72.LR_ITR <= 6 AND LN72.LC_INT_RDC_PGM = 'M')
