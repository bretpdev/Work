...
(
	...
	WHERE
		...
		AND LN94.LC_RMT_BCH_SRC_IPT = 'E'--COMMENT OUT IF USING OPENQUERY
) NEWDATA
	ON NEWDATA.ArcTypeId = AAP.ArcTypeId
	AND NEWDATA.AccountNumber = AAP.AccountNumber
	AND NEWDATA.ARC = AAP.ARC
...
--TEST:
--select 'r2'[r2],* from ULS..ArcAddProcessing where comment like '%APPMT%'
--select 'r3'[r3],* from ULS..ArcAddProcessing where comment like '%APMIL%'
--select 'r4'[r4],* from ULS..ArcAddProcessing where comment like '%APAMC%'
--select 'r5'[r5],* from ULS..ArcAddProcessing where comment like '%APACH%'

--delete from ULS..ArcAddProcessing where ScriptId = 'UTLWR21'
