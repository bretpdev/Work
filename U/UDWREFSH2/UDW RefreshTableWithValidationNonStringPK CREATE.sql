...
/****** Object:  StoredProcedure [dbo].[RefreshTableWithValidationNonStringPK]    Script Date: 5/4/2020 8:17:12 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[RefreshTableWithValidationNonStringPK]

	@TableName VARCHAR(50) ,
	@OverrideDateColumn VARCHAR(50) ,
	@OverrideLocalTableName VARCHAR(50) = NULL,
	@SSNField varchar(50) = 'BF_SSN'
...
BEGIN
	...
	IF DB_NAME() = 'CDW'
	BEGIN
		SET @ServerName = 'LEGEND_TEST_VUK1'
	...
	ELSE IF @CountDifference != 0 AND @LoopCount = 0
		BEGIN
			...
			'
				...
				WHERE
					' + SUBSTRING(@TableName, 1,4) + '.' +  @SSNField + ' IS NULL' + '

				DELETE FROM #' + SUBSTRING(@TableName, 1,4) + '
				...
				FROM
					...
					INNER JOIN ' + @RemoteSchema + '.' + @TableName + '  ' + SUBSTRING(@TableName, 1,4) + ' ON ' + SUBSTRING(@TableName, 1,4) + '.' + @SSNField + ' = ' + SUBSTRING(@TableName, 1,2) + '.' + @SSNField + '
				''
