-- CUSTOM REFRESH TO ADD IN LEADING ZEROS TO DF_SPE_ID
USE UDW
GO
...
IF @CountDifference != 0 AND @LoopCount > 0
	BEGIN
		RAISERROR('OLWHRM1.PH05_CNC_EML - The remote and local record counts do not match.  The local count is off by %i records.  A full refresh of the table is requireL.', 16, 11, @CountDifference)
	END
