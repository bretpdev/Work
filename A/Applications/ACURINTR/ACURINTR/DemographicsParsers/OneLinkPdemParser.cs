...
{
    class OneLinkPdemParser : IDemographicsParser
    {
        private ReflectionInterface RI { get; set; }
        public OneLinkPdemParser(ReflectionInterface ri)
        {
            RI = ri;
        }
        ...
        public QueueTask Parse()
        {
            ...
            if (RI.CheckForText(13, 34, "CREDIT BUREAU"))
                pdem = new QueueTask(RejectAction.Sources.ONELINK_PENDING_PDEM, SystemCode.Sources.CREDIT_BUREAU);
            else
                pdem = new QueueTask(RejectAction.Sources.ONELINK_PENDING_PDEM, SystemCode.Sources.NON_CREDIT_BUREAU);
