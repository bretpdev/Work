...
{
	class AccurintParser : DemographicsParserBase
	{
        public AccurintParser(ReflectionInterface ri, DataAccess da) : base(ri, da) { }
		/// <summary>
		...
		{
			QueueTask task = new QueueTask(RejectAction.Sources.ACCURINT, SystemCode.Sources.ACCURINT, DA);
		...
		private AccurintRDemographics ParseAddress(string commentText)
		{
			...
			try
			{
				...
				while (commentIndex >= 0) //Pick out the zip code, state, and city from the comment text, working backwards from the end.
				{
					...
					else if (addressElement.Length < 2 || addressElement == "FC" || DA.StateCodes().Contains(addressElement))
