...
PROC SQL;
	...
	CREATE TABLE NEWLNS AS
		...
		WHERE LNXX.LD_LON_EFF_ADD >= &BEGIN
			AND LNXX.LD_LON_ACL_ADD >= &BEGIN
			AND (LNXX.LD_LON_EFF_ADD >= TRAN.LD_FAT_APL
...
DATA NEWLNS; SET LEGEND.NEWLNS; RUN;

PROC EXPORT	
		DATA=LEGEND.TRANSXXXX
		OUTFILE='T:\SAS\SSAE XX DMCS and Recalls.xlsx'
		REPLACE;
RUN;
