LIBNAME XL 'T:\Copy of October-XXXX Transfers.xlsx';

DATA GRANITE_STATE;
      SET XL.'GRANITE STATE$'N;
...
PROC SQL;
	CREATE TABLE POP AS
		...
		FROM
			...
			LEFT JOIN /*GETS THE ACTIVE REPAYMENT SCHEDULE*/
			(
				SELECT DISTINCT
					RSXX.BF_SSN,
					LNXX.LN_SEQ,
			...
			INNER JOIN /*GETS THE TOTAL PRINCIPAL BALANCE*/
			(
				SELECT DISTINCT
					LNXX.BF_SSN,
			...
			LEFT JOIN /*GETS BORROWERS IN STATUSES THAT WE WANT TO EXCLUDE*/
			(
				SELECT DISTINCT 
					BF_SSN,
					WC_DW_LON_STA
			...
			LEFT JOIN /*GETS THE MAX DELIQUENCY */
			(
				SELECT DISTINCT
					BF_SSN,
			...
			LEFT JOIN /*GETS BORROWERS WITH THE DLCON ARC */
			(
				SELECT DISTINCT
					BF_SSN
				FROM
...
DATA _NULL_;
	SET		EXLD;
	FILE
		'T:\Borrower Transfer Out - FED RX.txt'
		DELIMITER = ','
		DSD
		DROPOVER
		LRECL = XXXXX
