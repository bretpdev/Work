...
--select * from (
--select count(*) over (partition by p.bf_ssn, p.ln_seq) as c,  * from 
--(
SELECT DISTINCT
	...
	CASE WHEN LN10.LF_LON_CUR_OWN IN('830248','826717') THEN '20140227'
	     WHEN LN10.LF_LON_CUR_OWN LIKE '829769%' THEN '20150223'
		 ELSE '00000000'
	END AS LOAN_PURCHASE_DATE,
	...
	COALESCE(CONVERT(VARCHAR,LN09.LD_LON_RHB_PCV,112),'00000000') AS LOAN_REHAB_DATE,
	' ' AS FILLER
FROM
	...
	(
		SELECT
			PD22.DF_PRS_ID,
			MAX(PD22.DD_PRS_DSA_SPS_SR) AS DD_PRS_DSA_SPS_SR
		FROM 
...
WHERE
	(
		...
		and SUBSTRING(LN10.IF_GTR,4,3) IN ('706','709','741','744','745','747','751','927','951','725','729','731','733','736','740','748','749','755')
	)
	--and SUBSTRING(LN10.IF_GTR,4,3) not IN ('708', '712', '723', '734', '742','717')
