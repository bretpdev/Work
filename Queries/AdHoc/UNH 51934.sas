...
LEFT OUTER JOIN OLWHRM1.LN75_BIL_LON_FAT LN75
...
CREATE TABLE DF10 AS
	...
	WHERE (DATEPART(LN50.LF_LST_DTS_LN50) >= &LAST_RUN 
...
CREATE TABLE FB10 AS
	...
	WHERE (DATEPART(LN60.LF_LST_DTS_LN60) >= &LAST_RUN
...
/*		,IN01.IF_GEN_ZIP*/
/*		,IN01.IN_GEN_PHN*/
/*	FROM OLWHRM1.BR02_BR_EMP BR02*/
/*	INNER JOIN OLWHRM1.IN01_LGS_IDM_MST IN01*/
/*		ON BR02.BF_EMP_ID_1 = IN01.IF_IST*/
/*	WHERE DATEPART(BR02.BF_LST_DTS_BR02) >= &LAST_RUN;*/
/*QUIT;*/
/*%SSN2ACC(BR02,);*/
/**/
/*/**********************************************/
...
CREATE TABLE PD30 AS
	SELECT DISTINCT PD30.DF_PRS_ID AS BF_SSN
		,PD30.DX_STR_ADR_1
		,PD30.DX_STR_ADR_2
		,PD30.DM_CT
		,PD30.DC_DOM_ST
		...
		,PD30.DD_VER_ADR
		,PD30.DI_VLD_ADR
	FROM OLWHRM1.PD30_PRS_ADR PD30
	WHERE PD30.DC_ADR = 'L'
		AND DATEPART(PD30.DF_LST_DTS_PD30) >= &LAST_RUN;

CREATE TABLE PD42 AS
	SELECT DISTINCT PD42.DF_PRS_ID AS BF_SSN
		,PD42.DC_PHN
		,PD42.DC_ALW_ADL_PHN
		,PD42.DD_PHN_VER
		,PD42.DI_PHN_VLD
		...
		,PD42.DN_FGN_PHN_CT
		,PD42.DN_FGN_PHN_LCL
	FROM  OLWHRM1.PD42_PRS_PHN PD42
	WHERE DATEPART(PD42.DF_LST_DTS_PD42) >= &LAST_RUN;
...
/*	SELECT DISTINCT RM31.BF_SSN*/
/*		,SUM(RM31.LA_BR_RMT_PST) AS LA_BR_RMT_PST*/
/*	FROM OLWHRM1.RM31_BR_RMT_PST RM31*/
...
/*FROM OLWHRM1.LN56_LON_CRB_RPT LN56*/
/*LEFT OUTER JOIN OLWHRM1.LN56_LON_CRB_RPT LN56B*/
...
/*WHERE LN56.LD_RPT_CRB > INTNX('YEAR',TODAY(),-1,'S')*/
...
/*/* NOTE: ENTIRE TABLE IS PULLED DAILY*/*/
/**/
/*					FOR READ ONLY WITH UR*/
/*				)*/
/*	;*/
