...
FROM CONNECTION TO DBX (
	...
	FROM PKUB.LNXX_LON B
		...
		LEFT OUTER JOIN PKUB.LNXX_LON_DLQ_HST F
			...
			AND F.LC_DLQ_TYP = 'P'
		LEFT OUTER JOIN  PKUB.DWXX_DW_CLC_CLU G
...
DATA MSBTFF;
	SET MSBTFF;
	IF WC_DW_LON_STA IN ('XX','XX','XX','XX') AND DAYS_DELQ <= X THEN LS = 'XA';
	ELSE IF WC_DW_LON_STA IN ('XX','XX','XX','XX') AND DAYS_DELQ > X THEN LS = 'XB';
	ELSE IF WC_DW_LON_STA IN (
		'XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX'
		) THEN LS = 'IS';
	ELSE IF WC_DW_LON_STA = 'XX' AND LA_OTS_PRI_ELG < X THEN LS = 'CB';
...
PROC FORMAT;
	VALUE $LNSTA
	'XA' = 'In Repayment(X-X days delinquent)'
	'XB' = 'In Repayment(>X days delinquent)'
	'XX' = 'Grace'
	'XX' = 'In School'
	'XX' = 'In Deferment'
	'XX' = 'In Forbearance'
