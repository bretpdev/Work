LIBNAME XL 'T:\February XXXX Transfers.xlsx';

DATA SOURCE;
      SET XL.'CORNERSTONE$'N;
      BF_SSN = STRIP(COMPRESS(bf_ssn,'-'));
      FORMAT BF_SSN $X.;
      KEEP BF_SSN;
RUN;
		...
		FROM
			...
			JOIN 
				(
					SELECT DISTINCT
						LNXX.BF_SSN,
						SUM(LNXX.LA_CUR_PRI + coalesce(DWXX.LA_NSI_OTS,X) + coalesce(DWXX.LA_NSI_ACR, X)) AS LA_TOT_BAL
					FROM
						PKUB.LNXX_LON LNXX
						JOIN PKUB.DWXX_DW_CLC_CLU DWXX
							ON LNXX.BF_SSN = DWXX.BF_SSN
