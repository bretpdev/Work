...
/*							,CASE */
/*								WHEN PD30_CO.DI_VLD_ADR = 'Y' OR PH05_CO.DF_SPE_ID IS NOT NULL /*And the Address is valid (DI_VLD_ADR = 'Y') or (PH05.DI_CNC_ELT_OPI = 'Y' and DI_VLD_CNC_EML_ADR = 'Y'))*/
/*								THEN 'Y'*/
/*							 	ELSE 'N'*/
/*							 END AS CO_VLD_ADR*/
/**/
...
/*							AND PD30_CO.DC_ADR= 'L'*/
/*						LEFT JOIN OLWHRM1.PH01_SUPER_ID PH01_CO*/
/*							ON LN20.LF_EDS = PH01_CO.DF_PRS_ID*/
/*						LEFT JOIN OLWHRM1.PH05_CNC_EML PH05_CO /*or (PH05.DI_CNC_ELT_OPI = 'Y' and DI_VLD_CNC_EML_ADR = 'Y')*/
/*							ON PH01_CO.DF_SPE_ID = PH05_CO.DF_SPE_ID*/
/*							AND PH05_CO.DI_CNC_ELT_OPI = 'Y'*/
/*							AND PH05_CO.DI_VLD_CNC_EML_ADR = 'Y'*/
/**/
			...
			AND (
			 	(LN10.IC_LON_PGM IN 
					(
						...
						'UNSPC','PLUSGB','CNSLDN','SLS','TILP','COMPLT'
					)
			 	 AND (RS10.LD_SNT_RPD_DIS >= &CHAR_DAYS_2	
			 	 	  OR 
			 	  	  LN65.BF_SSN IN 
							(SELECT A.BF_SSN 						
							 FROM OLWHRM1.AY10_BR_LON_ATY A
...
/*data rpds_rs_a1_brw; set duster.rpds_rs_a1_brw; run;*/
/*data rpds_rs_a1_edr; set duster.rpds_rs_a1_edr; run;*/
DATA RPDS_RS_A1;
	SET RPDS_RS_A1_BRW RPDS_RS_A1_EDR;
RUN;

/*data rpds_is_a_brw; set duster.rpds_is_a_brw; run;*/
/*data rpds_is_a_edr; set duster.rpds_is_a_edr; run;*/
DATA RPDS_IS_A;
	SET RPDS_IS_A_BRW RPDS_IS_A_EDR;
RUN;

/*data rpds_ln1_brw; set duster.rpds_ln1_brw; run;*/
/*data rpds_ln1_edr; set duster.rpds_ln1_edr; run;*/
DATA RPDS_LN1;
	SET RPDS_LN1_BRW RPDS_LN1_EDR;
RUN;

/*data ay10_brw; set duster.ay10_brw; run;*/
/*data ay10_edr; set duster.ay10_edr; run;*/
DATA AY10;
...
CREATE TABLE RPDS_RS_A AS 
	SELECT DISTINCT RS.*
		,CASE 
			WHEN LN.IC_LON_PGM IN ('CONSOLIDATION') AND 
				LN.LD_SNT_RPD_DIS = LN.LD_LON_1_DSB AND
				LN.LD_SNT_RPD_DIS = &INT_DAYS_2
			THEN 'B' 
		 	ELSE 'A'
		 END AS STYPE
	FROM RPDS_RS_A1 RS
	...
	WHERE 
		(
			LN.IC_LON_PGM IN ('STAFFORD','PLUS','PLUSGB','SLS','COMPLT') AND 
			LN.LD_SNT_RPD_DIS = &INT_TODAY
		)
		OR 
		(
			LN.IC_LON_PGM IN ('CONSOLIDATION') AND 
			(
				LN.LD_SNT_RPD_DIS = LN.LD_LON_1_DSB AND
				LN.LD_SNT_RPD_DIS = &INT_DAYS_2
			)
...
/*			CO_DF_ZIP_CDE CO_DM_FGN_CNY BR_ACCT CO_DC_ADR CO_VLD_ADR); */
/*	SET RPDS_RS_A; */
/*RUN;*/
...
DATA RPDS_LN;
	SET RPDS_LN;
	RPS_UID = TRIM(LEFT(SSN))||' '||TRIM(LEFT(LN_RPS_SEQ));
RUN;
...
/*	FROM RPDS */
/*	INNER JOIN COBR */
/*		ON COBR.BF_SSN = RPDS.BF_SSN*/
/*		AND COBR.LN_RPS_SEQ = RPDS.LN_RPS_SEQ*/
/*;*/
/*QUIT;*/
