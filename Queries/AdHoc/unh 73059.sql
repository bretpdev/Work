...
SELECT
	...
	COUNT(*) AS LOAN_COUNT,
	SUM(ISNULL(LA_CLM_BAL,0.00) - ISNULL(LA_CLM_PRJ_COL_CST,0.00)) AS PRINCIPAL_INTERST
FROM
(
SELECT 
	...
	(
		...
		FROM
			...
			WHERE
					...
					AND ISNULL(DC01.LC_REA_CLM_ASN_DOE,'') in ('')
	) DC01
...
FROM
	...
	(
		...
		WHERE 
			...
			AND LD_TRX_EFF between '03/13/2020' AND  '05/24/2021'
			AND LC_REV_IND_TYP = ''
		GROUP BY
			AF_APL_ID, 
			AF_APL_ID_SFX
