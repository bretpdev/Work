...
INSERT INTO @TEMP (EmailCampaignID, AccountNumber, ActualFile, FirstName, LastName, EmailData, ArcNeeded, AddedBy, AddedAt, PDXXEmail, PHXXEmail)
	...
	FROM
		...
		(
			...
			WHERE
				LNXX.LC_STA_LONXX = 'A'
				AND CAST(LNXX.LD_FOR_BEG AS DATE) BETWEEN CAST(@Begin AS DATE) AND CAST(@End AS DATE)
...
--CampaignDataId	EmailCampaignId	Recipient	AccountNumber	FirstName	LastName	AddedAt	AddedBy	EmailSentAt	ArcProcessedAt	ArcAddProcessingId	InactivatedAt
