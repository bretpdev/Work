USE [CDW]
GO
/****** Object:  StoredProcedure [dbo].[LT_TSXXBTRTX_Loans]    Script Date: XX/XX/XXXX XX:XX:XX ******/
SET ANSI_NULLS ON
GO
...
IF @ROWS = X 
	BEGIN
		RAISERROR('[dbo].[LT_TSXXBTRTX_Loans] - No data returned for AccountNumber %s',XX,X, @AccountNumber)
	END
...
USE [CDW]
GO
/****** Object:  StoredProcedure [dbo].[LT_TSXXBAPIDR_Loans]    Script Date: XX/XX/XXXX XX:XX:XX ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
USE [CDW]
GO
/****** Object:  StoredProcedure [dbo].[LtXXMarkLetterPrinted]    Script Date: XX/XX/XXXX XX:XX:XX ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[LtXXMarkLetterPrinted]
	@LetterSeq int,
	@LetterId varchar(XX),
...
AS BEGIN
	...
	ELSE
		BEGIN
			RAISERROR ('Failed to retrieve dbo.DocumentDetails record for processing. LetterSeq: %i, LetterId: %s, AccountNumber: %s', -- error message
				   XX, -- Severity.
				   X, -- State.
				  	@LetterSeq,
					@LetterId,
...
SET
	LR.PrintedAt = NULL,
	LR.InactivatedAt = NULL,
	LR.SystemLetterExclusionReasonId = NULL
FROM 
