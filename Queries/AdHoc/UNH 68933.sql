...
(
	SELECT DISTINCT
		...
		M.MONTH_END,
		SUM(CASE
			WHEN EOMONTH(LD_DLQ_MAX) = '07/31/2020' THEN JUL.LA_CUR_PRI
			WHEN EOMONTH(LD_DLQ_MAX) = '08/31/2020' THEN AUG.LA_CUR_PRI
			ELSE LN10.LA_CUR_PRI
		END) AS BALANCE,
		MAX(CASE 
	...
	FROM
		...
		LEFT JOIN AuditUDW..LN10_LON_Jul2020 JUL
			ON JUL.BF_SSN = LN16.BF_SSN
			AND JUL.LN_SEQ = LN16.LN_SEQ
		LEFT JOIN AuditUDW..LN10_LON_Aug2020 AUG
			ON AUG.BF_SSN = LN16.BF_SSN
			AND AUG.LN_SEQ = LN16.LN_SEQ
		INNER JOIN @MONTHS M
...
(
	SELECT DISTINCT
			...
			FB10.LC_FOR_TYP,
			SUM(CASE
			WHEN  '07/31/2020'  BETWEEN LN60.LD_FOR_BEG AND LN60.LD_FOR_END THEN JUL.LA_CUR_PRI
			WHEN  '08/31/2020'  BETWEEN LN60.LD_FOR_BEG AND LN60.LD_FOR_END THEN AUG.LA_CUR_PRI
			ELSE LN10.LA_CUR_PRI
		END) OVER(PARTITION BY LN60.BF_SSN) AS BALANCE
		FROM
			...
			LEFT JOIN AuditUDW..LN10_LON_Jul2020 JUL
				ON JUL.BF_SSN = LN10.BF_SSN
				AND JUL.LN_SEQ = LN10.LN_SEQ
			LEFT JOIN AuditUDW..LN10_LON_Aug2020 AUG
				ON AUG.BF_SSN = LN10.BF_SSN
				AND AUG.LN_SEQ = LN10.LN_SEQ
		WHERE
			LN60.LC_STA_LON60 = 'A'
