LIBNAME XL 'T:\PSLF TransferX-XX-XXXX - Cornerstone.xls';

DATA WORK.SOURCE (DROP=Borrower_SSN);
	SET XL.'SheetX$'N;
...
PROC SQL;
	CREATE TABLE POP AS
		SELECT DISTINCT	
			...
			BAL.LA_TOT_BAL,
			CASE 
				WHEN DWXXS.WC_DW_LON_STA IN ('XX','XX') 
				THEN X
				ELSE X
			END AS IN_BNK,
			CASE 
			...
			CASE
				...
				THEN AYXX.PF_REQ_ACT
				ELSE ''
			END AS OTHER_SPCL,
			DELQ.LN_DLQ_MAX,
			CASE
				WHEN CONS.BF_SSN IS NOT NULL 
				THEN X
				ELSE X
			END AS IS_CONS
		FROM
...
DATA REASONS;
	SET POP;
	IF LA_TOT_BAL <= XX  THEN REASONX = 'Total Balance <= $XX';
	IF LN_DLQ_MAX >= XXX THEN REASONX = 'Active Delinquency >= XXX';
	IF IS_CONS = X 		 THEN REASONX = 'Consolidation in Process';
	IF IN_BNK = X 		 THEN REASONX = 'Specialty Claim/Bankruptcy';
	IF IN_TPD = X 		 THEN REASONX = 'Specialty Claim/TPD';
	IF IN_DTH = X 		 THEN REASONX = 'Specialty Claim/Death';
	IF OTHER_SPCL ^= ''  THEN REASONX = CATS('Specialty Claim/',OTHER_SPCL);
...
PROC EXPORT
	DATA=EXLD
	OUTFILE='T:\CNH XXXXX - PSLF Transfer - X-XX-XXXX.xlsx'
	REPLACE;
RUN;
