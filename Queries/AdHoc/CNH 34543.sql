...
FROM
	...
	INNER JOIN CDW..LNXX_INT_RTE_HST LNXX --Allow for S types as well as M's
...
FROM 
	...
	( 
		...
		WHERE
			BLXX.LD_BIL_DU BETWEEN CAST(DATEADD(D,-(DAY(DATEADD(M,-X,GETDATE()-X))),DATEADD(M,-X,GETDATE()-X)) AS DATE) /*first day prev month*/ AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) /*Last Day Prev Month*/
			AND BLXX.LC_IND_BIL_SNT != 'X' --Exclude paid ahead bills from counting
			AND LNXX.LD_BIL_DU_LON BETWEEN CAST(DATEADD(D,-(DAY(DATEADD(M,-X,GETDATE()-X))),DATEADD(M,-X,GETDATE()-X)) AS DATE) /*first day prev month*/ AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) /*Last Day Prev Month*/
					...
					CASE 
						...
						WHEN BD.WC_DW_LON_STA IN ('XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX','XX') THEN 
							CASE
								...
								WHEN BD.LN_DLQ_MAX = X AND BD.LC_CAM_LON_STA = 'XX' THEN 'XX'
								WHEN BD.LN_DLQ_MAX = X AND BD.LC_CAM_LON_STA = 'XX' THEN 'XX'
