	...
	(			
		...
		'		
			SELECT	
				...
				LNXX.LF_DOE_SCL_ORG,
				LNXX.LC_STA_LONXX,
				LNXX.LA_CUR_PRI + LNXX.LA_LTE_FEE_OTS "LA_CUR_PRI_PLUS_LA_LTE_FEE_OT",
			...
			WHERE
				LNXX.LC_STA_LONXX = ''R''
				AND
				LNXX.LA_CUR_PRI + LNXX.LA_LTE_FEE_OTS > X.XX
...
FROM
	(			
		SELECT
			PDXX.DF_PRS_ID [SSN], -- was requested after the _LegendData populating query was completed (which takes XX minutes or so to run) so just joined in local PDXX to obtain SSN	
			LD.*,
