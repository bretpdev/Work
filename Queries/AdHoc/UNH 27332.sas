...
LAST_RUN = 0;	*COMMENT FOR PROD, IT WILL READ THE DATE FROM A TABLE;
...
PROC SQL;
	...
	where b.bf_ssn in ('191581264','156880952','566596721')
;
QUIT;
...
/*QUIT; */
/*proc sort data=ln80; 
/*by bf_ssn ln_seq LD_BIL_CRT LN_SEQ_BIL_WI_DTE LN_BIL_OCC_SEQ LN_FAT_SEQ; run;*/*/
...
/*RUN;*/
/*PROC SORT DATA=LN83; 
/*BY BF_SSN LN_SEQ DESCENDING BN_EFT_SEQ; RUN;*/*/
/*DATA LN83;*/
/*SET LN83;*/
/*BY BF_SSN LN_SEQ;*/
/*IF FIRST.LN_SEQ;*/
...
/*RUN;*/
/*PROC SORT DATA=LN16; 
/*BY BF_SSN LN_SEQ DESCENDING LN_DLQ_SEQ; RUN;*/*/
/*DATA LN16 TILPCHECK;*/
...
CREATE TABLE TILP AS
	...
	and a.bf_ssn in ('191581264','156880952','566596721');
...
/*	get all OneLINK borrowers*/
	CREATE TABLE ALL_ONELINK AS
		...
		WHERE
			...
			and pd01.df_prs_id in ('191581264','156880952','566596721')
...
/*	get all COMPASS borrowers*/
	CREATE TABLE ALL_COMPASS AS
		...
		where pd10.df_prs_id in ('191581264','156880952','566596721')
...
/*	combine and sort COMPASS, OneLINK-only, and TILP borrowers*/
	CREATE TABLE CALLFWD AS
		...
		where bf_ssn in ('191581264','156880952','566596721')
		ORDER BY
			DF_SPE_ACC_ID,
			CLUID,
			LN_SEQ
...
/*RUN;*/
/*PROC SORT DATA=LN54; 
/*BY BF_SSN LN_SEQ LC_STA_LN54; RUN;*/*/
...
/*RUN;*/
/*PROC SORT DATA=LN55; 
/*BY BF_SSN LN_SEQ LC_STA_LN55; RUN;*/*/
/*DATA LN55;*/
/*	SET LN55;*/
...
/*RUN;*/
/*PROC SORT DATA=LN55; 
/*BY PM_BBS_PGM PN_BBS_PGM_SEQ;RUN;*/*/
/*PROC SORT DATA=RP02; 
/*BY PM_BBS_PGM PN_BBS_PGM_SEQ;RUN;*/*/
...
/*QUIT;*/
/*PROC SORT DATA=LN65; 
/*BY BF_SSN LN_SEQ LN_GRD_RPS_SEQ; RUN;*/*/
...
/*QUIT;*/
/*PROC SORT DATA=SD10; 
/*BY BF_SSN LN_SEQ LN_STU_SPR_SEQ;RUN;*/*/
...
/*RUN;*/
/*PROC SORT DATA=BR30; 
/*BY BF_SSN BC_EFT_STA DESCENDING BN_EFT_SEQ; RUN;*/*/
/*DATA BR30; */
/*SET BR30; */
...
CREATE TABLE PD10 AS
	...
	WHERE substr(df_prs_id,1,1) ^= 'P'
		...
		and a.df_prs_id in ('191581264','156880952','566596721')
;
...
CREATE TABLE PD30 AS
	...
	WHERE C.DC_ADR = 'L'
		...
		and c.df_prs_id in ('191581264','156880952','566596721');
...
CREATE TABLE PD42 AS
	...
	and d.df_prs_id in ('191581264','156880952','566596721');
	...
	CREATE TABLE PD32 AS
		...
		WHERE 
			...
			AND DC_STA_PD32 = 'A'
			and df_prs_id in ('191581264','156880952','566596721')
	;
QUIT;
			...
			CONNECTION TO DB2 
				(
					...
					WHERE
						...
						and pd10.df_prs_id in ('191581264','156880952','566596721')
					FOR READ ONLY WITH UR
				)
	;
