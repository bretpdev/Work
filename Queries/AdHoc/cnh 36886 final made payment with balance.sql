...
(
	SELECT
		...
		PMT.PMT_AMT,
		(LN10.LA_CUR_PRI + ISNULL(ln10.LA_NSI_OTS,0)) as BAL_BEFORE_PMT,
		((LN10.LA_CUR_PRI + ISNULL(ln10.LA_NSI_OTS,0))  - PMT.PMT_AMT) AS BAL_AFTER_PMT
	FROM
		CDW..[CNH_36886_Past Due] PD
		INNER JOIN CDW..CNH_36886_PAYMENT_run3 P
			ON PD.BF_SSN = P.BF_SSN
			AND PD.LN_SEQ = P.LN_SEQ
		INNER JOIN [AuditCDW]..[LN10_LON_Jan2019] LN10
			ON LN10.BF_SSN = PD.BF_SSN
			AND LN10.LN_SEQ = PD.LN_SEQ
