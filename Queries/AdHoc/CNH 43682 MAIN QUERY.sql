...
FROM
	...
	(
		SELECT DISTINCT
			cnh.[X digit SSN],
			MAX(LD_FAT_EFF) over(partition by lnXX.bf_ssn) AS TRANSFER_DATE
		FROM
			CDW..LNXX_FIN_ATY LNXX
			INNER JOIN CDW..CNHXXXXXZ CNH
		...
		ON TRANS.[X digit SSN] = CNH.[X digit SSN]
	LEFT JOIN
	(
		SELECT DISTINCT
			DD.Ssn,
	...
	LEFT JOIN cdw..CNHXXXXXCredit c
