...
DECLARE @ARC TABLE (ARC VARCHAR(X));
INSERT INTO @ARC (ARC) VALUES
('DICSK'),--note: this is not "DISCK"!!!"
('DIFCR')
;
--select * from @ARC
...
--SELECT DISTINCT * FROM LNXX_LON		A INNER JOIN @OPEID O ON O.OPEID = A.LF_DOE_SCL_ORG
--SELECT DISTINCT * FROM SDXX_STU_SPR	A INNER JOIN @OPEID O ON O.OPEID = A.LF_DOE_SCL_ENR_CUR

DROP TABLE IF EXISTS #BORROWERS;
SELECT * INTO #BORROWERS FROM (
...
----if borrowers show up in the above, then:
--SELECT DISTINCT
--	BOR.FirstName,
--	BOR.LastName,
--	BOR.SSN,
...
--		WHEN AYXX.PF_REQ_ACT IN ('CSFSA','DICSK') THEN 'PENDING'
...
--			AYXX_BR_LON_ATY
--		WHERE
--			PF_REQ_ACT IN ('CSDNY','CSFSA','ADCSH','DICSK')
--		GROUP BY
--			BF_SSN
--	)AYXXmax
...
--WHERE
--	BOR.LD_TRM_BEG BETWEEN CONVERT(DATE,'XXXXXXXX') AND CONVERT(DATE,'XXXXXXXX')
--	AND AYXX.PF_REQ_ACT IN ('CSDNY','CSFSA','ADCSH','DICSK')
--ORDER BY
--	BOR.SSN,
--	AYXX.LD_ATY_REQ_RCV
