LIBNAME XL 'T:\December-XXXX Transfer.xlsx';
...
PROC SQL;
	CREATE TABLE POP AS
		SELECT DISTINCT	
			...
			LNXX.LN_SEQ,
			LNXX.IC_LON_PGM,
			COALESCE(LNXX.LA_CUR_PRI,X) + COALESCE(DWXX.LA_NSI_OTS,X) + COALESCE(DWXX.LA_NSI_ACR,X) AS LA_LON_BAL,
			RPS.LC_TYP_SCH_DIS,
			BAL.LA_TOT_BAL,
			CASE 
		...
		FROM
			...
			JOIN 
				(
					SELECT DISTINCT
						LNXX.BF_SSN,
						SUM(COALESCE(LNXX.LA_CUR_PRI,X) + COALESCE(DWXX.LA_NSI_OTS,X) + COALESCE(DWXX.LA_NSI_ACR,X)) AS LA_TOT_BAL
					FROM
						PKUB.LNXX_LON LNXX
						JOIN PKUB.DWXX_DW_CLC_CLU DWXX
							ON LNXX.BF_SSN = DWXX.BF_SSN
