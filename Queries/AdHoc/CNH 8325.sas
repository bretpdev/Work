...
PROC IMPORT OUT= WORK.Arcs
            DATAFILE= "T:\NHXXXX_Arcs.xlsx" 
            DBMS=EXCEL REPLACE;
     RANGE="SheetX$"; 
     GETNAMES=YES;
     MIXED=NO;
...
PROC SQL;
	CREATE TABLE BDSX AS
					...
					SELECT DISTINCT
						...
						LNXX.LF_LON_CUR_OWN,
						LNXX.LD_DLQ_ITL_MIN,
						LNXX.LN_DLQ_MAX + X AS LN_DLQ_MAX,
					...
					FROM 
						...
						INNER JOIN PKUB.AYXX_BR_LON_ATY AYXX
							...
							AND AYXX.PF_REQ_ACT IN (SELECT * FROM ARCS)
							AND AYXX.AYXX.LD_ATY_REQ_RCV > INPUT('XX/XX/XXXX',MMDDYYXX.)
					WHERE DWXX.BF_SSN IS NULL
;
QUIT;
