...
SELECT DISTINCT
	...
	SPACE(19) + '|' + --Filler
	PD10.DF_PRS_ID + '|' + --Recip SSN
	LEFT(RTRIM(PD10.DM_PRS_1) + ' ' + RTRIM(PD10.DM_PRS_LST) + SPACE(34),35) + '|' + --Recip Name
	CASE WHEN LN20.LC_EDS_TYP = 'M' THEN 'ENDORSER  '
		 WHEN LN20.LC_EDS_TYP = 'S' THEN 'ENDORSER  '
		 ELSE '          ' END + '|' + --Recip Relation
	LN20.LC_EDS_TYP + ' ' + '|' + --Recip Relation Code
	CASE WHEN LN20.LC_EDS_TYP = 'M' THEN 'ENDORSER  '
		 WHEN LN20.LC_EDS_TYP = 'S' THEN 'ENDORSER  '
		 ELSE '          ' END + '|' + --Regards to
	LN20.LC_EDS_TYP + ' ' + '|' + --Regards to type
	LN20.BF_SSN + '|' + --Regards SSN
	...
	RIGHT('00' + CAST(ln16.CountLoans AS VARCHAR(2)),2) + '|' + --num loans
	RIGHT('0000' + CAST(LN16.LN_DLQ_MAX AS VARCHAR(4)),4) + '|' + --Days Delq
	RIGHT('0000000' + CAST(REPLACE(CAST(CurrentBill.TotalDue AS VARCHAR),'.','') AS VARCHAR(7)),7) + '|' + --Total due 0010050 = 100.50
	RIGHT('0000000000' + CAST(REPLACE(CAST(ln16.OutstandingBal AS VARCHAR),'.','') AS VARCHAR(10)),10) + '|' + --Outstanding Bal
	...
	isnull(SD10.LC_REA_SCL_SPR,'') + '|' + --Sep reason
...
FROM
	...
	(
		SELECT 
			LN16.BF_SSN,
			count(distinct ln16.ln_seq) as CountLoans,
	...
	(
		...
		ON MaxDisburseLoan.BF_SSN = LN10.BF_SSN
	LEFT JOIN
	(
		SELECT DISTINCT
			SD10.LF_STU_SSN,
		...
		FROM
			...
			(
				...
				FROM
					CDW..SD10_STU_SPR SD10
					INNER JOIN CDW..LN20_EDS LN20
						ON LN20.BF_SSN = SD10.LF_STU_SSN
						AND LN20.LC_STA_LON20 = 'A'
				GROUP BY
					SD10.LF_STU_SSN
			) SD10Max
	...
	LEFT JOIN CDW.CALC.IVRDATA CurrentBill
		ON CurrentBill.SSN = LN10.BF_SSN
	LEFT JOIN 
	(
		SELECT DISTINCT
			AY10.BF_SSN,
			MAX(LD_ATY_REQ_RCV) OVER(PARTITION BY AY10.BF_SSN) AS LD_ATY_REQ_RCV,
			MAX(CASE WHEN PF_RSP_ACT = 'CNTCT' THEN LD_ATY_REQ_RCV ELSE NULL END) OVER(PARTITION BY AY10.BF_SSN) AS LD_ATY_REQ_RCV_CNTCT
		FROM
		...
		ON DDPHN.BF_SSN = LN10.BF_SSN
	LEFT JOIN CDW..PD40_PRS_PHN PD40H 
		ON PD40H.DF_PRS_ID = LN20.LF_EDS
		AND PD40H.DC_PHN = 'H'
	LEFT JOIN CDW..PD40_PRS_PHN PD40A 
		ON PD40A.DF_PRS_ID = LN20.LF_EDS
		AND PD40A.DC_PHN = 'A'
	LEFT JOIN CDW..PD40_PRS_PHN PD40W 
		ON PD40W.DF_PRS_ID = LN20.LF_EDS
		AND PD40W.DC_PHN = 'W'
	...
	(
		...
		FROM
			...
			INNER JOIN CDW..LN20_EDS LN20
				ON LN20.BF_SSN = LN83.BF_SSN
				AND LN20.LN_SEQ = LN83.LN_SEQ
				AND LN20.LC_STA_LON20 = 'A'
		WHERE
			LC_STA_LN83 = 'A'
	...
	(
		...
		ON RecentForbDefer.BF_SSN = LN10.BF_SSN
