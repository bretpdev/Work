		...
		FROM	
			CONNECTION TO DBX 
				(
					...
					WHERE
						LNXX.PC_FAT_TYP = 'XX'
						AND (LNXX.PC_FAT_SUB_TYP = 'XX'  OR LNXX.PC_FAT_SUB_TYP = 'XX')
						AND DAYS(LNXX.LD_FAT_APP) > DAYS('XXXX-XX-XX')
...
PROC EXPORT DATA= WORK.DEMO 
            OUTFILE= "T:\SAS\NH XXXX - Another Audit Query Needed for Adjustments.CSV" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
