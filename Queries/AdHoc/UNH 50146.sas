...
DATA TestFile;
	INFILE 'T:\J00BX_8*' DLM="," DSD MISSOVER FIRSTOBS=2;
...
SELECT DISTINCT
	...
	CASE 
		WHEN 
			TestFile.DISQUALDATE = '000000' 
			AND TestFile.BBCODE IN('0290','0835','1560','1565','1717','0007','1716','1000','1520','4400','5101','6742','6749','1570','1580','6720','1706','0830','1715','2585','2590','6740','6741','6744','6745','6746','6747','1710','0006','1020','0230','0800','1010','1530','3330','3335','3340')
			AND TestFile.INTSTAT IN('00001','00002','00003','00007','00009','00012','00014','00017','00018')
			THEN 'Qualified' 
		WHEN
			TestFile.DISQUALDATE = '000000' 
			AND TestFile.BBCODE IN('2740','2741','6748','2204','2205','2206','2207','2208','2550','2552','2555','2560','2565','2214','2215','2216','2217','2530','2533','2535','2540','2570','2573','2575','2580','7722','7723','7724','7725')
			AND TestFile.REBATESTAT IN('00001','00002','00003','00005','00007','00009','00010','00012','00015','00016','00017','00018','00019','00021','00027')
			THEN 'Qualified'
		ELSE 'Disqualified' END AS CorrectedStatus,
	'01' AS Tier,
	...
	CASE WHEN TestFile.BBCODE = '0000' OR TestFile.BBCODE = '' OR TestFile.BBCODE IS NULL THEN 'EXCLUDE'
		 WHEN 
			TestFile.DISQUALDATE = '000000' 
			AND TestFile.BBCODE IN('0290','0835','1560','1565','1717','0007','1716','1000','1520','4400','5101','6742','6749','1570','1580','6720','1706','0830','1715','2585','2590','6740','6741','6744','6745','6746','6747','1710','0006','1020','0230','0800','1010','1530','3330','3335','3340')
			AND TestFile.INTSTAT IN('00000','00004','00005','00006','00008','00010','00011','00013','00015','00016','00019','00099')
			THEN 'EXCLUDE'
		 WHEN 
			TestFile.DISQUALDATE = '000000' 
			AND TestFile.BBCODE IN('2740','2741','6748','2204','2205','2206','2207','2208','2550','2552','2555','2560','2565','2214','2215','2216','2217','2530','2533','2535','2540','2570','2573','2575','2580','7722','7723','7724','7725')
			AND TestFile.REBATESTAT IN('00000','00004','00005','00006','00008','00011','00013','00014','00020','00022','00023','00024','00025','00026','00099')
			THEN 'EXCLUDE'
...
PROC SQL;
	CREATE TABLE R2 AS
		...
		FROM 
			MATCHES
		WHERE
			(R3Output='' AND R4Output='' AND BorrowerBenefitType <> '')
;
QUIT;
...
DATA _NULL_;
	SET MATCHES(drop=LN_SEQ where=(R3Output='R3' AND R4Output <> 'EXCLUDE' AND BorrowerBenefitType <> ''));
		RETAIN _SSN;
...
DATA _NULL_;
	SET MATCHES(drop=LN_SEQ where=(R3Output='' AND R4Output='R4' AND BorrowerBenefitType <> ''));
		RETAIN _SSN;
		IF (SSN eq _SSN) THEN delete;
		ELSE output;
		_SSN = SSN;
