...
DECLARE @SeparationDate DATE = CONVERT(DATE,'XXXXXXXX'); --select @SeparationDate
...
FROM
	(
		SELECT DISTINCT
			...
			LNXX.LA_CUR_PRI,
			LNXX.LN_SEQ,
			CASE WHEN CSRQX.BF_SSN IS NOT NULL THEN X ELSE X END AS HasCSRQX,
			CASE WHEN CSREQ.BF_SSN IS NOT NULL THEN X ELSE X END AS HasCSREQ,
			CASE WHEN CSDNY.BF_SSN IS NOT NULL THEN X ELSE X END AS HasCSDNY,
			CASE WHEN CSRQX.BF_SSN IS NOT NULL THEN CAST(CSRQX.LD_ATY_REQ_RCV AS DATE) ELSE NULL END AS CSRQX_CreateDate,
			CASE WHEN CSREQ.BF_SSN IS NOT NULL THEN CAST(CSREQ.LD_ATY_REQ_RCV AS DATE) ELSE NULL END AS CSREQ_CreateDate,
			CASE WHEN CSDNY.BF_SSN IS NOT NULL THEN CAST(CSDNY.LD_ATY_REQ_RCV AS DATE) ELSE NULL END AS CSDNY_CreateDate
		FROM
			PDXX_PRS_NME PDXX			
			INNER JOIN 
			(
			...
			(
				...
				FROM
					...
					(
						...
						FROM
							SDXX_STU_SPR
						WHERE
							CAST(ISNULL(LD_SCL_SPR, GETDATE()) AS DATE) >= @SeparationDate
						GROUP BY
							LF_STU_SSN,
							LF_DOE_SCL_ENR_CUR
					)SDXXMin
				...
				AND CAST(ISNULL(Separation.LD_SCL_SPR, GETDATE()) AS DATE) >= @SeparationDate
			LEFT JOIN
			(
				SELECT DISTINCT
					BF_SSN,
				...
				ON CSRQX.BF_SSN = PDXX.DF_PRS_ID
			LEFT JOIN
			(
				SELECT DISTINCT
					BF_SSN,
				...
				ON CSREQ.BF_SSN = PDXX.DF_PRS_ID
			LEFT JOIN
			(
				SELECT DISTINCT
					BF_SSN,
				...
				ON CSDNY.BF_SSN = PDXX.DF_PRS_ID
...
WHERE
	CAST(ISNULL(X.LastDayOfEnrollment, GETDATE()) AS DATE) >= @SeparationDate
	AND	(
