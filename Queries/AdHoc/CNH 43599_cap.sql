...
FROM
	...
	(
		SELECT 
			...
			FSXX.LF_FED_AWD,
			FSXX.LN_FED_AWD_SEQ,
			SUM(LNXX.LA_DSB - ISNULL(LNXX.LA_DSB_CAN,X) + LNXX.LA_PCV_ADJ_CUR_PRI ) AS DSB_AMT,
			SUM(LNXX.LA_DSB - ISNULL(LNXX.LA_DSB_CAN,X))  AS DSB_AMTX
...
SELECT distinct 
	d.disbursement_amount,
	 p.DISB_AMT,
	 p.DISB_AMT - d.disbursement_amount,
	--LNXX.LA_PCV_ADJ_CUR_PRI,
	D.*
FROM 
	#DATA D
	inner join CDW..PSAOH_RECON_PHEAA p
		on p.BF_SSN = d.SSN
		and p.AWARD_ID = d.AWARD_ID
	--LEFT JOIN CDW..LNXX_PCV_FIN_SCP LNXX  
	--			ON LNXX.BF_SSN = D.SSN
	--			AND LNXX.LN_SEQ = D.LN_SEQ
	--			AND LNXX.PC_FAT_TYP = 'XX' 
	--			AND LNXX.PC_FAT_SUB_TYP IN ('XX','XX') 
	--			AND LNXX.LC_STA_LONXX = 'A'
where
	d.disbursement_amount != p.DISB_AMT
