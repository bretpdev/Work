...
FROM
	...
	INNER JOIN CDW.[dbo].[TransferXDisbAmt] TP
		ON TP.[Disb Amt award] = FSXX.LF_FED_AWD + RIGHT('XXX' + CAST(FSXX.LN_FED_AWD_SEQ AS VARCHAR(XX)),X)
...
FROM 
	#DATA D
WHERE DISBURSEMENT_AMOUNT != [PH Disb Amt] 
order by rn