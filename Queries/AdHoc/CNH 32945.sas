LIBNAME XL 'T:\December -XXXX Transfers.xlsx';

DATA WORK.SOURCE (KEEP=BF_SSN /*TAB*/);
	LENGTH
		Borrower_SSN $XX.
		BF_SSN $X.
		TAB $ XX. 
		Borrower_First_Name $ XX. 
		Borrower_Last_name $ XXX.
		FX $ XX.;
	SET XL.'MOHELA$'N (IN=A)
		XL.'ESA$'N (IN=B)
		XL.'CORNERSTONE$'N (IN=C)
		XL.'GRANITE STATE$'N (IN=D)
		XL.'OSLA$'N (IN=E)
		XL.'SLMA$'N (IN=F)
		XL.'PHEAA$'N (IN=G)
		XL.'NELNET$'N (IN=H)
		XL.'GREAT LAKES$'N (IN=I)
	;
	BF_SSN = STRIP(COMPRESS(BORROWER_SSN,'-'));
	
	IF A=X THEN TAB='MOHELA';
	IF B=X THEN TAB='ESA';
	IF C=X THEN TAB='CORNERSTONE';
	IF D=X THEN TAB='GRANITE STATE';
	IF E=X THEN TAB='OSLA';
	IF F=X THEN TAB='SLMA';
	IF G=X THEN TAB='PHEAA';
	IF H=X THEN TAB='NELNET';
	IF I=X THEN TAB='GREAT LAKES';
RUN;
...
PROC EXPORT
		DATA=EXLD
		OUTFILE='T:\CNH XXXXX - Split Loan Transfer - December XXXX.xlsx'
		REPLACE;
RUN;
