/* DISCLOSURE PRINT JOB
This job is scheduled to run as a predecessor to UTLWG01, the daily
guaranty reporting job, because it references the lookup dataset
/uheaa/whse/olrp_lookup_directory/UTLWG02_RUNDT maintained by UTLWG01.

Other similar jobs, such as UTLWG12, have since been designed to run
as a successor to UTLWG01, using the PRV_RUNDT value in the lookup file.

This job creates the following output files:
ULWO07.LWO07R2 = All Stafford loans with valid addresses
ULWO07.LWO07R3 = All Staffords with invalid address
ULWO07.LWO07R4 = All Westminster Staffords, regardless of address validity
ULWO07.LWO07R5 = All Plus loans with valid address
ULWO07.LWO07R6 = All Plus loans with invalid address
ULWO07.LWO07R7 = Westminster Plus loans regardless of valid address
ULWO07.LWO07R8 = Paper Report of all invalid addresses
ULWO07.LWO07R9 = Paper Report of daily disclosure totals
ULWO07.LWO07R10 = UCMT Stafford loans regardless of valid address
ULWO07.LWO07R11 = UCMT Plus loans regardless of valid address
*/

/*LIBNAME DLGSUTWH DB2 DATABASE=DLGSUTWH OWNER=OLWHRM1;
%LET RPTLIB = %SYSGET(reportdir);
FILENAME REPORT2 "&RPTLIB/ULWO07.LWO07R2";
FILENAME REPORT3 "&RPTLIB/ULWO07.LWO07R3";
FILENAME REPORT4 "&RPTLIB/ULWO07.LWO07R4";
FILENAME REPORT5 "&RPTLIB/ULWO07.LWO07R5";
FILENAME REPORT6 "&RPTLIB/ULWO07.LWO07R6";
FILENAME REPORT7 "&RPTLIB/ULWO07.LWO07R7";
FILENAME REPORT8 "&RPTLIB/ULWO07.LWO07R8";
FILENAME REPORT9 "&RPTLIB/ULWO07.LWO07R9";
FILENAME REPORT10 "&RPTLIB/ULWO07.LWO07R10";
FILENAME REPORT11 "&RPTLIB/ULWO07.LWO07R11";
*/

OPTIONS SYMBOLGEN;
*CALCULATE JULY 1 OF CURRENT YEAR;
DATA _NULL_;
CALL SYMPUT('JUL',"'07/01/"||TRIM(LEFT(YEAR(DATE())))||"'");
RUN;
...
/*ACTIVE FIRST DISB IS >= JUL1 
OR (ACTIVE FIRST DISB IS >= 4/7/03 (FIRST DAY OF GTY SIDE DISCLOSURE)
	AND A HISTORY FIRST DISB ROW IS >= JUL1)*/
AND ((D1.AD_DSB_ADJ >= &JUL)
	OR 
		(D1.AD_DSB_ADJ >= '04/07/2003'
		AND EXISTS
			(SELECT *
			FROM OLWHRM1.GA11_LON_DSB_ATY XYZ
			WHERE XYZ.AF_APL_ID = B.AF_APL_ID
			AND XYZ.AF_APL_ID_SFX = B.AF_APL_ID_SFX
			/*AND XYZ.AC_DSB_ADJ_STA = 'I'*/ 	/*INACTIVE FIRST DISB ROW WAS AFTER JUL 1,*/
			AND XYZ.AN_DSB_SEQ = 1			/*PREVENTING INITIAL DISCLOSURE*/
			AND XYZ.AD_DSB_ADJ >= &JUL
			)
	))
ORDER BY A.AF_CUR_APL_OPS_LDR, A.DF_PRS_ID_BR, B.AF_APL_ID||B.AF_APL_ID_SFX
);

/****************IDENTIFICATION OF ALL PREVIOUSLY DISCLOSED LOANS**************************/
...
DATA DEMO1 (KEEP=CLUID1)
DEMO2 (WHERE=(CLUID2 NE ' ') KEEP=CLUID2) ;
SET DEMO;
...
/******************EXCLUSION OF PREVIOUSLY DISCLOSED LOANS*****************************/
PROC SORT DATA=DISC;
BY CLUID;
RUN;
PROC SORT DATA=DEMO;
...
DATA DISC;
MERGE DISC (IN=A) DEMO (IN=B);
BY CLUID;
IF A AND NOT B;
RUN;

/********IDENTIFICATION OF LAST NIGHT'S GUARANTEES, WHICH WILL DISCLOSE TODAY************/
*rsubmit;
LIBNAME OLRPLD V8 '/uheaa/whse/olrp_lookup_directory';	*CYPRUS LIVE;

*READ previous RUN DATE FOR THIS JOB FROM CYPRUS DATASET;
DATA _NULL_;
...
AND B.ad_prc > &PVS_DT
AND	(
		DAYS(B.af_lst_dts_ga10) = DAYS(CURRENT DATE)	
	OR
		(DAYS(B.af_lst_dts_ga10) = DAYS(CURRENT DATE) - 1 
		AND HOUR(B.af_lst_dts_ga10) > 22) 
	)
);

/********EXCLUSION OF LAST NIGHT'S GUARANTEES, WHICH WILL DISCLOSE TODAY************/
...
%STAF(DISCGL, 'C:\Windows\Temp\ULWO07.LWO07R2', %STR(WHERE DI_VLD_ADR = "Y"));
*All Stafford Loans with invalid address;
%STAF(DISCGL, 'C:\Windows\Temp\ULWO07.LWO07R3', %STR(WHERE DI_VLD_ADR = "N"));
*All Westminster loans;
%STAF(DISCGL, 'C:\Windows\Temp\ULWO07.LWO07R4', %STR(WHERE SCL_ID = '00368100'));
PROC SORT DATA=DISCGL;
BY SCL_ID SSN;
RUN;
*All UCMT loans;
%STAF(DISCGL, 'C:\Windows\Temp\ULWO07.LWO07R10',
	%STR(WHERE SCL_ID IN('03030600','03030601','03030603','03030604','03030605')));
...
%PLUS(DISCPL, 'C:\Windows\Temp\ULWO07.LWO07R5', %STR(WHERE DI_VLD_ADR = "Y"));
*All Plus loans with invalid address;
%PLUS(DISCPL, 'C:\Windows\Temp\ULWO07.LWO07R6', %STR(WHERE DI_VLD_ADR = "N"));
*Westminster Plus loans regardless of valid address;
%PLUS(DISCPL, 'C:\Windows\Temp\ULWO07.LWO07R7', %STR(WHERE SCL_ID = '00368100'));
PROC SORT DATA=DISCPL;
BY SCL_ID SSN;
RUN;
*UCMT loans regardless of valid address;
%PLUS(DISCPL, 'C:\Windows\Temp\ULWO07.LWO07R11',
	%STR(WHERE SCL_ID IN('03030600','03030601','03030603','03030604','03030605')));
...
FOOTNOTE  'JOB = UTLWO07     REPORT = ULWO07.LWO07R8';
RUN;
...
TITLE 'DISCLOSURES FOR LOANS WITH NEW INTEREST RATES - TOTALS';
TITLE2 "DATE: &RUNDATE";
FOOTNOTE  'JOB = UTLWO07     REPORT = ULWO07.LWO07R9';
