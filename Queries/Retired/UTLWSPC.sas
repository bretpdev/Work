...
*LIBNAME  WORK  REMOTE  SERVER=CYPRUS  SLIBREF=WORK;
*RSUBMIT;
DATA _NULL_;
...
PROC SORT DATA=WORK.SPOU1;
BY CLUID;
RUN;

PROC SORT DATA=WORK.SPOU2;
BY CLUID;
RUN;

DATA WORK.SPOU;
MERGE WORK.SPOU1 WORK.SPOU2;
BY CLUID;
RUN;

DATA WORK.SPOU (DROP = BLAST BFIRST BMI ELAST EFIRST EMI);
SET WORK.SPOU;
BNAME = TRIM(BLAST)||', '||TRIM(BFIRST)||' '||BMI;
ENAME = TRIM(ELAST)||', '||TRIM(EFIRST)||' '||EMI;
RUN;

PROC SORT DATA = WORK.SPOU;
