/*-------------------------------------------------------------------------------------* 
UTLWO01 - DISCLOSURE PRINT JOB
This job is scheduled to run as a successor to UTLWG01, the daily
guaranty reporting job, because it references the lookup dataset
/sas/whse/olrp_lookup_directory/UTLWG02_RUNDT maintained by UTLWG01.
*--------------------------------------------------------------------------------------*
IMPORTANT: THERE ARE 2 CODE BLOCKS THAT NEED TO BE COMMENTED OR UNCOMMENTED DEPENDING
ON IF THIS JOB IS BEING RUN IN LIVE OR TEST. THESE CODE BLOCKS ASSIGN THE CORRECT 
PSEDUO REG CAT TABLE DEPENDING ON RUN ENVIORNMENT.
*--------------------------------------------------------------------------------------*
This job creates the following output files:
ULWO01.LWO01RZ = SQL error report
ULWO01.LWO01R2 = All Stafford loans with valid domestic addresses (Not UHEAA)
ULWO01.LWO01R3 = All Staffords with invalid address (Not UHEAA)
ULWO01.LWO01R4 = All Westminster Staffords, regardless of address validity
ULWO01.LWO01R5 = All Plus and Grad PLUS loans with valid domestic addresses (Not UHEAA)
ULWO01.LWO01R6 = All Plus and Grad PLUS loans with invalid address (Not UHEAA)
ULWO01.LWO01R7 = Westminster Plus and Grad PLUS loans regardless of valid address
ULWO01.LWO01R8 = Paper Report of all invalid addresses
ULWO01.LWO01R9 = Paper Report of daily disclosure totals
ULWO01.LWO01R10 = UCMT Stafford loans regardless of valid address
ULWO01.LWO01R11 = UCMT Plus and Grad PLUS loans regardless of valid address
ULWO01.LWO01R12 = All Stafford loans with valid foreign addresses (Not UHEAA)
ULWO01.LWO01R13 = All Plus and Grad PLUS loans with valid foreign addresses (Not UHEAA)
ULWO01.LWO01R14 = All Stafford loans with valid domestic addresses (UHEAA)
ULWO01.LWO01R15 = All Staffords with invalid address (UHEAA)
ULWO01.LWO01R16 = All Plus and Grad PLUS loans with valid domestic addresses (UHEAA)
ULWO01.LWO01R17 = All Plus and Grad PLUS loans with invalid address (UHEAA)
ULWO01.LWO01R18 = All Stafford loans with valid foreign addresses (UHEAA)
ULWO01.LWO01R19 = All Plus and Grad PLUS loans with valid foreign addresses (UHEAA)
*-------------------------------------------------------------------------------------*/
LIBNAME DLGSUTWH DB2 DATABASE=DLGSUTWH OWNER=OLWHRM1;
%LET RPTLIB = %SYSGET(reportdir);
/*%LET RPTLIB = T:\SAS;*/
FILENAME REPORTZ "&RPTLIB/ULWO01.LWO01RZ";
FILENAME REPORT2 "&RPTLIB/ULWO01.LWO01R2";
FILENAME REPORT3 "&RPTLIB/ULWO01.LWO01R3";
FILENAME REPORT4 "&RPTLIB/ULWO01.LWO01R4";
FILENAME REPORT5 "&RPTLIB/ULWO01.LWO01R5";
FILENAME REPORT6 "&RPTLIB/ULWO01.LWO01R6";
FILENAME REPORT7 "&RPTLIB/ULWO01.LWO01R7";
FILENAME REPORT8 "&RPTLIB/ULWO01.LWO01R8";
FILENAME REPORT9 "&RPTLIB/ULWO01.LWO01R9";
FILENAME REPORT10 "&RPTLIB/ULWO01.LWO01R10";
FILENAME REPORT11 "&RPTLIB/ULWO01.LWO01R11";
FILENAME REPORT12 "&RPTLIB/ULWO01.LWO01R12";
FILENAME REPORT13 "&RPTLIB/ULWO01.LWO01R13";
FILENAME REPORT14 "&RPTLIB/ULWO01.LWO01R14";
FILENAME REPORT15 "&RPTLIB/ULWO01.LWO01R15";
FILENAME REPORT16 "&RPTLIB/ULWO01.LWO01R16";
FILENAME REPORT17 "&RPTLIB/ULWO01.LWO01R17";
FILENAME REPORT18 "&RPTLIB/ULWO01.LWO01R18";
FILENAME REPORT19 "&RPTLIB/ULWO01.LWO01R19";
FILENAME REPORT20 "&RPTLIB/ULWO01.LWO01R20";
/*LIBNAME  WORKLOCL  REMOTE  SERVER=CYPRUS  SLIBREF=WORK  ;*/
/*RSUBMIT;*/
/********************************************************************************** 
* START DISCLOSURE PROCESSING - READ LAST RUN DATE FOR UTLWG01 FROM DUSTER DATASET
***********************************************************************************/
LIBNAME OLRPLD V8 '/sas/whse/olrp_lookup_directory';	
DATA _NULL_;
SET OLRPLD.UTLWG02_RUNDT;
CALL SYMPUT('PVS_DT_DATE',"'"||PUT(DATEPART(PVS_RUNDT),MMDDYY10.)||"'");
CALL SYMPUT('PVS_DT_TIME',"'"||PUT(TIMEPART(PVS_RUNDT),TOD.)||"'");
RUN;
OPTIONS SYMBOLGEN NOCENTER NODATE NONUMBER LS=132;
%MACRO SQLCHECK ;
  %IF  &SQLXRC NE 0  %THEN  %DO  ;
    DATA _NULL_  ;
            FILE REPORTZ NOTITLES  ;
            PUT @01 " ********************************************************************* "
              / @01 " ****  THE SQL CODE ABOVE HAS EXPERIENCED AN ERROR.               **** "
              / @01 " ****  THE SAS SHOULD BE REVIEWED.                                **** "       
              / @01 " ********************************************************************* "
              / @01 " ****  THE SQL ERROR CODE IS  &SQLXRC  AND THE SQL ERROR MESSAGE  **** "
              / @01 " ****  &SQLXMSG   **** "
              / @01 " ********************************************************************* "
            ;
         RUN  ;
  %END  ;
%MEND  ;
PROC SQL STIMER;
CONNECT TO DB2 (DATABASE=DLGSUTWH);
CREATE TABLE DISCA AS
SELECT *
FROM CONNECTION TO DB2 (
SELECT	A.DF_PRS_ID_BR
	,C.DF_SPE_ACC_ID
	,INTEGER(A.DF_PRS_ID_BR)	AS SSN
	,B.AF_APL_ID
	,B.AF_APL_ID||B.AF_APL_ID_SFX	AS CLUID
	,B.AD_PRC
	,B.AC_LON_TYP
	,CASE WHEN RTRIM(C.DM_PRS_MID) <> ' ' 
		THEN RTRIM(C.DM_PRS_1)||' '||RTRIM(C.DM_PRS_MID)||' '||RTRIM(C.DM_PRS_LST)
		WHEN RTRIM(C.DM_PRS_MID) = ' ' 
		THEN RTRIM(C.DM_PRS_1)||' '||RTRIM(C.DM_PRS_LST)
	END						AS BWRNAME
	,C.DX_STR_ADR_1
	,C.DX_STR_ADR_2
	,C.DM_CT
	,CASE 
		WHEN C.DC_DOM_ST = 'FC' THEN ''
		ELSE C.DC_DOM_ST 
	 END					AS DC_DOM_ST
	,C.DF_ZIP
	,C.DM_FGN_CNY
	,C.DC_ADR
	,C.DI_VLD_ADR
	,A.DF_PRS_ID_STU
	,CASE WHEN RTRIM(G.DM_PRS_MID) <> ' ' 
		THEN RTRIM(G.DM_PRS_1)||' '||RTRIM(G.DM_PRS_MID)||' '||RTRIM(G.DM_PRS_LST)
		WHEN RTRIM(G.DM_PRS_MID) = ' ' 
		THEN RTRIM(G.DM_PRS_1)||' '||RTRIM(G.DM_PRS_LST)
	END						AS STUNAME
	,A.AF_CUR_APL_OPS_LDR 	AS LDR_ID
	,D.IM_IST_FUL 			AS LDR_NAME
	,D.IX_GEN_STR_ADR_1 	AS LDR_ADR_1
	,D.IX_GEN_STR_ADR_2		AS LDR_ADR_2
	,D.IX_GEN_STR_ADR_3		AS LDR_ADR_3
	,D.IM_GEN_CT			AS LDR_CT
	,D.IC_GEN_ST			AS LDR_ST
	,D.IF_GEN_ZIP			AS LDR_ZIP
	,A.AF_APL_OPS_SCL		AS SCL_ID
	,E.IM_IST_FUL			AS SCL_NAME
	,E.IX_GEN_STR_ADR_1		AS SCL_ADR1
	,E.IX_GEN_STR_ADR_2		AS SCL_ADR2
	,E.IX_GEN_STR_ADR_3		AS SCL_ADR3
	,E.IM_GEN_CT			AS SCL_CITY
	,E.IC_GEN_ST			AS SCL_ST
	,E.IF_GEN_ZIP			AS SCL_ZIP
	,A.AD_XPC_GRD
	,B.AA_GTE_LON_AMT 
	- COALESCE(B.AA_TOT_CAN,0) 
	- B.AA_TOT_RFD 			AS NETGTYAMT
	,COALESCE(D1.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D2.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D3.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D4.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D5.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D6.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D7.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D8.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D9.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D10.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D11.AA_ORG_FEE_ADJ,0)
	+ COALESCE(D12.AA_ORG_FEE_ADJ,0)	AS AA_ORG_TOT
	,COALESCE(D1.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D2.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D3.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D4.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D5.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D6.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D7.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D8.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D9.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D10.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D11.AA_GTE_FEE_ADJ,0)
	+ COALESCE(D12.AA_GTE_FEE_ADJ,0)	AS AA_GTE_TOT
	,B.AA_GTE_LON_AMT 
	- COALESCE(B.AA_TOT_CAN,0) 
	- B.AA_TOT_RFD
	- COALESCE(D1.AA_ORG_FEE_ADJ,0)
	- COALESCE(D2.AA_ORG_FEE_ADJ,0)
	- COALESCE(D3.AA_ORG_FEE_ADJ,0)
	- COALESCE(D4.AA_ORG_FEE_ADJ,0)
	- COALESCE(D5.AA_ORG_FEE_ADJ,0)
	- COALESCE(D6.AA_ORG_FEE_ADJ,0)
	- COALESCE(D7.AA_ORG_FEE_ADJ,0)
	- COALESCE(D8.AA_ORG_FEE_ADJ,0)
	- COALESCE(D9.AA_ORG_FEE_ADJ,0)
	- COALESCE(D10.AA_ORG_FEE_ADJ,0)
	- COALESCE(D11.AA_ORG_FEE_ADJ,0)
	- COALESCE(D12.AA_ORG_FEE_ADJ,0)	
	- COALESCE(D1.AA_GTE_FEE_ADJ,0)
	- COALESCE(D2.AA_GTE_FEE_ADJ,0)
	- COALESCE(D3.AA_GTE_FEE_ADJ,0)
	- COALESCE(D4.AA_GTE_FEE_ADJ,0)
	- COALESCE(D5.AA_GTE_FEE_ADJ,0)
	- COALESCE(D6.AA_GTE_FEE_ADJ,0)
	- COALESCE(D7.AA_GTE_FEE_ADJ,0)
	- COALESCE(D8.AA_GTE_FEE_ADJ,0)
	- COALESCE(D9.AA_GTE_FEE_ADJ,0)
	- COALESCE(D10.AA_GTE_FEE_ADJ,0)
	- COALESCE(D11.AA_GTE_FEE_ADJ,0)
	- COALESCE(D12.AA_GTE_FEE_ADJ,0)	AS NETLONAMT
	,CASE B.AC_LON_TYP 
		WHEN 'SF' THEN INTEGER(A.AX_SCL_SUB_CER_IAA )
		WHEN 'SU' THEN INTEGER(A.AX_SCL_USB_CER_IAA)
		WHEN 'PL' THEN INTEGER(A.AX_SCL_PS_CER_IAA)
		WHEN 'GB' THEN INTEGER(A.AX_SCL_PS_CER_IAA)
		ELSE 0
	END									AS SCL_CERT_AMT
	,A.AD_IST_TRM_BEG
	,A.AD_IST_TRM_END
	,F.NETGTYSUM
/**** DISBURSEMENT INFO ****/
	,D1.AD_DSB_ADJ			AS AD_DSB_ADJ1
	,D1.AA_DSB_ADJ			AS AA_DSB_ADJ1
	,D1.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ1
	,D1.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ1
	,D1.AA_DSB_ADJ
	- D1.AA_ORG_FEE_ADJ
	- D1.AA_GTE_FEE_ADJ		AS NETDSB1
	,D2.AD_DSB_ADJ			AS AD_DSB_ADJ2
	,D2.AA_DSB_ADJ			AS AA_DSB_ADJ2
	,D2.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ2
	,D2.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ2
	,D2.AA_DSB_ADJ
	- D2.AA_ORG_FEE_ADJ
	- D2.AA_GTE_FEE_ADJ		AS NETDSB2
	,D3.AD_DSB_ADJ			AS AD_DSB_ADJ3
	,D3.AA_DSB_ADJ			AS AA_DSB_ADJ3
	,D3.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ3
	,D3.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ3
	,D3.AA_DSB_ADJ
	- D3.AA_ORG_FEE_ADJ
	- D3.AA_GTE_FEE_ADJ		AS NETDSB3
	,D4.AD_DSB_ADJ			AS AD_DSB_ADJ4
	,D4.AA_DSB_ADJ			AS AA_DSB_ADJ4
	,D4.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ4
	,D4.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ4
	,D4.AA_DSB_ADJ
	- D4.AA_ORG_FEE_ADJ
	- D4.AA_GTE_FEE_ADJ		AS NETDSB4
	,D5.AD_DSB_ADJ			AS AD_DSB_ADJ5
	,D5.AA_DSB_ADJ			AS AA_DSB_ADJ5
	,D5.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ5
	,D5.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ5
	,D5.AA_DSB_ADJ
	- D5.AA_ORG_FEE_ADJ
	- D5.AA_GTE_FEE_ADJ		AS NETDSB5
	,D6.AD_DSB_ADJ			AS AD_DSB_ADJ6
	,D6.AA_DSB_ADJ			AS AA_DSB_ADJ6
	,D6.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ6
	,D6.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ6
	,D6.AA_DSB_ADJ
	- D6.AA_ORG_FEE_ADJ
	- D6.AA_GTE_FEE_ADJ		AS NETDSB6
	,D7.AD_DSB_ADJ			AS AD_DSB_ADJ7
	,D7.AA_DSB_ADJ			AS AA_DSB_ADJ7
	,D7.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ7
	,D7.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ7
	,D7.AA_DSB_ADJ
	- D7.AA_ORG_FEE_ADJ
	- D7.AA_GTE_FEE_ADJ		AS NETDSB7
	,D8.AD_DSB_ADJ			AS AD_DSB_ADJ8
	,D8.AA_DSB_ADJ			AS AA_DSB_ADJ8
	,D8.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ8
	,D8.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ8
	,D8.AA_DSB_ADJ
	- D8.AA_ORG_FEE_ADJ
	- D8.AA_GTE_FEE_ADJ		AS NETDSB8
	,D9.AD_DSB_ADJ			AS AD_DSB_ADJ9
	,D9.AA_DSB_ADJ			AS AA_DSB_ADJ9
	,D9.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ9
	,D9.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ9
	,D9.AA_DSB_ADJ
	- D9.AA_ORG_FEE_ADJ
	- D9.AA_GTE_FEE_ADJ		AS NETDSB9
	,D10.AD_DSB_ADJ			AS AD_DSB_ADJ10
	,D10.AA_DSB_ADJ			AS AA_DSB_ADJ10
	,D10.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ10
	,D10.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ10
	,D10.AA_DSB_ADJ
	- D10.AA_ORG_FEE_ADJ
	- D10.AA_GTE_FEE_ADJ	AS NETDSB10
	,D11.AD_DSB_ADJ			AS AD_DSB_ADJ11
	,D11.AA_DSB_ADJ			AS AA_DSB_ADJ11
	,D11.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ11
	,D11.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ11
	,D11.AA_DSB_ADJ
	- D11.AA_ORG_FEE_ADJ
	- D11.AA_GTE_FEE_ADJ	AS NETDSB11
	,D12.AD_DSB_ADJ			AS AD_DSB_ADJ12
	,D12.AA_DSB_ADJ			AS AA_DSB_ADJ12
	,D12.AA_ORG_FEE_ADJ		AS AA_ORG_FEE_ADJ12
	,D12.AA_GTE_FEE_ADJ		AS AA_GTE_FEE_ADJ12
	,D12.AA_DSB_ADJ
	- D12.AA_ORG_FEE_ADJ
	- D12.AA_GTE_FEE_ADJ	AS NETDSB12
	,DL.LST_AD_DSB_ADJ
	,CHAR(PHBR.DF_SPE_ID)	AS DF_SPE_ID_BR
	,CHAR(PHST.DF_SPE_ID)	AS DF_SPE_ID_STU
	,A.AF_BS_MPN_APL_ID		AS BASEID
	,A.AC_ELS_LON			AS ELSIND
	,CASE
		WHEN A.AC_ACA_GDE_LEV IN ('1','2','3','4','5')
			THEN 'UNDERGRADUATE'
		WHEN A.AC_ACA_GDE_LEV IN ('A','B','C','D')
			THEN 'GRADUATE'
		END AS GRAD
FROM OLWHRM1.GA01_APP A 
INNER JOIN OLWHRM1.GA10_LON_APP B
	ON A.AF_APL_ID = B.AF_APL_ID
	AND B.AC_PRC_STA = 'A' /*APPROVED LOAN*/
INNER JOIN OLWHRM1.PD01_PDM_INF C
	ON A.DF_PRS_ID_BR = C.DF_PRS_ID
	AND C.DC_ADR = 'L' /*LEGAL ADDRESS*/
INNER JOIN OLWHRM1.PD01_PDM_INF G
	ON A.DF_PRS_ID_STU = G.DF_PRS_ID
INNER JOIN OLWHRM1.LR01_LGS_LDR_INF D
	ON D.IF_IST = A.AF_CUR_APL_OPS_LDR
INNER JOIN 
	(SELECT DISTINCT 
		IF_IST
		,IM_IST_FUL
		,IX_GEN_STR_ADR_1
		,IX_GEN_STR_ADR_2
		,IX_GEN_STR_ADR_3
		,IM_GEN_CT
		,IC_GEN_ST
		,IF_GEN_ZIP
	FROM OLWHRM1.SC01_LGS_SCL_INF) E
	ON E.IF_IST = A.AF_APL_OPS_SCL
/*THIS JOIN SLOWS THE JOB DOWN BY SEVERAL MINUTES*/
INNER JOIN
	(SELECT X.DF_PRS_ID_BR
		,SUM(Y.AA_GTE_LON_AMT 
		- COALESCE(Y.AA_TOT_CAN,0) 
		- Y.AA_TOT_RFD) 			AS NETGTYSUM
	FROM OLWHRM1.GA01_APP X
	INNER JOIN OLWHRM1.GA10_LON_APP Y
		ON Y.AF_APL_ID = X.AF_APL_ID
	INNER JOIN OLWHRM1.GA14_LON_STA Z
		ON Z.AF_APL_ID = Y.AF_APL_ID
		AND Z.AF_APL_ID_SFX = Y.AF_APL_ID_SFX
		AND Z.AC_STA_GA14 = 'A'
	WHERE Z.AC_LON_STA_TYP IN 
		('IA','ID','IM','IG','RP','DA','FB','UC','CR')
		AND Y.AC_GTE_TRF != 'O'
	GROUP BY X.DF_PRS_ID_BR) F
	ON F.DF_PRS_ID_BR = A.DF_PRS_ID_BR
INNER JOIN OLWHRM1.PH01_SUPER_ID PHBR
	ON PHBR.DF_PRS_ID = A.DF_PRS_ID_BR
	AND PHBR.DC_STA_PH01 = 'A'
	AND PHBR.DF_SBS = 'LGS'
	AND PHBR.DF_RGN = '749'
INNER JOIN OLWHRM1.PH01_SUPER_ID PHST
	ON PHST.DF_PRS_ID = A.DF_PRS_ID_STU
	AND PHST.DC_STA_PH01 = 'A'
	AND PHST.DF_SBS = 'LGS'
	AND PHST.DF_RGN = '749'
/***** DISBURSEMENT TABLES ******/
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E' /*ESTIMATED DISBS ONLY*/
	AND AC_DSB_ADJ_STA = 'A' /*ACTIVE ROW*/
	AND AN_DSB_SEQ = 1) D1
	ON D1.AF_APL_ID = B.AF_APL_ID
	AND D1.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E' /*ESTIMATED DISBS ONLY*/
	AND AC_DSB_ADJ_STA = 'A' /*ACTIVE ROW*/
	AND AN_DSB_SEQ = 2) D2
	ON D2.AF_APL_ID = B.AF_APL_ID
	AND D2.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E' /*ESTIMATED DISBS ONLY*/
	AND AC_DSB_ADJ_STA = 'A' /*ACTIVE ROW*/
	AND AN_DSB_SEQ = 3) D3
	ON D3.AF_APL_ID = B.AF_APL_ID
	AND D3.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E' /*ESTIMATED DISBS ONLY*/
	AND AC_DSB_ADJ_STA = 'A' /*ACTIVE ROW*/
	AND AN_DSB_SEQ = 4) D4
	ON D4.AF_APL_ID = B.AF_APL_ID
	AND D4.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E' /*ESTIMATED DISBS ONLY*/
	AND AC_DSB_ADJ_STA = 'A' /*ACTIVE ROW*/
	AND AN_DSB_SEQ = 5) D5
	ON D5.AF_APL_ID = B.AF_APL_ID
	AND D5.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 6) D6
	ON D6.AF_APL_ID = B.AF_APL_ID
	AND D6.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 7) D7
	ON D7.AF_APL_ID = B.AF_APL_ID
	AND D7.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 8) D8
	ON D8.AF_APL_ID = B.AF_APL_ID
	AND D8.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 9) D9
	ON D9.AF_APL_ID = B.AF_APL_ID
	AND D9.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 10) D10
	ON D10.AF_APL_ID = B.AF_APL_ID
	AND D10.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 11) D11
	ON D11.AF_APL_ID = B.AF_APL_ID
	AND D11.AF_APL_ID_SFX = B.AF_APL_ID_SFX
LEFT OUTER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,AD_DSB_ADJ
		,AA_DSB_ADJ
		,AA_ORG_FEE_ADJ
		,AA_GTE_FEE_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	AND AN_DSB_SEQ = 12) D12
	ON D12.AF_APL_ID = B.AF_APL_ID
	AND D12.AF_APL_ID_SFX = B.AF_APL_ID_SFX
/*SELECT LAST DISBURSEMENT DATE*/
INNER JOIN 
	(SELECT AF_APL_ID
		,AF_APL_ID_SFX
		,MAX(AD_DSB_ADJ) AS LST_AD_DSB_ADJ
	FROM OLWHRM1.GA11_LON_DSB_ATY
	WHERE AC_DSB_ADJ = 'E'
	AND AC_DSB_ADJ_STA = 'A'
	GROUP BY AF_APL_ID, AF_APL_ID_SFX) DL
	ON DL.AF_APL_ID = B.AF_APL_ID
	AND DL.AF_APL_ID_SFX = B.AF_APL_ID_SFX
WHERE B.AC_PRC_STA = 'A'
AND B.AD_PRC >= &PVS_DT_DATE
AND	(DAYS(B.AF_LST_DTS_GA10) > DAYS(&PVS_DT_DATE)
	OR
	(DAYS(B.AF_LST_DTS_GA10) = DAYS(&PVS_DT_DATE)
		AND HOUR(B.AF_LST_DTS_GA10) > HOUR(&PVS_DT_TIME))
	)
AND B.AD_BR_NTF_PRC_RSL = CURRENT DATE
/*EXCEPTS RESCREENS AND BWR NOTIFICATION REPRINTS*/
AND B.AI_GTE_RSC = ' '
AND NOT EXISTS
	(
	SELECT 1
	FROM OLWHRM1.GA13_LON_PRC_RSL X
	WHERE X.AF_APL_ID = B.AF_APL_ID
	AND X.AF_APL_ID_SFX = B.AF_APL_ID_SFX
	AND X.AD_BR_NTF_PRC_HST <> B.AD_BR_NTF_PRC_RSL
	AND X.AC_PRC_STA_HST = 'A'
	)

ORDER BY A.AF_CUR_APL_OPS_LDR, A.DF_PRS_ID_BR, B.AF_APL_ID||B.AF_APL_ID_SFX
FOR READ ONLY WITH UR
);
DISCONNECT FROM DB2;
/**********************************************
* ADD INTEREST RATE FROM PSEDUO REG CAT TABLE
***********************************************/
/**********************************************
* UN-COMMENT THIS FOR LIVE
* BLOCK 1
***********************************************/

PROC SQL;
CREATE TABLE DISC AS 
SELECT DISTINCT A.*
	,RG.PR_RPD_FOR_ITR
FROM DISCA A
INNER JOIN OLRPLD.PSEDUO_REG_CAT RG
	ON A.AC_LON_TYP = RG.AC_LON_TYP
	AND A.GRAD = RG.GRAD
;
QUIT;

/**********************************************
* END BLOCK 1
/**********************************************
/**********************************************
* UN-COMMENT THIS FOR TEST
* BLOCK 2
***********************************************/
/*PROC SQL;*/
/*CREATE TABLE DISC AS */
/*SELECT DISTINCT A.**/
/*	,RG.PR_RPD_FOR_ITR*/
/*FROM DISCA A*/
/*INNER JOIN OLRPLD.PSEDUO_REG_CAT_TEST RG*/
/*	ON A.AC_LON_TYP = RG.AC_LON_TYP*/
/*	AND A.GRAD = RG.GRAD*/
/*;*/
/*QUIT;*/
/**********************************************
* END BLOCK 2
/***********************************************/
%PUT  SQLXRC= >>> &SQLXRC <<< ||| SQLXMSG= >>> &SQLXMSG >>> ;  * INCLUDES ERROR MESSAGES TO SAS LOG  ;
%SQLCHECK;
QUIT;
PROC SORT DATA=DISC;
BY LDR_ID DF_PRS_ID_BR CLUID;
RUN;
/*ENDRSUBMIT;*/
/*DATA DISC; SET WORKLOCL.DISC; RUN;*/
PROC SORT DATA=DISC;BY CLUID;RUN;
*ASSIGN LOANS AS BASE/SERIAL;
DATA DISC ;*(DROP=BASEID);
SET DISC;
LENGTH MPN_TYPE $ 6;
IF (BASEID = ' ' OR BASEID EQ AF_APL_ID) THEN MPN_TYPE = 'BASE';
ELSE IF (BASEID NE ' ' AND BASEID NE AF_APL_ID) THEN MPN_TYPE = 'SERIAL';
ELSE MPN_TYPE = 'ERROR';		/*ERROR DETECTION VALUE*/
RUN;

*CALCULATE KEYLINE;
DATA DISC (DROP = KEYSSN MODAY KEYLINE CHKDIG DIG I 
	CHKDIG CHK1 CHK2 CHK3 CHKDIGIT CHECK);
SET DISC;
KEYSSN = TRANSLATE(DF_PRS_ID_BR,'MYLAUGHTER','0987654321');
MODAY = PUT(DATE(),MMDDYYN4.);
KEYLINE = "P"||KEYSSN||MODAY||DC_ADR;
CHKDIG = 0;
LENGTH DIG $2.;
DO I = 1 TO LENGTH(KEYLINE);
	IF I/2 NE ROUND(I/2,1) 
		THEN DIG = PUT(INPUT(SUBSTR(KEYLINE,I,1),BITS4.4) * 2, 2.);
	ELSE DIG = PUT(INPUT(SUBSTR(KEYLINE,I,1),BITS4.4), 2.);
	IF SUBSTR(DIG,1,1) = " " 
		THEN CHKDIG = CHKDIG + INPUT(SUBSTR(DIG,2,1),1.);
		ELSE DO;
			CHK1 = INPUT(SUBSTR(DIG,1,1),1.);
			CHK2 = INPUT(SUBSTR(DIG,2,1),1.);
			IF CHK1 + CHK2 >= 10
				THEN DO;
					CHK3 = PUT(CHK1 + CHK2,2.);
					CHK1 = INPUT(SUBSTR(CHK3,1,1),1.);
					CHK2 = INPUT(SUBSTR(CHK3,2,1),1.);
				END;
			CHKDIG = CHKDIG + CHK1 + CHK2;
		END;
END;
CHKDIGIT = 10 - INPUT(SUBSTR((RIGHT(PUT(CHKDIG,3.))),3,1),3.);
IF CHKDIGIT = 10 THEN CHKDIGIT = 0;
CHECK = PUT(CHKDIGIT,1.);
ACSKEY = "#"||KEYLINE||CHECK||"#";
RUN;

DATA DISCPL; SET DISC; WHERE AC_LON_TYP IN ('PL','GB'); RUN;

*REPAYMENT CALCULATIONS FOR ALL PLUS LOANS;
DATA DISCPL (DROP = CAP_INT1-CAP_INT12 INTEREST P2P WPA);
SET DISCPL;
CAP_INT1 = SUM(NETDSB1,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ1,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT2 = SUM(NETDSB2,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ2,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT3 = SUM(NETDSB3,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ3,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT4 = SUM(NETDSB4,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ4,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT5 = SUM(NETDSB5,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ5,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT6 = SUM(NETDSB6,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ6,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT7 = SUM(NETDSB7,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ7,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT8 = SUM(NETDSB8,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ8,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT9 = SUM(NETDSB9,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ9,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT10 = SUM(NETDSB10,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ10,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT11 = SUM(NETDSB11,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ11,0)) * (PR_RPD_FOR_ITR*0.01/365);
CAP_INT12 = SUM(NETDSB12,0) * (LST_AD_DSB_ADJ - SUM(AD_DSB_ADJ12,0)) * (PR_RPD_FOR_ITR*0.01/365);
TOT_CAP_INT = SUM(OF CAP_INT1-CAP_INT12);
REPAY_PRIN = NETGTYAMT + TOT_CAP_INT;
PMT = MORT(REPAY_PRIN, ., PR_RPD_FOR_ITR*0.01/12, 10*12);
IF PMT < 50 THEN PMT = 50;
WPA = REPAY_PRIN;
TOT_REPAY_AMT = 0;
NUM_PAY = 0;
DO UNTIL (WPA < (1.5 * PMT));
	INTEREST = WPA * PR_RPD_FOR_ITR*0.01/12;
	P2P = PMT - INTEREST;
	WPA = WPA - P2P;
	NUM_PAY + 1;
	TOT_REPAY_AMT = TOT_REPAY_AMT + PMT;
END;
INTEREST = WPA * (PR_RPD_FOR_ITR * 0.01 / 12);
LAST_PMT = WPA + INTEREST;
TOT_REPAY_AMT = TOT_REPAY_AMT + LAST_PMT;
REPAY_INT = TOT_REPAY_AMT - REPAY_PRIN;
FORMAT TOT_CAP_INT 10.2 REPAY_PRIN 10.2 PMT 10.2 INTEREST 10.2 
P2P 10.2 LAST_PMT 10.2 TOT_REPAY_AMT 10.2 REPAY_INT 10.2;
*INT_EFF_TO = PUT(INTNX('YEAR.7',TODAY(),0,'END'), MMDDYY10.);
INT_EFF_TO = PUT(INTNX('YEAR.7',AD_DSB_ADJ1,0,'END'), MMDDYY10.);
PMT_DT = PUT(INTNX('MONTH',LST_AD_DSB_ADJ,1,'BEGINNING') + DAY(LST_AD_DSB_ADJ) -1,MMDDYY10.);
LAST_PMT_DT = PUT(INTNX('MONTH',LST_AD_DSB_ADJ,NUM_PAY + 1,'BEGINNING') + DAY(LST_AD_DSB_ADJ) -1,MMDDYY10.);
RUN;

*MASTER MERGEFILE FOR ALL STAFFORD LOANS;
DATA DISCSTAF (KEEP = SSN DF_SPE_ACC_ID AF_APL_ID BWRNAME DX_STR_ADR_1 DX_STR_ADR_2 DM_CT DC_DOM_ST DF_ZIP
DM_FGN_CNY DC_ADR DI_VLD_ADR DF_PRS_ID_STU STUNAME LDR_ID LDR_NAME LDR_ADR_1 LDR_ADR_2
LDR_ADR_3 LDR_CT LDR_ST LDR_ZIP SCL_ID SCL_NAME SCL_ADR1 SCL_ADR2 SCL_ADR3 SCL_CITY
SCL_ST SCL_ZIP AD_XPC_GRD AD_IST_TRM_BEG AD_IST_TRM_END NETGTYSUM ACSKEY DF_SPE_ID_BR
DF_SPE_ID_STU AD_PRC MPN_TYPE);
SET DISC;
WHERE AC_LON_TYP IN ('SF','SU');
RUN;

PROC SORT DATA=DISCSTAF NODUPREC;
BY LDR_ID SSN AF_APL_ID;
RUN;

*MERGEFILE FOR SUBSIDIZED STAFFORD LOANS;
DATA DISCSF (KEEP = SSN DF_SPE_ACC_ID AF_APL_ID LDR_ID CLUID AC_LON_TYP NETGTYAMT AA_ORG_TOT NETLONAMT
PR_RPD_FOR_ITR SCL_CERT_AMT AD_DSB_ADJ1 AA_DSB_ADJ1 AA_ORG_FEE_ADJ1 NETDSB1 AD_DSB_ADJ2
AA_DSB_ADJ2 AA_ORG_FEE_ADJ2 NETDSB2 AD_DSB_ADJ3 AA_DSB_ADJ3 AA_ORG_FEE_ADJ3 NETDSB3
AD_DSB_ADJ4 AA_DSB_ADJ4 AA_ORG_FEE_ADJ4 NETDSB4 AD_DSB_ADJ5 AA_DSB_ADJ5 AA_ORG_FEE_ADJ5
NETDSB5 AD_DSB_ADJ6 AA_DSB_ADJ6 AA_ORG_FEE_ADJ6 NETDSB6 AD_DSB_ADJ7 AA_DSB_ADJ7
AA_ORG_FEE_ADJ7 NETDSB7 AD_DSB_ADJ8 AA_DSB_ADJ8 AA_ORG_FEE_ADJ8 NETDSB8 AD_DSB_ADJ9
AA_DSB_ADJ9 AA_ORG_FEE_ADJ9 NETDSB9 AD_DSB_ADJ10 AA_DSB_ADJ10 AA_ORG_FEE_ADJ10
NETDSB10 AD_DSB_ADJ11 AA_DSB_ADJ11 AA_ORG_FEE_ADJ11 NETDSB11 AD_DSB_ADJ12
AA_DSB_ADJ12 AA_ORG_FEE_ADJ12 NETDSB12 AA_GTE_FEE_ADJ1 AA_GTE_FEE_ADJ2 AA_GTE_FEE_ADJ3 AA_GTE_FEE_ADJ4
AA_GTE_FEE_ADJ5 AA_GTE_FEE_ADJ6 AA_GTE_FEE_ADJ7 AA_GTE_FEE_ADJ8 AA_GTE_FEE_ADJ9 AA_GTE_FEE_ADJ10 
AA_GTE_FEE_ADJ11 AA_GTE_FEE_ADJ12 AA_GTE_TOT); 
SET DISC; 
WHERE AC_LON_TYP = 'SF'; 
RUN;

*MERGEFILE FOR UNSUBSIDIZED STAFFORD LOANS;
DATA DISCSU (KEEP = SSN DF_SPE_ACC_ID AF_APL_ID LDR_ID SU_CLUID SU_AC_LON_TYP SU_NETGTYAMT SU_AA_ORG_TOT
SU_NETLONAMT SU_PR_RPD_FOR_ITR SU_SCL_CERT_AMT SU_AD_DSB_ADJ1 SU_AA_DSB_ADJ1 SU_AA_ORG_FEE_ADJ1
SU_NETDSB1 SU_AD_DSB_ADJ2 SU_AA_DSB_ADJ2 SU_AA_ORG_FEE_ADJ2 SU_NETDSB2 SU_AD_DSB_ADJ3
SU_AA_DSB_ADJ3 SU_AA_ORG_FEE_ADJ3 SU_NETDSB3 SU_AD_DSB_ADJ4 SU_AA_DSB_ADJ4 SU_AA_ORG_FEE_ADJ4
SU_NETDSB4 SU_AD_DSB_ADJ5 SU_AA_DSB_ADJ5 SU_AA_ORG_FEE_ADJ5 SU_NETDSB5 SU_AD_DSB_ADJ6
SU_AA_DSB_ADJ6 SU_AA_ORG_FEE_ADJ6 SU_NETDSB6 SU_AD_DSB_ADJ7 SU_AA_DSB_ADJ7 SU_AA_ORG_FEE_ADJ7
SU_NETDSB7 SU_AD_DSB_ADJ8 SU_AA_DSB_ADJ8 SU_AA_ORG_FEE_ADJ8 SU_NETDSB8 SU_AD_DSB_ADJ9
SU_AA_DSB_ADJ9 SU_AA_ORG_FEE_ADJ9 SU_NETDSB9 SU_AD_DSB_ADJ10 SU_AA_DSB_ADJ10 
SU_AA_ORG_FEE_ADJ10 SU_NETDSB10 SU_AD_DSB_ADJ11 SU_AA_DSB_ADJ11 SU_AA_ORG_FEE_ADJ11
SU_NETDSB11 SU_AD_DSB_ADJ12 SU_AA_DSB_ADJ12 SU_AA_ORG_FEE_ADJ12 SU_NETDSB12
SU_AA_GTE_FEE_ADJ1 SU_AA_GTE_FEE_ADJ2 SU_AA_GTE_FEE_ADJ3 SU_AA_GTE_FEE_ADJ4 SU_AA_GTE_FEE_ADJ5
SU_AA_GTE_FEE_ADJ6 SU_AA_GTE_FEE_ADJ7 SU_AA_GTE_FEE_ADJ8 SU_AA_GTE_FEE_ADJ9 SU_AA_GTE_FEE_ADJ10
SU_AA_GTE_FEE_ADJ11 SU_AA_GTE_FEE_ADJ12 SU_AA_GTE_TOT); 
SET DISC; 
WHERE AC_LON_TYP = 'SU'; 
SU_CLUID = CLUID;
SU_AC_LON_TYP = AC_LON_TYP;
SU_NETGTYAMT = NETGTYAMT;
SU_AA_ORG_TOT = AA_ORG_TOT;
SU_AA_GTE_TOT = AA_GTE_TOT;
SU_NETLONAMT = NETLONAMT;
SU_PR_RPD_FOR_ITR = PR_RPD_FOR_ITR;
SU_SCL_CERT_AMT = SCL_CERT_AMT;
SU_AD_DSB_ADJ1 = AD_DSB_ADJ1;
SU_AA_DSB_ADJ1 = AA_DSB_ADJ1;
SU_AA_ORG_FEE_ADJ1 = AA_ORG_FEE_ADJ1;
SU_AA_GTE_FEE_ADJ1 = AA_GTE_FEE_ADJ1;
SU_NETDSB1 = NETDSB1;
SU_AD_DSB_ADJ2 = AD_DSB_ADJ2;
SU_AA_DSB_ADJ2 = AA_DSB_ADJ2;
SU_AA_ORG_FEE_ADJ2 = AA_ORG_FEE_ADJ2;
SU_AA_GTE_FEE_ADJ2 = AA_GTE_FEE_ADJ2;
SU_NETDSB2 = NETDSB2;
SU_AD_DSB_ADJ3 = AD_DSB_ADJ3;
SU_AA_DSB_ADJ3 = AA_DSB_ADJ3;
SU_AA_ORG_FEE_ADJ3 = AA_ORG_FEE_ADJ3;
SU_AA_GTE_FEE_ADJ3 = AA_GTE_FEE_ADJ3;
SU_NETDSB3 = NETDSB3;
SU_AD_DSB_ADJ4 = AD_DSB_ADJ4;
SU_AA_DSB_ADJ4 = AA_DSB_ADJ4;
SU_AA_ORG_FEE_ADJ4 = AA_ORG_FEE_ADJ4;
SU_AA_GTE_FEE_ADJ4 = AA_GTE_FEE_ADJ4;
SU_NETDSB4 = NETDSB4;
SU_AD_DSB_ADJ5 = AD_DSB_ADJ5;
SU_AA_DSB_ADJ5 = AA_DSB_ADJ5;
SU_AA_ORG_FEE_ADJ5 = AA_ORG_FEE_ADJ5;
SU_AA_GTE_FEE_ADJ5 = AA_GTE_FEE_ADJ5;
SU_NETDSB5 = NETDSB5;
SU_AD_DSB_ADJ6 = AD_DSB_ADJ6;
SU_AA_DSB_ADJ6 = AA_DSB_ADJ6;
SU_AA_ORG_FEE_ADJ6 = AA_ORG_FEE_ADJ6;
SU_AA_GTE_FEE_ADJ6 = AA_GTE_FEE_ADJ6;
SU_NETDSB6 = NETDSB6;
SU_AD_DSB_ADJ7 = AD_DSB_ADJ7;
SU_AA_DSB_ADJ7 = AA_DSB_ADJ7;
SU_AA_ORG_FEE_ADJ7 = AA_ORG_FEE_ADJ7;
SU_AA_GTE_FEE_ADJ7 = AA_GTE_FEE_ADJ7;
SU_NETDSB7 = NETDSB7;
SU_AD_DSB_ADJ8 = AD_DSB_ADJ8;
SU_AA_DSB_ADJ8 = AA_DSB_ADJ8;
SU_AA_ORG_FEE_ADJ8 = AA_ORG_FEE_ADJ8;
SU_AA_GTE_FEE_ADJ8 = AA_GTE_FEE_ADJ8;
SU_NETDSB8 = NETDSB8;
SU_AD_DSB_ADJ9 = AD_DSB_ADJ9;
SU_AA_DSB_ADJ9 = AA_DSB_ADJ9;
SU_AA_ORG_FEE_ADJ9 = AA_ORG_FEE_ADJ9;
SU_AA_GTE_FEE_ADJ9 = AA_GTE_FEE_ADJ9;
SU_NETDSB9 = NETDSB9;
SU_AD_DSB_ADJ10 = AD_DSB_ADJ10;
SU_AA_DSB_ADJ10 = AA_DSB_ADJ10;
SU_AA_ORG_FEE_ADJ10 = AA_ORG_FEE_ADJ10;
SU_AA_GTE_FEE_ADJ10 = AA_GTE_FEE_ADJ10;
SU_NETDSB10 = NETDSB10;
SU_AD_DSB_ADJ11 = AD_DSB_ADJ11;
SU_AA_DSB_ADJ11 = AA_DSB_ADJ11;
SU_AA_ORG_FEE_ADJ11 = AA_ORG_FEE_ADJ11;
SU_AA_GTE_FEE_ADJ11 = AA_GTE_FEE_ADJ11;
SU_NETDSB11 = NETDSB11;
SU_AD_DSB_ADJ12 = AD_DSB_ADJ12;
SU_AA_DSB_ADJ12 = AA_DSB_ADJ12;
SU_AA_ORG_FEE_ADJ12 = AA_ORG_FEE_ADJ12;
SU_AA_GTE_FEE_ADJ12 = AA_GTE_FEE_ADJ12;
SU_NETDSB12 = NETDSB12;
RUN;

PROC SORT DATA=DISCSTAF; BY LDR_ID SSN AF_APL_ID;RUN;
PROC SORT DATA=DISCSF; BY LDR_ID SSN AF_APL_ID;RUN;
PROC SORT DATA=DISCSU; BY LDR_ID SSN AF_APL_ID;RUN;

*ALL STAFFORD LOANS;
DATA DISCGL (DROP = AF_APL_ID);
MERGE DISCSTAF DISCSF DISCSU;
BY LDR_ID SSN AF_APL_ID;
RUN;

DATA DISCGL;
SET DISCGL;
FORMAT APP_CRT_AMT 10.2;
APP_CRT_AMT = SUM(SU_SCL_CERT_AMT, SCL_CERT_AMT);
*INT_EFF_TO = PUT(INTNX('YEAR.7',TODAY(),0,'END'), MMDDYY10.);
IF AD_DSB_ADJ1 NE . 
	THEN INT_EFF_TO = PUT(INTNX('YEAR.7',AD_DSB_ADJ1,0,'END'), MMDDYY10.);
	ELSE INT_EFF_TO = PUT(INTNX('YEAR.7',SU_AD_DSB_ADJ1,0,'END'), MMDDYY10.);
RUN;

%MACRO STAF(DS, OUT, COND);
DATA _NULL_ ;
SET  &DS END=EFIEOD;
&COND;
%LET _EFIERR_ = 0; /* SET THE ERROR DETECTION MACRO VARIABLE */
%LET _EFIREC_ = 0;     /* CLEAR EXPORT RECORD COUNT MACRO VARIABLE */
FILE &OUT DELIMITER=',' DSD DROPOVER LRECL=32767;
	FORMAT SSN SSN11. ;
	FORMAT DF_SPE_ACC_ID $10. ;
	FORMAT BWRNAME $50. ;
	FORMAT DX_STR_ADR_1 $35. ;
	FORMAT DX_STR_ADR_2 $35. ;
	FORMAT DM_CT $30. ;
	FORMAT DC_DOM_ST $2. ;
	FORMAT DF_ZIP $14. ;
	FORMAT DM_FGN_CNY $25. ;
	FORMAT DC_ADR $1. ;
	FORMAT DI_VLD_ADR $1. ;
	FORMAT DF_PRS_ID_STU $9. ;
	FORMAT STUNAME $50. ;
	FORMAT LDR_ID $8. ;
	FORMAT LDR_NAME $40. ;
	FORMAT LDR_ADR_1 $40. ;
	FORMAT LDR_ADR_2 $40. ;
	FORMAT LDR_ADR_3 $40. ;
	FORMAT LDR_CT $30. ;
	FORMAT LDR_ST $2. ;
	FORMAT LDR_ZIP $14. ;
	FORMAT SCL_ID $8. ;
	FORMAT SCL_NAME $40. ;
	FORMAT SCL_ADR1 $40. ;
	FORMAT SCL_ADR2 $40. ;
	FORMAT SCL_ADR3 $40. ;
	FORMAT SCL_CITY $30. ;
	FORMAT SCL_ST $2. ;
	FORMAT SCL_ZIP $14. ;
	FORMAT AD_XPC_GRD MMDDYY10. ;
	FORMAT AD_IST_TRM_BEG MMDDYY10. ;
	FORMAT AD_IST_TRM_END MMDDYY10. ;
	FORMAT NETGTYSUM DOLLAR12.2 ;
	FORMAT MPN_TYPE $6.;
	FORMAT ACSKEY $18. ;

	FORMAT CLUID 			$19. ;
	FORMAT AD_PRC 			MMDDYY10. ;
	FORMAT AC_LON_TYP 		$2. ;
	FORMAT NETGTYAMT		DOLLAR17.2 ;
	FORMAT AA_ORG_TOT 		DOLLAR26.2 ;
	FORMAT AA_GTE_TOT 		DOLLAR26.2 ;
	FORMAT NETLONAMT 		DOLLAR29.2 ;
	FORMAT PR_RPD_FOR_ITR 	8.3 ;
	FORMAT SCL_CERT_AMT 	DOLLAR12.2 ;
	FORMAT AD_DSB_ADJ1 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ1 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ1 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ1 	DOLLAR9.2 ;
	FORMAT NETDSB1 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ2 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ2 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ2 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ2 	DOLLAR9.2 ;
	FORMAT NETDSB2 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ3 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ3 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ3 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ3 	DOLLAR9.2 ;
	FORMAT NETDSB3 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ4 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ4 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ4 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ4 	DOLLAR9.2 ;
	FORMAT NETDSB4 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ5 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ5 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ5 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ5 	DOLLAR9.2 ;
	FORMAT NETDSB5 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ6 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ6 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ6	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ6 	DOLLAR9.2 ;
	FORMAT NETDSB6 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ7 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ7 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ7 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ7 	DOLLAR9.2 ;
	FORMAT NETDSB7 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ8 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ8 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ8 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ8 	DOLLAR9.2 ;
	FORMAT NETDSB8 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ9 		MMDDYY10. ;
	FORMAT AA_DSB_ADJ9 		DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ9 	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ9 	DOLLAR9.2 ;
	FORMAT NETDSB9 			DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ10 	MMDDYY10. ;
	FORMAT AA_DSB_ADJ10 	DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ10	DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ10 DOLLAR9.2 ;
	FORMAT NETDSB10 		DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ11 	MMDDYY10. ;
	FORMAT AA_DSB_ADJ11 	DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ11 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ11 DOLLAR9.2 ;
	FORMAT NETDSB11 		DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ12 	MMDDYY10. ;
	FORMAT AA_DSB_ADJ12 	DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ12 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ12 DOLLAR9.2 ;
	FORMAT NETDSB12 		DOLLAR11.2 ;
	FORMAT SU_CLUID 			$19. ;
	FORMAT SU_AC_LON_TYP 		$2. ;
	FORMAT SU_NETGTYAMT 		DOLLAR17.2 ;
	FORMAT SU_AA_ORG_TOT 		DOLLAR26.2 ;
	FORMAT SU_AA_GTE_TOT 		DOLLAR26.2 ;
	FORMAT SU_NETLONAMT 		DOLLAR29.2 ;
	FORMAT SU_PR_RPD_FOR_ITR 	8.3 ;
	FORMAT SU_SCL_CERT_AMT 		DOLLAR12.2 ;
	FORMAT SU_AD_DSB_ADJ1 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ1 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ1 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ1 	DOLLAR9.2 ;
	FORMAT SU_NETDSB1 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ2 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ2 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ2	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ2 	DOLLAR9.2 ;
	FORMAT SU_NETDSB2			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ3 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ3 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ3 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ3 	DOLLAR9.2 ;
	FORMAT SU_NETDSB3 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ4 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ4 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ4 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ4 	DOLLAR9.2 ;
	FORMAT SU_NETDSB4 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ5 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ5 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ5 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ5 	DOLLAR9.2 ;
	FORMAT SU_NETDSB5 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ6 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ6 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ6 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ6 	DOLLAR9.2 ;
	FORMAT SU_NETDSB6 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ7 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ7 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ7 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ7 	DOLLAR9.2 ;
	FORMAT SU_NETDSB7 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ8 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ8 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ8 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ8 	DOLLAR9.2 ;
	FORMAT SU_NETDSB8 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ9 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ9 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ9 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ9 	DOLLAR9.2 ;
	FORMAT SU_NETDSB9 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ10 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ10 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ10 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ10 	DOLLAR9.2 ;
	FORMAT SU_NETDSB10 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ11 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ11 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ11 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ11 	DOLLAR9.2 ;
	FORMAT SU_NETDSB11 			DOLLAR11.2 ;
	FORMAT SU_AD_DSB_ADJ12 		MMDDYY10. ;
	FORMAT SU_AA_DSB_ADJ12 		DOLLAR10.2 ;
	FORMAT SU_AA_ORG_FEE_ADJ12 	DOLLAR9.2 ;
	FORMAT SU_AA_GTE_FEE_ADJ12 	DOLLAR9.2 ;
	FORMAT SU_NETDSB12 			DOLLAR11.2 ;
	FORMAT APP_CRT_AMT			DOLLAR11.2;
	FORMAT DF_SPE_ID_BR			$10.;
	FORMAT DF_SPE_ID_STU		$10.;
	FORMAT INT_EFF_TO 			$10. ;

IF _N_ = 1 THEN        /* WRITE COLUMN NAMES */
 DO;
   	PUT
	'LDR_ID'
   	','
   	'SSN'
   	','
	'ACCOUNT_NUM'
	','
   	'BWRNAME'
   	','
   	'DX_STR_ADR_1'
   	','
   	'DX_STR_ADR_2'
   	','
   	'DM_CT'
   	','
   	'DC_DOM_ST'
   	','
   	'DF_ZIP'
   	','
   	'DM_FGN_CNY'
   	','
   	'DC_ADR'
   	','
   	'DI_VLD_ADR'
   	','
   	'DF_PRS_ID_STU'
   	','
   	'STUNAME'
   	','
   	'LDR_ID'
   	','
   	'LDR_NAME'
   	','
   	'LDR_ADR_1'
   	','
   	'LDR_ADR_2'
   	','
   	'LDR_ADR_3'
   	','
   	'LDR_CT'
   	','
   	'LDR_ST'
   	','
   	'LDR_ZIP'
   	','
   	'SCL_ID'
   	','
   	'SCL_NAME'
   	','
   	'SCL_ADR1'
   	','
   	'SCL_ADR2'
   	','
   	'SCL_ADR3'
   	','
   	'SCL_CITY'
   	','
   	'SCL_ST'
   	','
   	'SCL_ZIP'
   	','
   	'AD_XPC_GRD'
   	','
   	'AD_IST_TRM_BEG'
   	','
   	'AD_IST_TRM_END'
   	','
   	'NETGTYSUM'
   	','
   	'ACSKEY'
   	','
   	'CLUID'
   	','
   	'AD_PRC'
   	','
   	'AC_LON_TYP'
   	','
   	'NETGTYAMT'
   	','
   	'AA_ORG_TOT'
   	','
   	'AA_GTE_TOT'
	','
   	'NETLONAMT'
   	','
   	'PR_RPD_FOR_ITR'
   	','
   	'SCL_CERT_AMT'
   	','
   	'AD_DSB_ADJ1'
   	','
   	'AA_DSB_ADJ1'
   	','
   	'AA_ORG_FEE_ADJ1'
   	','
	'AA_GTE_FEE_ADJ1'
   	','
   	'NETDSB1'
   	','
   	'AD_DSB_ADJ2'
   	','
   	'AA_DSB_ADJ2'
   	','
   	'AA_ORG_FEE_ADJ2'
   	','
   	'AA_GTE_FEE_ADJ2'
   	','
   	'NETDSB2'
   	','
   	'AD_DSB_ADJ3'
   	','
   	'AA_DSB_ADJ3'
   	','
   	'AA_ORG_FEE_ADJ3'
   	','
   	'AA_GTE_FEE_ADJ3'
   	','
   	'NETDSB3'
   	','
   	'AD_DSB_ADJ4'
   	','
   	'AA_DSB_ADJ4'
   	','
   	'AA_ORG_FEE_ADJ4'
   	','
   	'AA_GTE_FEE_ADJ4'
   	','
   	'NETDSB4'
   	','
   	'AD_DSB_ADJ5'
   	','
   	'AA_DSB_ADJ5'
   	','
   	'AA_ORG_FEE_ADJ5'
   	','
   	'AA_GTE_FEE_ADJ5'
   	','
   	'NETDSB5'
   	','
   	'AD_DSB_ADJ6'
   	','
   	'AA_DSB_ADJ6'
   	','
   	'AA_ORG_FEE_ADJ6'
   	','
   	'AA_GTE_FEE_ADJ6'
   	','
   	'NETDSB6'
   	','
   	'AD_DSB_ADJ7'
   	','
   	'AA_DSB_ADJ7'
   	','
   	'AA_ORG_FEE_ADJ7'
   	','
   	'AA_GTE_FEE_ADJ7'
   	','
   	'NETDSB7'
   	','
   	'AD_DSB_ADJ8'
   	','
   	'AA_DSB_ADJ8'
   	','
   	'AA_ORG_FEE_ADJ8'
   	','
   	'AA_GTE_FEE_ADJ8'
   	','
   	'NETDSB8'
   	','
   	'AD_DSB_ADJ9'
   	','
   	'AA_DSB_ADJ9'
   	','
   	'AA_ORG_FEE_ADJ9'
   	','
   	'AA_GTE_FEE_ADJ9'
   	','
   	'NETDSB9'
   	','
   	'AD_DSB_ADJ10'
   	','
   	'AA_DSB_ADJ10'
   	','
   	'AA_ORG_FEE_ADJ10'
   	','
   	'AA_GTE_FEE_ADJ10'
   	','
   	'NETDSB10'
   	','
   	'AD_DSB_ADJ11'
   	','
   	'AA_DSB_ADJ11'
   	','
   	'AA_ORG_FEE_ADJ11'
   	','
   	'AA_GTE_FEE_ADJ11'
   	','
   	'NETDSB11'
   	','
   	'AD_DSB_ADJ12'
   	','
   	'AA_DSB_ADJ12'
   	','
   	'AA_ORG_FEE_ADJ12'
    ','
   	'AA_GTE_FEE_ADJ12'
   	','
   	'NETDSB12'
   	','
   	'SU_CLUID'
   	','
   	'SU_AC_LON_TYP'
   	','
   	'SU_NETGTYAMT'
   	','
   	'SU_AA_ORG_TOT'
   	','
	'SU_AA_GTE_TOT'
	','
   	'SU_NETLONAMT'
   	','
   	'SU_PR_RPD_FOR_ITR'
   	','
   	'SU_SCL_CERT_AMT'
   	','
   	'SU_AD_DSB_ADJ1'
   	','
   	'SU_AA_DSB_ADJ1'
   	','
   	'SU_AA_ORG_FEE_ADJ1'
   	','
	'SU_AA_GTE_FEE_ADJ1'
	','
   	'SU_NETDSB1'
   	','
   	'SU_AD_DSB_ADJ2'
   	','
   	'SU_AA_DSB_ADJ2'
   	','
   	'SU_AA_ORG_FEE_ADJ2'
   	','
	'SU_AA_GTE_FEE_ADJ2'
	','
   	'SU_NETDSB2'
   	','
   	'SU_AD_DSB_ADJ3'
   	','
   	'SU_AA_DSB_ADJ3'
   	','
   	'SU_AA_ORG_FEE_ADJ3'
   	','
	'SU_AA_GTE_FEE_ADJ3'
	','
   	'SU_NETDSB3'
   	','
   	'SU_AD_DSB_ADJ4'
   	','
   	'SU_AA_DSB_ADJ4'
   	','
   	'SU_AA_ORG_FEE_ADJ4'
   	','
	'SU_AA_GTE_FEE_ADJ4'
	','
   	'SU_NETDSB4'
   	','
   	'SU_AD_DSB_ADJ5'
   	','
   	'SU_AA_DSB_ADJ5'
   	','
   	'SU_AA_ORG_FEE_ADJ5'
   	','
	'SU_AA_GTE_FEE_ADJ5'
	','
   	'SU_NETDSB5'
   	','
   	'SU_AD_DSB_ADJ6'
   	','
   	'SU_AA_DSB_ADJ6'
   	','
   	'SU_AA_ORG_FEE_ADJ6'
   	','
	'SU_AA_GTE_FEE_ADJ6'
	','
   	'SU_NETDSB6'
   	','
   	'SU_AD_DSB_ADJ7'
   	','
   	'SU_AA_DSB_ADJ7'
   	','
   	'SU_AA_ORG_FEE_ADJ7'
   	','
	'SU_AA_GTE_FEE_ADJ7'
	','
   	'SU_NETDSB7'
   	','
   	'SU_AD_DSB_ADJ8'
   	','
   	'SU_AA_DSB_ADJ8'
   	','
   	'SU_AA_ORG_FEE_ADJ8'
   	','
	'SU_AA_GTE_FEE_ADJ8'
	','
   	'SU_NETDSB8'
   	','
   	'SU_AD_DSB_ADJ9'
   	','
   	'SU_AA_DSB_ADJ9'
   	','
   	'SU_AA_ORG_FEE_ADJ9'
   	','
	'SU_AA_GTE_FEE_ADJ9'
	','
   	'SU_NETDSB9'
   	','
   	'SU_AD_DSB_ADJ10'
   	','
   	'SU_AA_DSB_ADJ10'
   	','
   	'SU_AA_ORG_FEE_ADJ10'
   	','
	'SU_AA_GTE_FEE_ADJ10'
	','
   	'SU_NETDSB10'
   	','
   	'SU_AD_DSB_ADJ11'
   	','
   	'SU_AA_DSB_ADJ11'
   	','
   	'SU_AA_ORG_FEE_ADJ11'
   	','
	'SU_AA_GTE_FEE_ADJ11'
	','
   	'SU_NETDSB11'
   	','
   	'SU_AD_DSB_ADJ12'
   	','
   	'SU_AA_DSB_ADJ12'
   	','
   	'SU_AA_ORG_FEE_ADJ12'
   	','
	'SU_AA_GTE_FEE_ADJ12'
	','
   	'SU_NETDSB12'
	','
	'APP_CRT_AMT'
	','
	'DF_SPE_ID_BR'
	','
	'DF_SPE_ID_STU'
	','
	'INT_EFF_TO'
	','
	'MPN_TYPE'
   	;
END;

DO;
   	EFIOUT + 1;
	PUT LDR_ID $ @;
   	PUT SSN @;
	PUT DF_SPE_ACC_ID @;
   	PUT BWRNAME $ @;
   	PUT DX_STR_ADR_1 $ @;
   	PUT DX_STR_ADR_2 $ @;
   	PUT DM_CT $ @;
   	PUT DC_DOM_ST $ @;
   	PUT DF_ZIP $ @;
   	PUT DM_FGN_CNY $ @;
   	PUT DC_ADR $ @;
   	PUT DI_VLD_ADR $ @;
   	PUT DF_PRS_ID_STU $ @;
   	PUT STUNAME $ @;
   	PUT LDR_ID $ @;
   	PUT LDR_NAME $ @;
   	PUT LDR_ADR_1 $ @;
   	PUT LDR_ADR_2 $ @;
   	PUT LDR_ADR_3 $ @;
   	PUT LDR_CT $ @;
   	PUT LDR_ST $ @;
   	PUT LDR_ZIP $ @;
   	PUT SCL_ID $ @;
   	PUT SCL_NAME $ @;
   	PUT SCL_ADR1 $ @;
   	PUT SCL_ADR2 $ @;
   	PUT SCL_ADR3 $ @;
   	PUT SCL_CITY $ @;
   	PUT SCL_ST $ @;
   	PUT SCL_ZIP $ @;
   	PUT AD_XPC_GRD @;
   	PUT AD_IST_TRM_BEG @;
   	PUT AD_IST_TRM_END @;
   	PUT NETGTYSUM @;
   	PUT ACSKEY $ @;
   	PUT CLUID $ @;
   	PUT AD_PRC @;
   	PUT AC_LON_TYP $ @;
   	PUT NETGTYAMT @;
   	PUT AA_ORG_TOT @;
	PUT AA_GTE_TOT @;
   	PUT NETLONAMT @;
   	PUT PR_RPD_FOR_ITR @;
   	PUT SCL_CERT_AMT @;
   	PUT AD_DSB_ADJ1 @;
   	PUT AA_DSB_ADJ1 @;
   	PUT AA_ORG_FEE_ADJ1 @;
	PUT AA_GTE_FEE_ADJ1 @;
   	PUT NETDSB1 @;
   	PUT AD_DSB_ADJ2 @;
   	PUT AA_DSB_ADJ2 @;
   	PUT AA_ORG_FEE_ADJ2 @;
   	PUT AA_GTE_FEE_ADJ2 @;
	PUT NETDSB2 @;
   	PUT AD_DSB_ADJ3 @;
   	PUT AA_DSB_ADJ3 @;
   	PUT AA_ORG_FEE_ADJ3 @;
	PUT AA_GTE_FEE_ADJ3 @;
   	PUT NETDSB3 @;
   	PUT AD_DSB_ADJ4 @;
   	PUT AA_DSB_ADJ4 @;
   	PUT AA_ORG_FEE_ADJ4 @;
	PUT AA_GTE_FEE_ADJ4 @;
   	PUT NETDSB4 @;
   	PUT AD_DSB_ADJ5 @;
   	PUT AA_DSB_ADJ5 @;
   	PUT AA_ORG_FEE_ADJ5 @;
	PUT AA_GTE_FEE_ADJ5 @;
   	PUT NETDSB5 @;
   	PUT AD_DSB_ADJ6 @;
   	PUT AA_DSB_ADJ6 @;
   	PUT AA_ORG_FEE_ADJ6 @;
	PUT AA_GTE_FEE_ADJ6 @;
   	PUT NETDSB6 @;
   	PUT AD_DSB_ADJ7 @;
   	PUT AA_DSB_ADJ7 @;
   	PUT AA_ORG_FEE_ADJ7 @;
	PUT AA_GTE_FEE_ADJ7 @;
   	PUT NETDSB7 @;
   	PUT AD_DSB_ADJ8 @;
   	PUT AA_DSB_ADJ8 @;
   	PUT AA_ORG_FEE_ADJ8 @;
	PUT AA_GTE_FEE_ADJ8 @;
   	PUT NETDSB8 @;
   	PUT AD_DSB_ADJ9 @;
   	PUT AA_DSB_ADJ9 @;
   	PUT AA_ORG_FEE_ADJ9 @;
	PUT AA_GTE_FEE_ADJ9 @;
   	PUT NETDSB9 @;
   	PUT AD_DSB_ADJ10 @;
   	PUT AA_DSB_ADJ10 @;
   	PUT AA_ORG_FEE_ADJ10 @;
	PUT AA_GTE_FEE_ADJ10 @;
   	PUT NETDSB10 @;
   	PUT AD_DSB_ADJ11 @;
   	PUT AA_DSB_ADJ11 @;
   	PUT AA_ORG_FEE_ADJ11 @;
	PUT AA_GTE_FEE_ADJ11 @;
   	PUT NETDSB11 @;
   	PUT AD_DSB_ADJ12 @;
   	PUT AA_DSB_ADJ12 @;
   	PUT AA_ORG_FEE_ADJ12 @;
	PUT AA_GTE_FEE_ADJ12 @;
   	PUT NETDSB12 @;
   	PUT SU_CLUID $ @;
   	PUT SU_AC_LON_TYP $ @;
   	PUT SU_NETGTYAMT @;
   	PUT SU_AA_ORG_TOT @;
	PUT SU_AA_GTE_TOT @;
   	PUT SU_NETLONAMT @;
   	PUT SU_PR_RPD_FOR_ITR @;
   	PUT SU_SCL_CERT_AMT @;
   	PUT SU_AD_DSB_ADJ1 @;
   	PUT SU_AA_DSB_ADJ1 @;
   	PUT SU_AA_ORG_FEE_ADJ1 @;
	PUT SU_AA_GTE_FEE_ADJ1 @;
   	PUT SU_NETDSB1 @;
   	PUT SU_AD_DSB_ADJ2 @;
   	PUT SU_AA_DSB_ADJ2 @;
   	PUT SU_AA_ORG_FEE_ADJ2 @;
	PUT SU_AA_GTE_FEE_ADJ2 @;
   	PUT SU_NETDSB2 @;
   	PUT SU_AD_DSB_ADJ3 @;
   	PUT SU_AA_DSB_ADJ3 @;
   	PUT SU_AA_ORG_FEE_ADJ3 @;
	PUT SU_AA_GTE_FEE_ADJ3 @;
   	PUT SU_NETDSB3 @;
   	PUT SU_AD_DSB_ADJ4 @;
   	PUT SU_AA_DSB_ADJ4 @;
   	PUT SU_AA_ORG_FEE_ADJ4 @;
	PUT SU_AA_GTE_FEE_ADJ4 @;
   	PUT SU_NETDSB4 @;
   	PUT SU_AD_DSB_ADJ5 @;
   	PUT SU_AA_DSB_ADJ5 @;
   	PUT SU_AA_ORG_FEE_ADJ5 @;
	PUT SU_AA_GTE_FEE_ADJ5 @;
   	PUT SU_NETDSB5 @;
   	PUT SU_AD_DSB_ADJ6 @;
   	PUT SU_AA_DSB_ADJ6 @;
   	PUT SU_AA_ORG_FEE_ADJ6 @;
	PUT SU_AA_GTE_FEE_ADJ6 @;
   	PUT SU_NETDSB6 @;
   	PUT SU_AD_DSB_ADJ7 @;
   	PUT SU_AA_DSB_ADJ7 @;
   	PUT SU_AA_ORG_FEE_ADJ7 @;
	PUT SU_AA_GTE_FEE_ADJ7 @;
   	PUT SU_NETDSB7 @;
   	PUT SU_AD_DSB_ADJ8 @;
   	PUT SU_AA_DSB_ADJ8 @;
   	PUT SU_AA_ORG_FEE_ADJ8 @;
	PUT SU_AA_GTE_FEE_ADJ8 @;
   	PUT SU_NETDSB8 @;
   	PUT SU_AD_DSB_ADJ9 @;
   	PUT SU_AA_DSB_ADJ9 @;
   	PUT SU_AA_ORG_FEE_ADJ9 @;
	PUT SU_AA_GTE_FEE_ADJ9 @;
   	PUT SU_NETDSB9 @;
   	PUT SU_AD_DSB_ADJ10 @;
   	PUT SU_AA_DSB_ADJ10 @;
   	PUT SU_AA_ORG_FEE_ADJ10 @;
	PUT SU_AA_GTE_FEE_ADJ10 @;
   	PUT SU_NETDSB10 @;
   	PUT SU_AD_DSB_ADJ11 @;
   	PUT SU_AA_DSB_ADJ11 @;
   	PUT SU_AA_ORG_FEE_ADJ11 @;
	PUT SU_AA_GTE_FEE_ADJ11 @;
   	PUT SU_NETDSB11 @;
   	PUT SU_AD_DSB_ADJ12 @;
   	PUT SU_AA_DSB_ADJ12 @;
   	PUT SU_AA_ORG_FEE_ADJ12 @;
	PUT SU_AA_GTE_FEE_ADJ12 @;
   	PUT SU_NETDSB12 @;
	PUT APP_CRT_AMT @;
	PUT DF_SPE_ID_BR $ @;
	PUT DF_SPE_ID_STU $ @;
	PUT INT_EFF_TO $ @;
	PUT MPN_TYPE $;
   	;
END;
IF _ERROR_ THEN CALL SYMPUT('_EFIERR_',1);  /* SET ERROR DETECTION MACRO VARIABLE */
IF EFIEOD THEN
   CALL SYMPUT('_EFIREC_',EFIOUT);
RUN;
%MEND;

/**ALL STAFFORD LOANS WITH VALID ADDRESS;*/
/*%STAF(DISCGL, REPORT2, %STR(WHERE DI_VLD_ADR = "Y"));*/
/**ALL STAFFORD LOANS WITH INVALID ADDRESS;*/
/*%STAF(DISCGL, REPORT3, %STR(WHERE DI_VLD_ADR = "N"));*/
/**ALL WESTMINSTER LOANS;*/
/*%STAF(DISCGL, REPORT4, %STR(WHERE SCL_ID = '00368100'));*/

*ALL STAFFORD LOANS WITH VALID DOMESTIC ADDRESS NON UHEAA;
%STAF(DISCGL, REPORT2, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST <> '' AND LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*ALL STAFFORD LOANS WITH VALID FOREIGN ADDRESS NON UHEAA;
%STAF(DISCGL, REPORT12, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST = '' AND LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*ALL STAFFORD LOANS WITH INVALID ADDRESS NON UHEAA;
%STAF(DISCGL, REPORT3, %STR(WHERE DI_VLD_ADR = "N" 
	AND LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*ALL WESTMINSTER LOANS;
%STAF(DISCGL, REPORT4, %STR(WHERE SCL_ID = '00368100' AND 
	AD_DSB_ADJ9 = .));
*ALL STAFFORD LOANS WITH VALID DOMESTIC ADDRESS - UHEAA;
%STAF(DISCGL, REPORT14, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST <> '' AND LDR_ID = '828476' AND AD_DSB_ADJ9 = .));
*ALL STAFFORD LOANS WITH VALID FOREIGN ADDRESS - UHEAA;
%STAF(DISCGL, REPORT18, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST = '' AND LDR_ID = '828476' AND AD_DSB_ADJ9 = .));
*ALL STAFFORD LOANS WITH INVALID ADDRESS - UHEAA;
%STAF(DISCGL, REPORT15, %STR(WHERE DI_VLD_ADR = "N" AND 
	LDR_ID = '828476' AND AD_DSB_ADJ9 = .));


PROC SORT DATA=DISCGL;
BY SCL_ID SSN;
RUN;

%STAF(DISCGL, REPORT10,	%STR(WHERE SCL_ID IN(
	'03030600','03030601','03030603','03030604','03030605','03030606') AND AD_DSB_ADJ9 = .));

%MACRO PLUS(DS, OUT, COND);
DATA _NULL_;
SET  &DS END=EFIEOD;
&COND;
%LET _EFIERR_ = 0; /* SET THE ERROR DETECTION MACRO VARIABLE */
%LET _EFIREC_ = 0;     /* CLEAR EXPORT RECORD COUNT MACRO VARIABLE */
FILE &OUT DELIMITER=',' DSD DROPOVER LRECL=32767;
	FORMAT SSN SSN11. ;
	FORMAT DF_SPE_ACC_ID $10. ;
	FORMAT AF_APL_ID $17. ;
	FORMAT CLUID $19. ;
	FORMAT AD_PRC MMDDYY10. ;
	FORMAT AC_LON_TYP $2. ;
	FORMAT BWRNAME $50. ;
	FORMAT DX_STR_ADR_1 $35. ;
	FORMAT DX_STR_ADR_2 $35. ;
	FORMAT DM_CT $30. ;
	FORMAT DC_DOM_ST $2. ;
	FORMAT DF_ZIP $14. ;
	FORMAT DM_FGN_CNY $25. ;
	FORMAT DC_ADR $1. ;
	FORMAT DI_VLD_ADR $1. ;
	FORMAT DF_PRS_ID_STU $9. ;
	FORMAT STUNAME $50. ;
	FORMAT LDR_ID $8. ;
	FORMAT LDR_NAME $40. ;
	FORMAT LDR_ADR_1 $40. ;
	FORMAT LDR_ADR_2 $40. ;
	FORMAT LDR_ADR_3 $40. ;
	FORMAT LDR_CT $30. ;
	FORMAT LDR_ST $2. ;
	FORMAT LDR_ZIP $14. ;
	FORMAT SCL_ID $8. ;
	FORMAT SCL_NAME $40. ;
	FORMAT SCL_ADR1 $40. ;
	FORMAT SCL_ADR2 $40. ;
	FORMAT SCL_ADR3 $40. ;
	FORMAT SCL_CITY $30. ;
	FORMAT SCL_ST $2. ;
	FORMAT SCL_ZIP $14. ;
	FORMAT AD_XPC_GRD MMDDYY10. ;
	FORMAT NETGTYAMT COMMA17.2 ;
	FORMAT AA_ORG_TOT DOLLAR12.2 ;
	FORMAT AA_GTE_TOT DOLLAR12.2 ;
	FORMAT NETLONAMT DOLLAR29.2 ;
	FORMAT PR_RPD_FOR_ITR 8.3 ;
	FORMAT SCL_CERT_AMT DOLLAR12.2 ;
	FORMAT AD_IST_TRM_BEG MMDDYY10. ;
	FORMAT AD_IST_TRM_END MMDDYY10. ;
	FORMAT NETGTYSUM DOLLAR12.2 ;
	FORMAT AD_DSB_ADJ1 MMDDYY10. ;
	FORMAT AA_DSB_ADJ1 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ1 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ1 DOLLAR9.2 ;
	FORMAT NETDSB1 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ2 MMDDYY10. ;
	FORMAT AA_DSB_ADJ2 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ2 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ2 DOLLAR9.2 ;
	FORMAT NETDSB2 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ3 MMDDYY10. ;
	FORMAT AA_DSB_ADJ3 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ3 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ3 DOLLAR9.2 ;
	FORMAT NETDSB3 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ4 MMDDYY10. ;
	FORMAT AA_DSB_ADJ4 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ4 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ4 DOLLAR9.2 ;
	FORMAT NETDSB4 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ5 MMDDYY10. ;
	FORMAT AA_DSB_ADJ5 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ5 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ5 DOLLAR9.2 ;
	FORMAT NETDSB5 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ6 MMDDYY10. ;
	FORMAT AA_DSB_ADJ6 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ6 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ6 DOLLAR9.2 ;
	FORMAT NETDSB6 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ7 MMDDYY10. ;
	FORMAT AA_DSB_ADJ7 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ7 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ7 DOLLAR9.2 ;
	FORMAT NETDSB7 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ8 MMDDYY10. ;
	FORMAT AA_DSB_ADJ8 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ8 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ8 DOLLAR9.2 ;
	FORMAT NETDSB8 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ9 MMDDYY10. ;
	FORMAT AA_DSB_ADJ9 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ9 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ9 DOLLAR9.2 ;
	FORMAT NETDSB9 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ10 MMDDYY10. ;
	FORMAT AA_DSB_ADJ10 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ10 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ10 DOLLAR9.2 ;
	FORMAT NETDSB10 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ11 MMDDYY10. ;
	FORMAT AA_DSB_ADJ11 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ11 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ11 DOLLAR9.2 ;
	FORMAT NETDSB11 DOLLAR11.2 ;
	FORMAT AD_DSB_ADJ12 MMDDYY10. ;
	FORMAT AA_DSB_ADJ12 DOLLAR10.2 ;
	FORMAT AA_ORG_FEE_ADJ12 DOLLAR9.2 ;
	FORMAT AA_GTE_FEE_ADJ12 DOLLAR9.2 ;
	FORMAT NETDSB12 DOLLAR11.2 ;
	FORMAT LST_AD_DSB_ADJ MMDDYY10. ;
	FORMAT ACSKEY $18. ;
	FORMAT TOT_CAP_INT COMMA10.2 ;
	FORMAT REPAY_PRIN DOLLAR10.2 ;
	FORMAT PMT DOLLAR10.2 ;
	FORMAT TOT_REPAY_AMT COMMA10.2 ;
	FORMAT NUM_PAY BEST12. ;
	FORMAT LAST_PMT DOLLAR10.2 ;
	FORMAT REPAY_INT COMMA10.2 ;
	FORMAT INT_EFF_TO $10. ;
	FORMAT PMT_DT $10. ;
	FORMAT LAST_PMT_DT $10. ;
	FORMAT DF_SPE_ID_BR			$10.;
	FORMAT DF_SPE_ID_STU		$10.;
	FORMAT MPN_TYPE $6.;

IF _N_ = 1 THEN        /* WRITE COLUMN NAMES */
 DO;
   PUT
   'LDR_ID'
   ','
   'SSN'
   ','
   'ACCOUNT_NUM'
   ','
   'AF_APL_ID'
   ','
   'CLUID'
   ','
   'AD_PRC'
   ','
   'AC_LON_TYP'
   ','
   'BWRNAME'
   ','
   'DX_STR_ADR_1'
   ','
   'DX_STR_ADR_2'
   ','
   'DM_CT'
   ','
   'DC_DOM_ST'
   ','
   'DF_ZIP'
   ','
   'DM_FGN_CNY'
   ','
   'DC_ADR'
   ','
   'DI_VLD_ADR'
   ','
   'DF_PRS_ID_STU'
   ','
   'STUNAME'
   ','
   'LDR_ID'
   ','
   'LDR_NAME'
   ','
   'LDR_ADR_1'
   ','
   'LDR_ADR_2'
   ','
   'LDR_ADR_3'
   ','
   'LDR_CT'
   ','
   'LDR_ST'
   ','
   'LDR_ZIP'
   ','
   'SCL_ID'
   ','
   'SCL_NAME'
   ','
   'SCL_ADR1'
   ','
   'SCL_ADR2'
   ','
   'SCL_ADR3'
   ','
   'SCL_CITY'
   ','
   'SCL_ST'
   ','
   'SCL_ZIP'
   ','
   'AD_XPC_GRD'
   ','
   'NETGTYAMT'
   ','
   'AA_ORG_TOT'
   ','
   'AA_GTE_TOT'
   ','
   'NETLONAMT'
   ','
   'PR_RPD_FOR_ITR'
   ','
   'SCL_CERT_AMT'
   ','
   'AD_IST_TRM_BEG'
   ','
   'AD_IST_TRM_END'
   ','
   'NETGTYSUM'
   ','
   'AD_DSB_ADJ1'
   ','
   'AA_DSB_ADJ1'
   ','
   'AA_ORG_FEE_ADJ1'
   ','
   'AA_GTE_FEE_ADJ1'
   ','
   'NETDSB1'
   ','
   'AD_DSB_ADJ2'
   ','
   'AA_DSB_ADJ2'
   ','
   'AA_ORG_FEE_ADJ2'
   ','
   'AA_GTE_FEE_ADJ2'
   ','
   'NETDSB2'
   ','
   'AD_DSB_ADJ3'
   ','
   'AA_DSB_ADJ3'
   ','
   'AA_ORG_FEE_ADJ3'
   ','
   'AA_GTE_FEE_ADJ3'
   ','
   'NETDSB3'
   ','
   'AD_DSB_ADJ4'
   ','
   'AA_DSB_ADJ4'
   ','
   'AA_ORG_FEE_ADJ4'
   ','
   'AA_GTE_FEE_ADJ4'
   ','
   'NETDSB4'
   ','
   'AD_DSB_ADJ5'
   ','
   'AA_DSB_ADJ5'
   ','
   'AA_ORG_FEE_ADJ5'
   ','
   'AA_GTE_FEE_ADJ5'
   ','
   'NETDSB5'
   ','
   'AD_DSB_ADJ6'
   ','
   'AA_DSB_ADJ6'
   ','
   'AA_ORG_FEE_ADJ6'
   ','
   'AA_GTE_FEE_ADJ6'
   ','
   'NETDSB6'
   ','
   'AD_DSB_ADJ7'
   ','
   'AA_DSB_ADJ7'
   ','
   'AA_ORG_FEE_ADJ7'
   ','
   'AA_GTE_FEE_ADJ7'
   ','
   'NETDSB7'
   ','
   'AD_DSB_ADJ8'
   ','
   'AA_DSB_ADJ8'
   ','
   'AA_ORG_FEE_ADJ8'
   ','
   'AA_GTE_FEE_ADJ8'
   ','
   'NETDSB8'
   ','
   'AD_DSB_ADJ9'
   ','
   'AA_DSB_ADJ9'
   ','
   'AA_ORG_FEE_ADJ9'
   ','
   'AA_GTE_FEE_ADJ9'
   ','
   'NETDSB9'
   ','
   'AD_DSB_ADJ10'
   ','
   'AA_DSB_ADJ10'
   ','
   'AA_ORG_FEE_ADJ10'
   ','
   'AA_GTE_FEE_ADJ10'
   ','
   'NETDSB10'
   ','
   'AD_DSB_ADJ11'
   ','
   'AA_DSB_ADJ11'
   ','
   'AA_ORG_FEE_ADJ11'
   ','
   'AA_GTE_FEE_ADJ11'
   ','
   'NETDSB11'
   ','
   'AD_DSB_ADJ12'
   ','
   'AA_DSB_ADJ12'
   ','
   'AA_ORG_FEE_ADJ12'
   ','
   'AA_GTE_FEE_ADJ12'
   ','
   'NETDSB12'
   ','
   'LST_AD_DSB_ADJ'
   ','
   'ACSKEY'
   ','
   'TOT_CAP_INT'
   ','
   'REPAY_PRIN'
   ','
   'PMT'
   ','
   'TOT_REPAY_AMT'
   ','
   'NUM_PAY'
   ','
   'LAST_PMT'
   ','
   'REPAY_INT'
   ','
   'INT_EFF_TO'
   ','
   'PMT_DT'
   ','
   'LAST_PMT_DT'
	','
	'DF_SPE_ID_BR'
	','
	'DF_SPE_ID_STU'
	','
	'MPN_TYPE'
   ;
 END;
DO;
   EFIOUT + 1;
   PUT LDR_ID $ @;
   PUT SSN @;
   PUT DF_SPE_ACC_ID @;
   PUT AF_APL_ID $ @;
   PUT CLUID $ @;
   PUT AD_PRC @;
   PUT AC_LON_TYP $ @;
   PUT BWRNAME $ @;
   PUT DX_STR_ADR_1 $ @;
   PUT DX_STR_ADR_2 $ @;
   PUT DM_CT $ @;
   PUT DC_DOM_ST $ @;
   PUT DF_ZIP $ @;
   PUT DM_FGN_CNY $ @;
   PUT DC_ADR $ @;
   PUT DI_VLD_ADR $ @;
   PUT DF_PRS_ID_STU $ @;
   PUT STUNAME $ @;
   PUT LDR_ID $ @;
   PUT LDR_NAME $ @;
   PUT LDR_ADR_1 $ @;
   PUT LDR_ADR_2 $ @;
   PUT LDR_ADR_3 $ @;
   PUT LDR_CT $ @;
   PUT LDR_ST $ @;
   PUT LDR_ZIP $ @;
   PUT SCL_ID $ @;
   PUT SCL_NAME $ @;
   PUT SCL_ADR1 $ @;
   PUT SCL_ADR2 $ @;
   PUT SCL_ADR3 $ @;
   PUT SCL_CITY $ @;
   PUT SCL_ST $ @;
   PUT SCL_ZIP $ @;
   PUT AD_XPC_GRD @;
   PUT NETGTYAMT @;
   PUT AA_ORG_TOT @;
   PUT AA_GTE_TOT @;
   PUT NETLONAMT @;
   PUT PR_RPD_FOR_ITR @;
   PUT SCL_CERT_AMT $ @;
   PUT AD_IST_TRM_BEG @;
   PUT AD_IST_TRM_END @;
   PUT NETGTYSUM @;
   PUT AD_DSB_ADJ1 @;
   PUT AA_DSB_ADJ1 @;
   PUT AA_ORG_FEE_ADJ1 @;
   PUT AA_GTE_FEE_ADJ1 @;
   PUT NETDSB1 @;
   PUT AD_DSB_ADJ2 @;
   PUT AA_DSB_ADJ2 @;
   PUT AA_ORG_FEE_ADJ2 @;
   PUT AA_GTE_FEE_ADJ2 @;
   PUT NETDSB2 @;
   PUT AD_DSB_ADJ3 @;
   PUT AA_DSB_ADJ3 @;
   PUT AA_ORG_FEE_ADJ3 @;
   PUT AA_GTE_FEE_ADJ3 @;
   PUT NETDSB3 @;
   PUT AD_DSB_ADJ4 @;
   PUT AA_DSB_ADJ4 @;
   PUT AA_ORG_FEE_ADJ4 @;
   PUT AA_GTE_FEE_ADJ4 @;
   PUT NETDSB4 @;
   PUT AD_DSB_ADJ5 @;
   PUT AA_DSB_ADJ5 @;
   PUT AA_ORG_FEE_ADJ5 @;
   PUT AA_GTE_FEE_ADJ5 @;
   PUT NETDSB5 @;
   PUT AD_DSB_ADJ6 @;
   PUT AA_DSB_ADJ6 @;
   PUT AA_ORG_FEE_ADJ6 @;
   PUT AA_GTE_FEE_ADJ6 @;
   PUT NETDSB6 @;
   PUT AD_DSB_ADJ7 @;
   PUT AA_DSB_ADJ7 @;
   PUT AA_ORG_FEE_ADJ7 @;
   PUT AA_GTE_FEE_ADJ7 @;
   PUT NETDSB7 @;
   PUT AD_DSB_ADJ8 @;
   PUT AA_DSB_ADJ8 @;
   PUT AA_ORG_FEE_ADJ8 @;
   PUT AA_GTE_FEE_ADJ8 @;
   PUT NETDSB8 @;
   PUT AD_DSB_ADJ9 @;
   PUT AA_DSB_ADJ9 @;
   PUT AA_ORG_FEE_ADJ9 @;
   PUT AA_GTE_FEE_ADJ9 @;
   PUT NETDSB9 @;
   PUT AD_DSB_ADJ10 @;
   PUT AA_DSB_ADJ10 @;
   PUT AA_ORG_FEE_ADJ10 @;
   PUT AA_GTE_FEE_ADJ10 @;
   PUT NETDSB10 @;
   PUT AD_DSB_ADJ11 @;
   PUT AA_DSB_ADJ11 @;
   PUT AA_ORG_FEE_ADJ11 @;
   PUT AA_GTE_FEE_ADJ11 @;
   PUT NETDSB11 @;
   PUT AD_DSB_ADJ12 @;
   PUT AA_DSB_ADJ12 @;
   PUT AA_ORG_FEE_ADJ12 @;
   PUT AA_GTE_FEE_ADJ12 @;
   PUT NETDSB12 @;
   PUT LST_AD_DSB_ADJ @;
   PUT ACSKEY $ @;
   PUT TOT_CAP_INT @;
   PUT REPAY_PRIN @;
   PUT PMT @;
   PUT TOT_REPAY_AMT @;
   PUT NUM_PAY @;
   PUT LAST_PMT @;
   PUT REPAY_INT @;
   PUT INT_EFF_TO $ @;
   PUT PMT_DT $ @;
   PUT LAST_PMT_DT $ @;
	PUT DF_SPE_ID_BR $ @;
	PUT DF_SPE_ID_STU $ @;
	PUT MPN_TYPE $;
   ;
 END;
IF _ERROR_ THEN CALL SYMPUT('_EFIERR_',1);  /* SET ERROR DETECTION MACRO VARIABLE */
IF EFIEOD THEN
   CALL SYMPUT('_EFIREC_',EFIOUT);
RUN;
%MEND;

/**ALL PLUS LOANS WITH VALID ADDRESS;*/
/*%PLUS(DISCPL, REPORT5, %STR(WHERE DI_VLD_ADR = "Y"));*/
/**ALL PLUS LOANS WITH INVALID ADDRESS;*/
/*%PLUS(DISCPL, REPORT6, %STR(WHERE DI_VLD_ADR = "N"));*/
/**WESTMINSTER PLUS LOANS REGARDLESS OF VALID ADDRESS;*/
/*%PLUS(DISCPL, REPORT7, %STR(WHERE SCL_ID = '00368100'));*/

*ALL PLUS LOANS WITH VALID DOMESTIC ADDRESS NON UHEAA;
%PLUS(DISCPL, REPORT5, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST <> '' AND LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*ALL PLUS LOANS WITH VALID FOREIGN ADDRESS NON UHEAA;
%PLUS(DISCPL, REPORT13, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST = '' AND LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*ALL PLUS LOANS WITH INVALID ADDRESS NON UHEAA;
%PLUS(DISCPL, REPORT6, %STR(WHERE DI_VLD_ADR = "N" AND 
	LDR_ID <> '828476' AND AD_DSB_ADJ9 = .));
*WESTMINSTER PLUS LOANS REGARDLESS OF VALID ADDRESS;
%PLUS(DISCPL, REPORT7, %STR(WHERE SCL_ID = '00368100' AND 
	AD_DSB_ADJ9 = .));
*ALL PLUS LOANS WITH VALID DOMESTIC ADDRESS - UHEAA;
%PLUS(DISCPL, REPORT16, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST <> '' AND LDR_ID = '828476' AND AD_DSB_ADJ9 = .));
*ALL PLUS LOANS WITH VALID FOREIGN ADDRESS - UHEAA;
%PLUS(DISCPL, REPORT19, %STR(WHERE DI_VLD_ADR = "Y" AND 
	DC_DOM_ST = '' AND LDR_ID = '828476' AND AD_DSB_ADJ9 = .));
*ALL PLUS LOANS WITH INVALID ADDRESS - UHEAA;
%PLUS(DISCPL, REPORT17, %STR(WHERE DI_VLD_ADR = "N" AND 
	LDR_ID = '828476' AND AD_DSB_ADJ9 = .));

PROC SORT DATA=DISCPL;
BY SCL_ID SSN;
RUN;
*UCMT LOANS REGARDLESS OF VALID ADDRESS;
%PLUS(DISCPL, REPORT11,	%STR(WHERE SCL_ID IN(
	'03030600','03030601','03030603','03030604','03030605','03030606') AND AD_DSB_ADJ9 = .));

*BAD ADDRESS REPORT;
DATA INVADR (KEEP = SSN CLUID AC_LON_TYP NETGTYAMT);
SET DISC;
WHERE DI_VLD_ADR = 'N' AND AD_DSB_ADJ9 = .;
RUN;

PROC SORT DATA=INVADR;
BY SSN CLUID;
RUN;

DATA _NULL_;
CALL SYMPUT('RUNDATE',PUT(INTNX('DAY',TODAY(),0,'BEGINNING'), MMDDYY10.));
RUN;

PROC PRINTTO PRINT=REPORT8 NEW;
RUN;
OPTIONS CENTER NODATE NUMBER PAGENO=1 LS=126;
PROC PRINT NOOBS SPLIT='/' DATA=INVADR WIDTH=UNIFORM WIDTH=MIN N='NUMBER OF LOANS:  ';
VAR SSN CLUID AC_LON_TYP NETGTYAMT;
LABEL SSN='BORROWER SSN' CLUID='COMMONLINE UNIQUE ID' 
AC_LON_TYP='LOAN TYPE' NETGTYAMT='NET GUARANTY AMOUNT';
FORMAT SSN SSN11. NETGTYAMT DOLLAR12.2;
TITLE 'BORROWERS WITH INVALID ADDRESSES - NO DISCLOSURE CREATED';
TITLE2 "DISCLOSURE DATE: &RUNDATE";
FOOTNOTE  'JOB = UTLWO01     REPORT = ULWO01.LWO01R8';
RUN;
PROC PRINTTO;
RUN;

*RE-ASSIGN LOANS AS BASE/SERIAL/ESIGN;
DATA DISC ;
LENGTH AC_LON_TYP $ 10;
FORMAT AC_LON_TYP $10.;
SET DISC;
LENGTH MPN_TYPE $ 6;
IF (BASEID = ' ' OR BASEID EQ AF_APL_ID) AND ELSIND NE 'E' THEN MPN_TYPE = 'NEW';
ELSE IF (BASEID NE ' ' AND BASEID NE AF_APL_ID) THEN MPN_TYPE = 'SERIAL';
ELSE IF (BASEID = ' ' OR BASEID EQ AF_APL_ID) AND ELSIND EQ 'E' THEN MPN_TYPE = 'E-SIGN';
ELSE MPN_TYPE = 'ERROR';		/*ERROR DETECTION VALUE*/
RUN;

PROC PRINTTO PRINT=REPORT9 NEW;
RUN;
OPTIONS CENTER NODATE NUMBER PAGENO=1 LS=126;
PROC REPORT DATA=DISC NOWD SPACING=1 /*HEADSKIP*/ HEADLINE SPLIT='/';
TITLE 'DAILY DISCLOSURE TOTALS';
TITLE2 "DATE: &RUNDATE";
FOOTNOTE  'JOB = UTLWO01     REPORT = ULWO01.LWO01R9';
COLUMN MPN_TYPE AC_LON_TYP N;
DEFINE MPN_TYPE / GROUP "MPN TYPE" WIDTH=10;
DEFINE AC_LON_TYP / GROUP "LOAN TYPE" WIDTH=10;
DEFINE N /  FORMAT=COMMA8. "LOANS";
COMPUTE AFTER MPN_TYPE;
AC_LON_TYP = "SUBTOTAL:";
ENDCOMP;
BREAK AFTER MPN_TYPE / SUMMARIZE SKIP DOL DUL SUPPRESS;
COMPUTE AFTER;
MPN_TYPE = "TOTAL:";
ENDCOMP;
RBREAK AFTER / SUMMARIZE SKIP DOL DUL SUPPRESS;
RUN;
PROC PRINTTO;
RUN;
/*----------------------------------------------------------*
| CREATE FILE WITH BORROWERS WITH MORE THAN 8 DISBURSEMENTS |
*-----------------------------------------------------------*/
DATA GRT8 (KEEP=DF_PRS_ID_BR BWRNAME NETGTYSUM CLUID AD_PRC AC_LON_TYP AD_DSB_ADJ1 AA_DSB_ADJ1);
SET DISC;
WHERE AD_DSB_ADJ9 ^= .;
RUN;
PROC PRINTTO PRINT=REPORT20 NEW;
RUN;
OPTIONS CENTER NODATE NUMBER PAGENO=1 LS=126;
PROC CONTENTS DATA=GRT8 OUT=EMPTYSET NOPRINT;
DATA _NULL_;
SET EMPTYSET;
FILE PRINT;
IF  NOBS=0 AND _N_ =1 THEN DO;
	PUT // 126*'-';
	PUT      //////
		@51 '**** NO OBSERVATIONS FOUND ****';
	PUT //////
		@57 '-- END OF REPORT --';
	PUT ////////////
		@46 "JOB = UTLWO01     REPORT = ULWO01.LWO01R20";
	END;
RETURN;
RUN;
OPTIONS CENTER DATE NUMBER PAGENO=1 LS=126;
PROC PRINT NOOBS SPLIT='/' DATA=GRT8 WIDTH=UNIFORM WIDTH=MIN N='NUMBER OF LOANS:  ';
VAR DF_PRS_ID_BR BWRNAME NETGTYSUM CLUID AD_PRC AC_LON_TYP AD_DSB_ADJ1 AA_DSB_ADJ1;
LABEL DF_PRS_ID_BR = 'SSN' BWRNAME = 'NAME' NETGTYSUM = 'NET GUARANTEE SUM' CLUID = 'CLUID' 
AD_PRC = 'PROCESSING DATE' AC_LON_TYP = 'LOAN TYPE' AD_DSB_ADJ1 = 'FIRST DISB DATE' 
AA_DSB_ADJ1 = 'FIRST DIB AMOUNT';
FORMAT NETGTYSUM  AA_DSB_ADJ1 12.2 AD_DSB_ADJ1 AD_PRC MMDDYY10.;
TITLE 'BORROWERS W/ MORE THAN 8 DISBURSEMENTS';
FOOTNOTE  'JOB = UTLWO01     REPORT = ULWO01.LWO01R20';
RUN;
PROC PRINTTO;
RUN;


/****************************************************************/
/****************************************************************/
/*ONLY FOR TESTING*/
/*CREATES THE PSEDUO_REG_CAT_TEST TABLE*/
/****************************************************************/
/****************************************************************/
/*
LIBNAME  WORKLOCL  REMOTE  SERVER=CYPRUS  SLIBREF=WORK;
RSUBMIT;
LIBNAME OLRPLD V8 '/sas/whse/olrp_lookup_directory';
DATA OLRPLD.PSEDUO_REG_CAT_TEST;
format GRAD $14.;
INPUT AC_LON_TYP $ GRAD $ PR_RPD_FOR_ITR;
DATALINES;
GB GRADUATE      8.5
PL GRADUATE      8.5
SF GRADUATE      6.8
SF UNDERGRADUATE 6
SU GRADUATE      6.8
SU UNDERGRADUATE 6.8
;
RUN;
ENDRSUBMIT;
*/
