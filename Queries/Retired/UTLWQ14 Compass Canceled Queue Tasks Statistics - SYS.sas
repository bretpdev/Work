/*UTLWQ14 SYS CANCELLED TASKS QUEUE STATISTICS*/

/*LIBNAME DLGSUTWH DB2 DATABASE=DLGSUTWH OWNER=OLWHRM1;
%LET RPTLIB = %SYSGET(reportdir);
FILENAME REPORT2 "&RPTLIB/ULWQ14.LWQ14R2";*/

LIBNAME  WORKLOCL  REMOTE  SERVER=CYPRUS  SLIBREF=WORK;
RSUBMIT;
PROC SQL;
CONNECT TO DB2 (DATABASE=DLGSUTWH);
CREATE TABLE SYSCTQS AS
SELECT *
...
AND A.WF_USR_OWN_QUE IN ('UT00017','UT00144')
);
DISCONNECT FROM DB2;
ENDRSUBMIT;

DATA SYSCTQS;
SET WORKLOCL.SYSCTQS;
RUN;

PROC SORT DATA=SYSCTQS;
...
TITLE 'SYS - CANCELLED TASKS QUEUE STATISTICS';
TITLE2 "Run Date: &CURDATE";

PROC CONTENTS DATA=SYSCTQS OUT=EMPTYSET NOPRINT;
DATA _NULL_;
SET EMPTYSET;
FILE PRINT;
IF  NOBS=0 AND _N_ =1 THEN DO;
	...
	PUT ////////
		@57 '-- END OF REPORT --';
	PUT ////////////////
		@46 "JOB = UTLWQ14  	 REPORT = ULWQ14.LWQ14R2";
	END;
RETURN;
RUN;
PROC PRINT NOOBS SPLIT='/' DATA=SYSCTQS WIDTH=UNIFORM N="TOTAL: ";
BY LF_PRF_BY ;
VAR WF_QUE
	WM_QUE_FUL
	WF_SUB_QUE
...
FOOTNOTE  'JOB = UTLWQ14  	 REPORT = ULWQ14.LWQ14R2';
RUN;
