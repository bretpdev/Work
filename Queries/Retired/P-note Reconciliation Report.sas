*---------------------------------------------------------------------------------------------*
| PNOTE RECONCILIATION REPORT - THIS JOB TAKES A LONG TIME TO RUN. BETWEEN 30 AND 120 MINUTES |
*---------------------------------------------------------------------------------------------*;
%LET RPTLIB = C:\WINDOWS\TEMP;
FILENAME REPORT2 "&RPTLIB/DSASI02.R2";
FILENAME REPORT3 "&RPTLIB/DSASI02.R3";
FILENAME REPORT4 "&RPTLIB/DSASI02.R4";
DATA _NULL_;
...
SET BASE_LOANS2;
IF BASE_2 ^= '' 
	THEN BASE_LOANS = LEFT(TRIM(BASE_1))||' '||LEFT(TRIM(BASE_2));
ELSE 
...
RETAIN SER_LOANS;
IF FIRST.MPN_ID THEN SER_LOANS = SERIAL_LN_ID;
ELSE SER_LOANS = TRIM(SER_LOANS)||' '||SERIAL_LN_ID;
IF LAST.MPN_ID THEN OUTPUT;
RUN;
...
RUN;
PROC SORT DATA=LNSTAT;BY MPN_ID CLUID;RUN;
...
RUN;
/*CREATE REPORT FOR NON E-SIGNED PROM NOTES*/
PROC PRINTTO PRINT=REPORT2 NEW;
RUN;
OPTIONS ORIENTATION = LANDSCAPE;
OPTIONS PS=39 LS=132 PAGENO=1;
PROC REPORT DATA=ALL_DAT NOWD SPACING=2 HEADSKIP SPLIT='/';
WHERE AC_ELS_LON ^= 'E';
TITLE "PROMISSORY NOTE RECONCILIATION REPORT";
FOOTNOTE  'JOB = PNOTE RECONCILIATION    REPORT = R2';
COLUMN BF_SSN DM_PRS_1 DM_PRS_LST AD_BR_SIG AF_APL_OPS_SCL AF_CUR_APL_OPS_LDR MPN_ID_DIGTS AC_LON_STA_TYP PNOTE VU;
DEFINE BF_SSN / DISPLAY GROUP "SSN" LEFT;
DEFINE DM_PRS_1 / DISPLAY GROUP "FIRST NAME" WIDTH=10;
DEFINE DM_PRS_LST / DISPLAY GROUP "LAST NAME" WIDTH=15;
DEFINE AD_BR_SIG / DISPLAY "STD SIGN" FORMAT=MMDDYY10.;
DEFINE AF_APL_OPS_SCL / DISPLAY "SCHOOL";
DEFINE AF_CUR_APL_OPS_LDR / DISPLAY "ORIG/LNDR" WIDTH=6;
DEFINE MPN_ID_DIGTS / DISPLAY "BASE/MPN ID" WIDTH=6;
DEFINE AC_LON_STA_TYP / DISPLAY "LOAN/STATUS" WIDTH=6;
DEFINE PNOTE / DISPLAY "P-NOTE" WIDTH=7;
DEFINE VU / DISPLAY "VUMASTER" WIDTH=8;
COMPUTE AFTER BF_SSN;
LINE ' ';
ENDCOMP;
RUN;
PROC PRINTTO;
RUN;
/*CREATE REPORT FOR E-SIGNED PROM NOTES*/
PROC PRINTTO PRINT=REPORT4 NEW;
RUN;
OPTIONS ORIENTATION = LANDSCAPE;
OPTIONS PS=39 LS=122 PAGENO=1;
PROC REPORT DATA=ALL_DAT NOWD SPACING=2 HEADSKIP SPLIT='/';
WHERE AC_ELS_LON = 'E';
TITLE "PROMISSORY NOTE RECONCILIATION REPORT ESIGNS";
FOOTNOTE  'JOB = PNOTE RECONCILIATION    REPORT = R4';
COLUMN BF_SSN DM_PRS_1 DM_PRS_LST AD_BR_SIG AF_APL_OPS_SCL AF_CUR_APL_OPS_LDR MPN_ID_DIGTS 
	AC_ELS_LON AC_LON_STA_TYP PNOTE ;
DEFINE BF_SSN / DISPLAY GROUP "SSN" LEFT;
DEFINE DM_PRS_1 / DISPLAY GROUP "FIRST NAME" WIDTH=10;
DEFINE DM_PRS_LST / DISPLAY GROUP "LAST NAME" WIDTH=15;
DEFINE AD_BR_SIG / DISPLAY "STD SIGN" FORMAT=MMDDYY10.;
DEFINE AF_APL_OPS_SCL / DISPLAY "SCHOOL";
DEFINE AF_CUR_APL_OPS_LDR / DISPLAY "ORIG/LNDR" WIDTH=6;
DEFINE MPN_ID_DIGTS / DISPLAY "BASE/MPN ID" WIDTH=17;
DEFINE AC_ELS_LON / DISPLAY "E-SIGN" WIDTH=6; 
DEFINE AC_LON_STA_TYP / DISPLAY "LOAN/STATUS" WIDTH=6;
DEFINE PNOTE / DISPLAY "P-NOTE" WIDTH=7;
COMPUTE AFTER BF_SSN;
LINE ' ';
...
FORMAT BF_SSN $9. ;
FORMAT MPN_ID_DIGTS $5. ;
FORMAT AD_BR_SIG MMDDYY10.;
FORMAT BASE_LOANS $39. ;
FORMAT SER_LOANS $600. ;
DO;
