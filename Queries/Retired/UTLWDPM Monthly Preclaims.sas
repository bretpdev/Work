...
quit;
ENDRSUBMIT;
proc sort data=worklocl.preclm nodupkey;
        by ssno appid timestmp;
run;

proc sort data=worklocl.cancels nodupkey;
        by ssno appid timestmp;
run;

proc sort data=worklocl.deft nodupkey;
        by ssno appid timestmp;
run;

data worklocl.combine(drop=ssno);
   set worklocl.preclm worklocl.cancels worklocl.deft;
   ssn=ssno+0;
run;

proc sort data=worklocl.combine;
   by ssn;
run;

data worklocl.pcaaccnt;
        set worklocl.combine;
        by ssn;
        if first.ssn then baltot=0;
        baltot+balance;
        if last.ssn then output worklocl.pcaaccnt;
run;
...
%rptprnt(worklocl.pcaaccnt);
options pageno = 1;
*proc printto print=report4 ;
*run;
proc print data = worklocl.combine split ='/';
var 
	ssn
	appid
	timestmp
