...
OPTIONS		NOCENTER DATE NUMBER PAGENO=1 LS=80  ;

PROC PRINT SPLIT='/' DATA = workmatt.DEANF n='Total number of accounts = ' NOOBS;
var SSN CHAPCODE RCV MAXREVIEW;
format  RCV MMDDYY10. MAXREVIEW MMDDYY10. SSN SSN11.;
title1 "Pending Bankruptcies - Complaint Received";
label	RCV = 'Complaint Received Date' MAXREVIEW = 'Last Review Date'
		CHAPCODE = 'Chapter';
where STATUS IN ('01') AND CHAPCODE IN ('07','13');
FOOTNOTE  'JOB = UTLWBK6     REPORT = ULWBK6.LWBK6R2';
RUN;
