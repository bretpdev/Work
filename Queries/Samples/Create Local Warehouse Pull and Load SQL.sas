...
PROC SQL;
	CREATE TABLE FMT AS
		SELECT
			...
			,KEYSEQ
			,CASE
				WHEN TYPENAME IN ('CHARACTER','VARCHAR') AND F.COL IS NULL THEN CATS(':$',NEW_LENGTH,'.') 
				WHEN TYPENAME IN ('CHARACTER','VARCHAR') AND F.COL IS NOT NULL THEN CATS(':',NEW_LENGTH,'.') 
