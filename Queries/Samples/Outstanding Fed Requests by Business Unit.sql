...
WHERE	A.CurrentStatus NOT IN ('Withdrawn','Complete')
		AND UPPER(A.Script) LIKE('%FED%') OR UPPER(A.Script) LIKE('%CORNERSTONE%')
...
FROM	dbo.SCKR_DAT_SASRequests A
		...
		INNER JOIN dbo.SCKR_REF_UnitSAS C
			ON A.Job = C.Program
WHERE	A.CurrentStatus NOT IN ('Withdrawn','Complete')
		AND UPPER(A.Job) LIKE('%FED%') OR UPPER(A.Job) LIKE('%CORNERSTONE%')
ORDER BY C.Unit, A.Request
...
WHERE	A.CurrentStatus NOT IN ('Withdrawn','Complete')
		AND UPPER(A.DocName) LIKE('%FED%') OR UPPER(A.DocName) LIKE('%CORNERSTONE%')
ORDER BY C.Unit, A.Request
