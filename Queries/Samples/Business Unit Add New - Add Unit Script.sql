...
DECLARE @SqlUserId INT = 1152; --use this query to get the CSYS SqlUserId of the manager:  SELECT * FROM CSYS..SYSA_DAT_Users WHERE FirstName = 'wendy' ( this script updates the business unit of the manager of the new business unit in CSYS..SYSA_DAT_Users; other agents are added to the new unit later by Systems Support)
DECLARE @WindowsUserName VARCHAR(50) = 'whack' --use this query to get the BSYS WindowsUserName of the manager: SELECT * FROM BSYS..SYSA_LST_Users WHERE FirstName = 'wendy'
