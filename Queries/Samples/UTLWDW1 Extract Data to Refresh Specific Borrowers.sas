...
LIBNAME XL 'T:\SAS\UDW.XLSX';

DATA UDW (DROP=DF_SPE_ACC_ID);
	SET XL.'SHEET1$'N;
	ACC_ID = PUT(DF_SPE_ACC_ID,Z10.);
RUN;

DATA DUSTER.UDWA; SET UDW; RUN;
...
/*******************************************
* CREATE PRIMER DATA SET WITH SSN INSTEAD OF ACCOUNT NO
********************************************/
PROC SQL;
	CREATE TABLE UDW AS
		SELECT
		...
		FROM
			...
			JOIN OLWHRM1.PD10_PRS_NME PD10
				ON UDWA.ACC_ID = PD10.DF_SPE_ACC_ID
	;
QUIT;
endrsubmit;
...
PROC SQL;
	...
	EXECUTE ( DECLARE GLOBAL TEMPORARY TABLE SESSION.UDW
	          ( DF_PRS_ID  CHAR(9)
	          )
			  ON COMMIT PRESERVE ROWS
	        ) BY DB2;
	INSERT INTO SESSION.UDW
	SELECT * 
	FROM UDW; /* THIS IS WHAT LOADS THE RECORDS FROM UDW INTO THE GLOBAL TEMP TABLE */
	;
	DISCONNECT FROM DB2;
QUIT;
endrsubmit;
...
LAST_RUN = TODAY() - 19958;	*COMMENT FOR PROD, IT WILL READ THE DATE FROM A TABLE;
...
PROC SQL;
	...
	FROM CONNECTION TO DB2 (
		SELECT
			...
			LN90.LD_FAT_EFF,
			LN90.LC_STA_LON90,
			COALESCE(LN90.LA_FAT_CUR_PRI,0) AS LA_FAT_CUR_PRI,
			COALESCE(LN90.LA_FAT_NSI,0) AS LA_FAT_NSI,
			COALESCE(LN90.LA_FAT_LTE_FEE,0) AS LA_FAT_LTE_FEE,
			LN90.LF_LST_DTS_LN90,
			LN90.PC_FAT_TYP,
			LN90.PC_FAT_SUB_TYP,
			LN90.LC_FAT_REV_REA
		FROM
			OLWHRM1.LN90_FIN_ATY LN90
			JOIN SESSION.UDW UDW
				ON LN90.BF_SSN = UDW.DF_PRS_ID
		WHERE
			LN90.LC_FAT_REV_REA ^= '0'
