...
DECLARE @LowerLimit INT = 0 --Change this for min delinquency
DECLARE @UpperLimit INT = 60 --Change this for upper delinquency
DECLARE @MaxNumber INT = 2000 --Max number of emails to send
DECLARE @EmailCampaignId INT = 61 --Director email in live

/*JAMS PART for inserting email campaign requests*/
--DECLARE @EmailCampaignId INT = (SELECT EmailCampaignId FROM CLS.emailbtcf.EmailCampaigns WHERE LetterId = 'MANEMLFED.html') /*TODO: Wait until emailCampaign exists*/
...
/*Dont want to do this in live*/--INSERT INTO CLS.emailbtcf.CampaignData(EmailCampaignId, Recipient, AccountNumber, FirstName, LastName, AddedAt, AddedBy)
