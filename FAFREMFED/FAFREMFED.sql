...
DECLARE @RunMonthOctober TINYINT = 10; --OCTOBER
DECLARE @RunMonthNovember TINYINT = 11; --NOVEMBER
DECLARE @SackerID VARCHAR(100) = 'FAFREMFED';
DECLARE @LetterID VARCHAR(50) = 'FAFSAEMFED.html';
DECLARE @Campaign VARCHAR(50) = 'FAFSA Reminder - FED';
DECLARE @Today DATE = GETDATE();
DECLARE @CurrentMonth TINYINT = MONTH(@Today);

IF @CurrentMonth IN (@RunMonthOctober,@RunMonthNovember) --run only in October & November
	BEGIN
	--attempt INSERT
		BEGIN TRY
			BEGIN TRANSACTION
			...
			FROM 
				...
				LEFT JOIN 
				(--flag for exclusion: CODFS in current year
					SELECT
						BF_SSN
			...
			WHERE
				...
				AND LN10.LA_CUR_PRI > 0.00 --balance greater than zero
	...
	END --only run in October & November
ELSE
	BEGIN
		PRINT @SackerID + '.sql is only run in October & November.'
	END
;
