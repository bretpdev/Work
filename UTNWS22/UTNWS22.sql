		...
		FROM
			...
			(-- USERMOD: identify the applied date for the min forb seq for user modified forbearances (user modified forbearances have an active and inactive row in Ln60 with the same LF_FOR_CTL_NUM)
				...
				FROM
					...
					CDW..LN60_BR_FOR_APV LN60A -- active record for the forbearance
					INNER JOIN CDW..LN60_BR_FOR_APV LN60I -- inactive record for the forbearance
					...
					INNER JOIN CDW..LN60_BR_FOR_APV LN60M
...
BEGIN CATCH
	...
	INSERT INTO ProcessLogs..ProcessLogs (StartedOn, EndedOn, ScriptId, Region, RunBy) VALUES(@RUNDATETIME,@RUNDATETIME,@SCRIPT_ID,'cornserstone',SUSER_SNAME());
