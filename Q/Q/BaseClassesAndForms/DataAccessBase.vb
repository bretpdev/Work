...
#End Region
    ...
    <Obsolete("BSYS will eventually be retired, use CSYS if you can or make sure the script gets added to the list of BSYS scripts")>
    ...
    ''' <summary>
    ''' Returns the business unit that the specified user belongs to, according to CSYS.
    ''' </summary>
    ''' <param name="testMode">True if running in test mode.</param>
    ''' <param name="user">The SqlUser to look up.</param>
    Public Shared Function GetBusinessUnit(ByVal testMode As Boolean, ByVal user As SqlUser) As BusinessUnit
        'TODO: I'm piggybacking on an existing method for now. We probably want to make a dedicated sproc for this.
        Dim units As IEnumerable(Of BusinessUnit) = GetBusinessUnitAgents(testMode).Where(Function(p) p.Agent.ID = user.ID).Select(Function(p) p.Unit)
        If (units.Count() = 0) Then
            Return New BusinessUnit()
        ElseIf (units.Count() = 1) Then
            Return units.Single()
        Else
            'See if there are multiple hits for the same BU (e.g., the user has more than one role in the BU).
            If (units.Select(Function(p) p.ID).Distinct().Count() = 1) Then
                Return units.First()
            Else
                'The user is in more than one BU. That's not right.
                Dim messageBuilder As New StringBuilder()
                messageBuilder.AppendFormat("{0} {1} was found in multiple business units, including the following:{2}", user.FirstName, user.LastName, Environment.NewLine)
                messageBuilder.Append(String.Join(Environment.NewLine, units.Select(Function(p) p.Name).Distinct().ToArray()))
    ...
    ''' Returns the business unit that the specified user belongs to, according to CSYS.
    ''' </summary>
    ''' <param name="mode">The mode NH is being run in</param>
    ''' <param name="user">The SqlUser to look up</param>
    ''' <returns>BusinessUnit</returns>
    ''' <remarks></remarks>
    Public Shared Function GetBusinessUnit(ByVal mode As ConfigurationMode, ByVal user As SqlUser) As BusinessUnit
        Return CSYSDataContext(mode).ExecuteQuery(Of BusinessUnit)("EXEC spGENR_GetBusinessUnit {0}", user.ID).Single()
    End Function

    ''' <summary>
    ''' Returns a collection of BusinessUnitAgent objects representing all roles assigned in the CSYS UserKeyAssignment table.
    ''' </summary>
    ''' <param name="testMode">True if running in test mode.</param>
    Public Shared Function GetBusinessUnitAgents(ByVal testMode As Boolean) As IEnumerable(Of BusinessUnitAgent)
        Dim flatQuery As String = "EXEC spGENR_GetBusinessUnitAgents"
        Dim flats As IEnumerable(Of FlattenedBusinessUnitAgent) = CSYSDataContext(testMode).ExecuteQuery(Of FlattenedBusinessUnitAgent)(flatQuery).ToList()
        Dim users As List(Of SqlUser) = GetSqlUsers(testMode, False)
        Dim units As List(Of BusinessUnit) = GetBusinessUnits(testMode)
        Return flats.Select(Function(p) p.Unflatten(testMode, users, units))
    End Function
    ...
    Public Shared Function GetBusinessUnitAgents(ByVal mode As ConfigurationMode) As IEnumerable(Of BusinessUnitAgent)
        Dim flatQuery As String = "EXEC spGENR_GetBusinessUnitAgents"
        Dim flats As IEnumerable(Of FlattenedBusinessUnitAgent) = CSYSDataContext(mode).ExecuteQuery(Of FlattenedBusinessUnitAgent)(flatQuery).ToList()
        Dim users As List(Of SqlUser) = GetSqlUsers(mode, False)
        Dim units As List(Of BusinessUnit) = GetBusinessUnits(mode)
        Return flats.Select(Function(p) p.Unflatten(users, units))
    End Function
    ...
    ''' <summary>
    ''' Retrieves a SqlUser object from CSYS based on the Windows user name and an 'Active' status.
    ''' </summary>
    ''' <param name="testMode">True if running in test mode.</param>
    ''' <param name="windowsUserName">The user name as reported by .Net's Environment.UserName property.</param>
    ''' <remarks>
    ''' An error will be thrown if there are no active records with the given user name (sequence contains no elements)
    ''' or if there are multiple active records with the given user name (sequence contains more than one element).
    ''' </remarks>
    Public Shared Function GetSqlUser(ByVal testMode As Boolean, ByVal windowsUserName As String) As SqlUser
        Return CSYSDataContext(testMode).ExecuteQuery(Of SqlUser)("EXEC spGetSqlUser {0}", windowsUserName).Single()
    End Function

    ''' <summary>
    ''' Retrieves a list of SqlUser objects from CSYS.
    ''' </summary>
    ''' <param name="testMode">True if running in test mode.</param>
    ''' <param name="includeInactiveRecords">
    ''' If true, former employees will be included in the results.
    ''' If false, only current employees will be included.
    ''' </param>
    ''' <remarks>We may as well have this return a List(Of String) to reassure clients about its behavior.</remarks>
    Public Shared Function GetSqlUsers(ByVal testMode As Boolean, ByVal includeInactiveRecords As Boolean) As IEnumerable(Of SqlUser)
        Return CSYSDataContext(testMode).ExecuteQuery(Of SqlUser)("EXEC spSYSA_GetSqlUsers {0}", includeInactiveRecords).ToList()
    End Function

    ''' <summary>
    ''' Retrieves a list of SqlUser objects from CSYS.
    ''' </summary>
    ''' <param name="mode">Sets up which configuration mode the program is in.</param>
    ''' <param name="includeInactiveRecords">
    ''' If true, former employees will be included in the results.
    ''' If false, only current employees will be included.
    ''' </param>
    ''' <remarks>We may as well have this return a List(Of String) to reassure clients about its behavior.</remarks>
    Public Shared Function GetSqlUsers(ByVal mode As ConfigurationMode, ByVal includeInactiveRecords As Boolean) As IEnumerable(Of SqlUser)
        Return CSYSDataContext(mode).ExecuteQuery(Of SqlUser)("EXEC spSYSA_GetSqlUsers {0}", includeInactiveRecords).ToList()
    End Function
    ...
    ''' <summary>
    ''' DEPRECATED!!!
    ''' Use the GetSqlUser() method instead, followed by the GetBusinessUnit() method if needed.
    ''' </summary>
    <Obsolete("Use the GetSqlUser method instead, followed by the GetBusinessUnit() method if needed")> _
    Public Shared Function GetUser(ByVal testMode As Boolean, ByVal windowsUserID As String) As UserWithBusinessUnit
        Dim queryBuilder As New StringBuilder("SELECT TOP 1")
        queryBuilder.Append(" A.WindowsUserName,")
        queryBuilder.Append(" Coalesce(A.LastName, '') as LName,")
        queryBuilder.Append(" Coalesce(A.FirstName, '') as FName,")
        queryBuilder.Append(" Coalesce(A.[Middle Initial], '') as MI,")
        queryBuilder.Append(" Coalesce(A.Ext, '') as Ext,")
        queryBuilder.Append(" Coalesce(A.Ext2, '') as Ext2,")
        queryBuilder.Append(" A.Gather4PhnDat,")
        queryBuilder.Append(" A.PseudoUser,")
        queryBuilder.Append(" Coalesce(A.PersonalLoanInfoID, '') as PersonalLoanInfoID,")
        queryBuilder.Append(" Coalesce(B.BusinessUnit, '') as BusinessUnit")
        queryBuilder.Append(" FROM dbo.SYSA_LST_Users A")
        queryBuilder.Append(" LEFT JOIN dbo.GENR_REF_BU_Agent_Xref B")
        queryBuilder.Append(" ON A.WindowsUserName = B.WindowsUserID")
        queryBuilder.Append(" AND B.Role = 'Member Of'")
        queryBuilder.Append(" WHERE A.WindowsUserName = {0}")
        Return BsysDataContext(testMode).ExecuteQuery(Of UserWithBusinessUnit)(queryBuilder.ToString(), windowsUserID).SingleOrDefault()
    End Function

    ''' <summary>
    ''' DEPRECATED!!!
    ''' Use the GetBusinessUnitAgents() method instead, and filter based on the desired business unit and role
    ''' (e.g., GetBusinessUnitAgents(testMode).Where(Function(p) p.BusinessUnit.ID == 35 AndAlso p.Role == "Member")).
    ''' </summary>
    <Obsolete("Use the GetBusinessUnitAgents() method instead, and filter based on the desired business unit and role")> _
    Public Shared Function GetUsersInThatRoleForThatBusinessUnit(ByVal testMode As Boolean, ByVal role As String, ByVal businessUnit As String) As List(Of User)
        ...
        queryBuilder.Append(" LEFT JOIN dbo.GENR_REF_BU_Agent_Xref B")
        queryBuilder.Append(" ON A.WindowsUserName = B.WindowsUserID")
        queryBuilder.Append(" WHERE B.Role = {0}")
        queryBuilder.Append(" AND B.BusinessUnit = {1}")
        Return BsysDataContext(testMode).ExecuteQuery(Of User)(queryBuilder.ToString(), role, businessUnit).ToList()
    End Function
    ...
    ''' <summary>
    ''' Gets logon information including favorite screen and logon region/mode.
    ''' </summary>
    ''' <param name="testMode">True if running in test mode.</param>
    Public Shared Function GetLogonInfo(ByVal testMode As Boolean) As UserLogonInfo
        Dim query As String = "SELECT TOP 1 FavoriteScreen, LogonRegion FROM SYSA_LST_UserLogonInfo WHERE WindowsUserID = {0}"
        Return BsysDataContext(testMode).ExecuteQuery(Of UserLogonInfo)(query, Environment.UserName).SingleOrDefault()
    End Function
...
#Region "Projection classes"
    Private Class FlattenedBusinessUnitAgent
        ...
        Public Function Unflatten(ByVal testMode As Boolean, ByVal users As List(Of SqlUser), ByVal units As List(Of BusinessUnit)) As BusinessUnitAgent
            Dim agent As New BusinessUnitAgent()
            agent.Role = Role
            agent.Agent = users.Single(Function(p) p.ID = SqlUserId)
            agent.Unit = units.Single(Function(p) p.ID = BusinessUnitId)
            Return agent
        End Function

        Public Function Unflatten(ByVal users As List(Of SqlUser), ByVal units As List(Of BusinessUnit)) As BusinessUnitAgent
            Dim agent As New BusinessUnitAgent()
            agent.Role = Role
            If users.Any(Function(p) p.ID = SqlUserId) Then
                agent.Agent = users.Single(Function(p) p.ID = SqlUserId)
            Else
                agent.Agent = Nothing
            End If
            If units.Any(Function(p) p.ID = BusinessUnitId) Then
                agent.Unit = units.Single(Function(p) p.ID = BusinessUnitId)
            Else
                agent.Unit = Nothing
            End If
