...
#Region "LG02"
    ...
    Public Shared Function GetLoanInfoFromLG02(ByVal rs As Session) As LG02LoanInfo
        If ReflectionInterface.Check4Text(rs, 1, 56, "CONSOLIDATION APPLICATION") Then
            Return New LG02LoanInfo With {
                .Cluid = ReflectionInterface.GetText(rs, 5, 31, 19),
                .LoanHolderCode = ReflectionInterface.GetText(rs, 11, 46, 6),
                .LoanServicerCode = ReflectionInterface.GetText(rs, 11, 71, 6),
                .LoanType = "CL",
                .StudentName = String.Empty,
                .StudentSsn = String.Empty
            }
        ElseIf ReflectionInterface.Check4Text(rs, 1, 60, "PLUS LOAN APPLICATION") Then
            Dim loanInfo As New LG02LoanInfo With {
                .LoanType = "PL",
                .StudentName = ReflectionInterface.GetText(rs, 9, 8, 9),
                .StudentSsn = ReflectionInterface.GetText(rs, 9, 18, 30)
            }
        ...
        ElseIf ReflectionInterface.Check4Text(rs, 1, 60, "PLUS MASTER PROM NOTE") Then
            Dim loanInfo As New LG02LoanInfo With {
                .Cluid = ReflectionInterface.GetText(rs, 3, 30, 19),
                .LoanHolderCode = ReflectionInterface.GetText(rs, 18, 41, 6),
                .LoanServicerCode = ReflectionInterface.GetText(rs, 18, 64, 6)
            }
        ...
        ElseIf ReflectionInterface.Check4Text(rs, 1, 61, "SLS LOAN APPLICATION") Then
            Return New LG02LoanInfo With {
                .Cluid = ReflectionInterface.GetText(rs, 2, 33, 19),
                .LoanHolderCode = ReflectionInterface.GetText(rs, 19, 44, 6),
                .LoanServicerCode = ReflectionInterface.GetText(rs, 19, 73, 6),
                .LoanType = "SL",
                .StudentName = String.Empty,
                .StudentSsn = String.Empty
            }
        ElseIf ReflectionInterface.Check4Text(rs, 1, 56, "STAFFORD MASTER PROM NOTE") Then
            Dim loanInfo As New LG02LoanInfo With {
                .Cluid = ReflectionInterface.GetText(rs, 3, 32, 19),
                .LoanHolderCode = ReflectionInterface.GetText(rs, 18, 49, 6),
                .LoanServicerCode = ReflectionInterface.GetText(rs, 18, 73, 6),
                .StudentName = String.Empty,
                .StudentSsn = String.Empty
            }
        ...
        ElseIf ReflectionInterface.Check4Text(rs, 1, 56, "STAFFORD LOAN APPLICATION") Then
            Dim loanInfo As New LG02LoanInfo With {
                .Cluid = ReflectionInterface.GetText(rs, 3, 33, 19),
                .LoanHolderCode = ReflectionInterface.GetText(rs, 18, 49, 6),
                .LoanServicerCode = ReflectionInterface.GetText(rs, 18, 73, 6),
                .StudentName = String.Empty,
                .StudentSsn = String.Empty
...
#Region "LP50Add"
    ...
    Public Shared Function AddCommentInLP50(ByVal rs As Session, ByVal ssn As String, ByVal arc As String, ByVal scriptId As String, Optional ByVal activityType As String = "", Optional ByVal contactType As String = "", Optional ByVal comment As String = "", Optional ByVal pauseForUserComments As Boolean = False, Optional ByVal isReference As Boolean = False) As Boolean
        ...
        Do Until row = 21
            If ReflectionInterface.Check4Text(rs, row, column, "____________", "            ") AndAlso
...
#Region "LPEM"
    ...
    Public Shared Function GetEmployerInfoFromLPEM(ByVal rs As Session, ByVal employerId As String, ByVal ParamArray preferredDepartments() As String) As EmployerDemographics
        ...
        Dim demographics As New EmployerDemographics With {
            .Addr1 = ReflectionInterface.GetText(rs, 8, 21, 40),
            .Addr2 = ReflectionInterface.GetText(rs, 9, 21, 40),
            .City = ReflectionInterface.GetText(rs, 11, 21, 30),
            .State = ReflectionInterface.GetText(rs, 11, 59, 2),
            .Zip = ReflectionInterface.GetText(rs, 11, 66, 9),
            .Name = ReflectionInterface.GetText(rs, 5, 21, 40)
            }
        Return demographics
    End Function
...
#Region "CreateDataTableFromFile"
    ...
    Public Shared Function CreateDataTableFromFile(ByVal fileName As String, ByVal firstRowIsHeader As Boolean, ByVal fields As Dictionary(Of String, Type)) As DataTable
        ...
        Using fileReader As New StreamR(fileName)
...
#Region "Login"
    ''' <summary>
    ''' Log in to a session during the execution of a script.
    ''' </summary>
    ''' <param name="rs">The Reflection Session</param>
    Public Shared Sub Login(ByVal rs As Session, ByVal testMode As Boolean)
        'Get the region.
        'FileOpen(1, DataAccessBase.PersonalDataDirectory & "userinfo.txt", OpenMode.Input)
        'For i As Integer = 1 To 6
        '    'Skip over the unneeded fields.
        '    VbaStyleFileInput(1)
        'Next i
        'Dim aesRegion As String = VbaStyleFileInput(1)
        'FileClose(1)
        Dim logonInfo As UserLogonInfo = Q.DataAccessBase.GetLogonInfo(testMode)
        Dim aesRegion As String = logonInfo.LogonRegion

        rs.ScrollLockState = False
        rs.Disconnect()
        rs.WaitForEvent(rcKbdEnabled, "30", "0", 1, 1)
        rs.Connect()
        rs.WaitForDisplayString(">", "0:0:30", 16, 10)
        If aesRegion = "Live" Then
            'Ensure the color of updateable fields is green.
            rs.SetColorMap(rcUnprotNormAlpha, rcGreen, rcBlack)
            ReflectionInterface.PutText(rs, 16, 12, "PHEAA", ReflectionInterface.Key.Enter)
        Else
            ReflectionInterface.PutText(rs, 16, 12, "QTOR", ReflectionInterface.Key.Enter)
        End If
        Try
            'TODO: Where do we get the MBS user ID and password?
            'If CalledByMasterBatchScript(rs) Then
            '    'enter the user ID and password entered in MBS
            '    rs.WaitForDisplayString(">", "0:0:30", 20, 16)
            '    ReflectionInterface.PutText(rs, 20, 18, userId)
            '    ReflectionInterface.PutText(rs, 20, 40, password, ReflectionInterface.Key.Enter)
            'Else
            'Pause for the user to enter the their ID and password.
            Do
                rs.WaitForTerminalKey(rcIBMInsertKey, "1:0:0")
                'Hit ENTER if the user didn't already.
                If ReflectionInterface.Check4Text(rs, 20, 8, "USERID") OrElse ReflectionInterface.Check4Text(rs, 8, 22, "USERID") Then
                    ReflectionInterface.Hit(rs, ReflectionInterface.Key.Enter)
                Else
                    ...
                    Exit Do
                End If
                'Exit the loop only if no user ID or password errors appear.
                If Not ReflectionInterface.Check4Text(rs, 23, 2, "ON") AndAlso Not ReflectionInterface.Check4Text(rs, 16, 2, "ON") Then
                    Exit Do
                End If
            Loop
            'End If

            'Log the user into the test region if needed.
            If aesRegion <> "Live" Then
                rs.SetColorMap(rcUnprotNormAlpha, rcMagenta, rcBlack)
                aesRegion = "Test"
                'Select the UT compass region.
                If rs.FindText("_ RS/UT", 3, 3) Then
                    ReflectionInterface.PutText(rs, rs.FoundTextRow, rs.FoundTextColumn, "X", ReflectionInterface.Key.Enter)
                Else
                    ReflectionInterface.PutText(rs, 3, 3, "X", ReflectionInterface.Key.Enter)
                End If
            End If
        Catch ex As Exception
            'TODO: The original code does nothing for error handling. Ideas are welcome.
        End Try
        Do
            'Access LP00 to verify that login was successful.
            ReflectionInterface.FastPath(rs, "LP00")
            If ReflectionInterface.Check4Text(rs, 3, 32, "M A I N   M E N U") Then
                Exit Do
            End If
            Dim message As String = String.Format("You are not logged in to the {0} region.  Click OK to pause the script so you can log in manually.  Hit Insert to resume processing once you are logged in.", aesRegion)
            MessageBox.Show(message, "Login Failed", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            rs.WaitForTerminalKey(rcIBMInsertKey, "2:0:0")
            ReflectionInterface.Hit(rs, ReflectionInterface.Key.Insert)
        Loop
    End Sub
    ...
    Public Shared Function GetNextPopulatedFile(ByVal path As String, ByVal searchPattern As String) As String
        ...
        If nextPopulatedFile.Length > 0 Then
            While New FileInfo(path + nextPopulatedFile).Length = 0
                FS.Delete(path + nextPopulatedFile)
...
#Region "TestMode"
    ...
    Public Shared Function TestMode(ByVal docFolder As String, ByRef tempTestMode As Boolean) As TestModeResults
        ...
        If tempTestMode Then
            ...
            ReturnResults.FtpFolder = Uheaa.Common.DataAccess.EnterpriseFileSystem.FtpFolder + "Test\"
            ReturnResults.DocFolder = docFolder & "Test\"
            ReturnResults.LogFolder = Uheaa.Common.DataAccess.EnterpriseFileSystem.LogsFolder + "Test\"
        Else
            ReturnResults.IsInTestMode = False
            ReturnResults.FtpFolder = Uheaa.Common.DataAccess.EnterpriseFileSystem.FtpFolder
            ReturnResults.DocFolder = docFolder
            ReturnResults.LogFolder = Uheaa.Common.DataAccess.EnterpriseFileSystem.LogsFolder
        End If
...
#Region "Unix"
    ...
    Public Shared Sub Dos2UnixNewline(ByVal fileName As String)
        ...
        Try
            Using fileReader As New StreamR(fileName)
                Using tempWriter As New StreamW(tempFile)
...
#End Region
    ...
    Public Shared Function HasWriteAccess() As Boolean
        ...
        Try
            'Create a temporary file that is named "TempFileTestAccess"
            TempFileName = "TempfileTestAccess"
            'Try and create file if an error occures goto ErrorHandler else
            'close the file and delete it.
            FileOpen(3, CurDir() & "\" & TempFileName, OpenMode.Output)
            FileClose(3)
            FS.Delete(CurDir() & "\" & TempFileName)
            Return True
        Catch ex As Exception
            Return False
        End Try
