...
DECLARE @LastRefresh VARCHAR(30) = (SELECT CONVERT(VARCHAR(30), ISNULL(MAX(PH05.DF_LST_DTS_PH05), '1-1-1900 00:00:00'), 21) FROM PH05_CNC_EML PH05)
...
'
	...
	USING
		...
		) L ON L.DF_CNC_SYS_ID = PH05.DF_CNC_SYS_ID AND L.DF_SPE_ID = PH05.DF_SPE_ID -- add in DF_SPE_ID leading zeros; PH05 data type is decimal
	WHEN MATCHED THEN 
		UPDATE SET 
...
BEGIN
	RAISERROR('PH05_CNC_EML - The remote and local record counts do not match.  The local count is off by %i records.  A full refresh of the table is required.', 16, 11, @CountDifference)
END