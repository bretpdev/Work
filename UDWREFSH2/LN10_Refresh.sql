...
ELSE IF @CountDifference != 0 AND @LoopCount = 0
	BEGIN
		...
		PRINT 'The local record count for these SSNs does not match the remote warehouse count: ' + @SSNs + '  Deleting all local LN10 records for these borrowers fullying refreshing from the remote warehouse.'

		DELETE FROM
			LN10
		FROM
			UDW..LN10_LON LN10
			INNER JOIN @SSN_LIST SL ON COALESCE(SL.L_BF_SSN, SL.R_BF_SSN) = LN10.BF_SSN
