CREATE PROCEDURE [dbo].[LT_TS06IDAPBR_Loans]
...
DECLARE	@LetterId VARCHAR(10) = 'TS06IDAPBR'
...
IF @IsCoborrower = 0
	BEGIN
		SELECT 
			LN10.BF_SSN,
			LN10.LN_SEQ,
			COALESCE(FMT.Label, LN10.IC_LON_PGM) AS [Loan_Program],
			CONVERT(VARCHAR, LN10.LD_LON_1_DSB, 101) AS [Date_Disbursed],
			LN72INT.LR_ITR AS [Interest],
			'$' + CAST(LN10.LA_LON_AMT_GTR AS VARCHAR) AS [Original_Principal],
			'$' + CAST(LN10.LA_CUR_PRI AS VARCHAR) AS [Current_Principal]
		FROM 
			PD10_PRS_NME PD10
			...
			( 
				SELECT  
					...
					LN72.LN_SEQ,
					LN72.LR_ITR, 
					ROW_NUMBER() OVER (PARTITION BY LN72.BF_SSN, LN72.LN_SEQ ORDER BY LD_STA_LON72 DESC) AS LN_ITR_SEQ
				FROM  
				...
				ON LN72INT.BF_SSN = LN10.BF_SSN 
				AND LN72INT.LN_SEQ = LN10.LN_SEQ
				AND LN72INT.LN_ITR_SEQ = 1
			INNER JOIN
			(
				SELECT
					LN85.BF_SSN,
