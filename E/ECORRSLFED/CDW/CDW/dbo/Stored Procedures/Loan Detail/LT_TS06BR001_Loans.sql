CREATE PROCEDURE [dbo].[LT_TS06BR001_Loans]
...
DECLARE	@LetterId VARCHAR(10) = 'TS06BR001'
...
IF @IsCoborrower = 0
	BEGIN
		SELECT DISTINCT
			...
			'$' + CAST(LN10.LA_LON_AMT_GTR AS VARCHAR) AS [Original Principal Balance],
			'$' + CAST(COALESCE(LN90.LA_FAT_CUR_PRI,0) + COALESCE(LN90.LA_FAT_NSI, 0) + COALESCE(LN90.LA_FAT_LTE_FEE,0) AS VARCHAR) AS [Refund Amount],
			CONVERT(VARCHAR(10), (MAX(LN90.LD_FAT_EFF) OVER (PARTITION BY LN90.BF_SSN, LN90.LN_SEQ, LN90.LN_FAT_SEQ)), 101) as [Refund Date],
