			...
			CASE
				WHEN BD.ORD = 0 THEN
					CASE 
						...
						ELSE '99'
					END
				WHEN BD.ORD = 1 AND LD_PIF_RPT BETWEEN CAST(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0) AS DATE) AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) THEN 'PIF'
				WHEN BD.ORD = 2 AND LC_STA_LON10 = 'D' AND LA_OTS_PRI_ELG  = 0 AND LD_STA_LON10 BETWEEN CAST(DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE())-1, 0) AS DATE) AND CAST(DATEADD(D,-(DAY(GETDATE())),GETDATE()) AS DATE) THEN 'TRN'
