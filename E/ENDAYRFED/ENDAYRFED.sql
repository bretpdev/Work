...
--DECLARE @RunMonth TINYINT = 10; --TEST CHANGE TO CURRENT MONTH OF YEAR
DECLARE @RunMonth TINYINT = 5; --MAY --LIVE

DECLARE @SackerID VARCHAR(100) = 'ENDAYRFED';
DECLARE @LetterID VARCHAR(50) = 'ACYREMLFED.html';
DECLARE @Campaign VARCHAR(50) = 'End of Academic Year - FED';

DECLARE @Today DATE = GETDATE();
DECLARE @CurrentMonth TINYINT = MONTH(@Today);

IF @CurrentMonth = @RunMonth --LIVE runs only in May
	BEGIN
	--attempt INSERT
		BEGIN TRY
			BEGIN TRANSACTION
			...
			FROM 
				...
				LEFT JOIN 
				(--flag for exclusion: CODEY in current year
					SELECT
						BF_SSN
...
ELSE
	BEGIN
		PRINT @SackerID + '.sql is only run in May.'
	END
;
